...
public class ArrSeqTest{
            ...
            for(int pre1=0;pre1 <= 5;pre1+=5){
                ...
                for(int post0=0;post0 <= 5;post0+=5){
                    for(int post1=0;post1 <= 5;post1+=5){
                        ...
                        for(final var checkedType:CheckedType.values()){
                            for(final var collectionType:DataType.values()){
                                final var subListParams=new SequenceInitParams(StructType.ArrSubList,collectionType,
    ...
    static MonitoredList<? extends AbstractSeq<?>> getMonitoredList(SequenceInitParams initParams,
        ...
        case ArrSubList:{
            ...
            for(int i=1;i < preAllocs.length;++i){
                ...
                SequenceInitialization.Ascending.initialize(subListMonitor,postAlloc,
                        Integer.MAX_VALUE - totalPostAlloc);
    ...
    public void testcontains_val(){
        final QueryTest<?> test=(monitor,queryVal,inputType,castType,modification,monitoredObjectGen,position,
                seqSize)->{
                    ...
                    TestExecutorService.submitTest(()->{
                        ...
                        for(final var functionGen:initParams.structType.validMonitoredFunctionGens){
                            if((functionGen.expectedException == null || size > 0)
                                    && (initParams.checkedType.checked || functionGen.expectedException == null)){
                                                ...
                                                if(illegalMod.expectedException == null){
                                                    if(functionGen.expectedException == null){
                                                        ...
                                                        Assertions.assertThrows(functionGen.expectedException,
                                                                ()->seqMonitor.verifyForEach(functionGen,
                                                                        functionCallType,finalRandSeed));
                                                        seqMonitor.repairModCount();
                                                    }
                                                }else{
                                                    seqMonitor.illegalMod(illegalMod);
                                                    final long finalRandSeed=randSeed;
                                                    Assertions.assertThrows(illegalMod.expectedException,()->seqMonitor
                                                            .verifyForEach(functionGen,functionCallType,finalRandSeed));
                                                    seqMonitor.repairModCount();
                                                }
                                            }
                                        }
                                        ...
                                        if((illegalMod.expectedException == null || numLeft > 0)
                                            ...
                                            for(final var functionGen:itrType.validMonitoredFunctionGens){
                                                if((functionGen.expectedException == null || numLeft > 0)
                                                        && (initParams.checkedType.checked || size == 0
    ...
    public void testpoll_void(){
        for(final var initParams:STACK_STRUCT_INIT_PARAMS){
            for(final var outputType:initParams.collectionType.validOutputTypes()){
                TestExecutorService.submitTest(()->{
                    final var monitor=SequenceInitialization.Ascending.initialize(getMonitoredStack(initParams,100),100,
                            0);
                    for(int i=0;;++i){
                                                ...
                                                for(final var threshold:thresholdArr){
                                                    TestExecutorService.submitTest(()->{
                                                        final var monitor=SequenceInitialization.Ascending.initialize(
                                                                getMonitoredSequence(initParams,size),size,initVal,
                                                                period);
                            ...
                            if(illegalMod.expectedException == null){
                                for(final var functionCallType:initParams.collectionType.validFunctionCalls){
                                    if(initParams.checkedType.checked){
                                        Assertions.assertThrows(IndexOutOfBoundsException.class,
                                                ()->monitor.verifySet(-1,
                                                        initParams.collectionType.convertValUnchecked(0),
                                                        functionCallType));
                                        Assertions.assertThrows(IndexOutOfBoundsException.class,
                                                ()->monitor.verifySet(size,
                                                        initParams.collectionType.convertValUnchecked(size + 1),
                                                        functionCallType));
                                    }
                                    for(int index=0;index < size;++index){
                                        monitor.verifySet(index,
                                                initParams.collectionType.convertValUnchecked(index + 1),
                                                functionCallType);
                                    }
                                }
                            }else{
                                        ...
                                        TestExecutorService.submitTest(()->{
                                            ...
                                            if(illegalMod.expectedException == null){
                                                if(size < 2 || comparatorGen.expectedException == null){
                                                    ...
                                                    Assertions.assertThrows(comparatorGen.expectedException,()->monitor
                                                            .verifyAscendingUnstableSort(size,comparatorGen));
                                                }
                                            }else{
                                                monitor.illegalMod(illegalMod);
                                                Assertions.assertThrows(illegalMod.expectedException,
                                        ...
                                        TestExecutorService.submitTest(()->{
                                            ...
                                            if(illegalMod.expectedException == null){
                                                if(size < 2 || comparatorGen.expectedException == null){
                                                    ...
                                                    Assertions.assertThrows(comparatorGen.expectedException,()->monitor
                                                            .verifyDescendingUnstableSort(size,comparatorGen));
                                                }
                                            }else{
                                                monitor.illegalMod(illegalMod);
                                                Assertions.assertThrows(illegalMod.expectedException,
    ...
    private static class ArrListMonitor<SEQ extends AbstractSeq<E>&OmniList<E>&Externalizable,E>
        ...
        ArrListMonitor(CheckedType checkedType,DataType dataType,int initCap){
            super(checkedType,dataType,initCap);
        }
        ArrListMonitor(SequenceInitParams initParams){
            super(initParams.checkedType,initParams.collectionType);
        }
        ArrListMonitor(SequenceInitParams initParams,int initCap){
            ...
            private class ItrMonitor extends AbstractItrMonitor<OmniIterator<?>>{
                ...
                public void verifyCloneHelper(Object clone){
                    ...
                    case BYTE:
                        if(checked){
                            Assertions.assertSame(seq,
                                    FieldAndMethodAccessor.ByteArrSeq.CheckedSubList.Itr.parent(clone));
                            Assertions.assertEquals(expectedCursor,
                            ...
                            Assertions.assertSame(seq,
                                    FieldAndMethodAccessor.ByteArrSeq.UncheckedSubList.Itr.parent(clone));
                            Assertions.assertEquals(expectedCursor,
                    ...
                    case CHAR:
                        if(checked){
                            Assertions.assertSame(seq,
                                    FieldAndMethodAccessor.CharArrSeq.CheckedSubList.Itr.parent(clone));
                            Assertions.assertEquals(expectedCursor,
                            ...
                            Assertions.assertSame(seq,
                                    FieldAndMethodAccessor.CharArrSeq.UncheckedSubList.Itr.parent(clone));
                            Assertions.assertEquals(expectedCursor,
                    ...
                    case DOUBLE:
                        if(checked){
                            Assertions.assertSame(seq,
                                    FieldAndMethodAccessor.DoubleArrSeq.CheckedSubList.Itr.parent(clone));
                            Assertions.assertEquals(expectedCursor,
                    ...
                    case FLOAT:
                        if(checked){
                            Assertions.assertSame(seq,
                                    FieldAndMethodAccessor.FloatArrSeq.CheckedSubList.Itr.parent(clone));
                            Assertions.assertEquals(expectedCursor,
                    ...
                    case INT:
                        if(checked){
                            Assertions.assertSame(seq,
                                    FieldAndMethodAccessor.IntArrSeq.CheckedSubList.Itr.parent(clone));
                            Assertions.assertEquals(expectedCursor,
                            ...
                            Assertions.assertSame(seq,
                                    FieldAndMethodAccessor.IntArrSeq.UncheckedSubList.Itr.parent(clone));
                            Assertions.assertEquals(expectedCursor,
                    ...
                    case LONG:
                        if(checked){
                            Assertions.assertSame(seq,
                                    FieldAndMethodAccessor.LongArrSeq.CheckedSubList.Itr.parent(clone));
                            Assertions.assertEquals(expectedCursor,
                            ...
                            Assertions.assertSame(seq,
                                    FieldAndMethodAccessor.LongArrSeq.UncheckedSubList.Itr.parent(clone));
                            Assertions.assertEquals(expectedCursor,
                    ...
                    case REF:
                        if(checked){
                            Assertions.assertSame(seq,
                                    FieldAndMethodAccessor.RefArrSeq.CheckedSubList.Itr.parent(clone));
                            Assertions.assertEquals(expectedCursor,
                            ...
                            Assertions.assertSame(seq,
                                    FieldAndMethodAccessor.RefArrSeq.UncheckedSubList.Itr.parent(clone));
                            Assertions.assertEquals(expectedCursor,
                    ...
                    case SHORT:
                        if(checked){
                            Assertions.assertSame(seq,
                                    FieldAndMethodAccessor.ShortArrSeq.CheckedSubList.Itr.parent(clone));
                            Assertions.assertEquals(expectedCursor,
        ...
        private class ListItrMonitor extends AbstractItrMonitor<OmniListIterator<?>>
            ...
            public void verifyCloneHelper(Object clone){
                ...
                case BOOLEAN:
                    if(checkedType.checked){
                        Assertions.assertSame(seq,
                                FieldAndMethodAccessor.BooleanArrSeq.CheckedList.ListItr.parent(clone));
                        Assertions.assertEquals(expectedCursor,
                        ...
                        Assertions.assertSame(seq,
                                FieldAndMethodAccessor.BooleanArrSeq.UncheckedList.ListItr.parent(clone));
                        Assertions.assertEquals(expectedCursor,
                ...
                case BYTE:
                    if(checkedType.checked){
                        ...
                        Assertions.assertSame(seq,
                                FieldAndMethodAccessor.ByteArrSeq.UncheckedList.ListItr.parent(clone));
                        Assertions.assertEquals(expectedCursor,
                ...
                case CHAR:
                    if(checkedType.checked){
                        ...
                        Assertions.assertSame(seq,
                                FieldAndMethodAccessor.CharArrSeq.UncheckedList.ListItr.parent(clone));
                        Assertions.assertEquals(expectedCursor,
                ...
                case SHORT:
                    if(checkedType.checked){
                        ...
                        Assertions.assertSame(seq,
                                FieldAndMethodAccessor.ShortArrSeq.UncheckedList.ListItr.parent(clone));
                        Assertions.assertEquals(expectedCursor,
                ...
                case LONG:
                    if(checkedType.checked){
                        ...
                        Assertions.assertSame(seq,
                                FieldAndMethodAccessor.LongArrSeq.UncheckedList.ListItr.parent(clone));
                        Assertions.assertEquals(expectedCursor,
                ...
                case FLOAT:
                    if(checkedType.checked){
                        ...
                        Assertions.assertSame(seq,
                                FieldAndMethodAccessor.FloatArrSeq.UncheckedList.ListItr.parent(clone));
                        Assertions.assertEquals(expectedCursor,
                ...
                case DOUBLE:
                    if(checkedType.checked){
                        Assertions.assertSame(seq,
                                FieldAndMethodAccessor.DoubleArrSeq.CheckedList.ListItr.parent(clone));
                        Assertions.assertEquals(expectedCursor,
                        ...
                        Assertions.assertSame(seq,
                                FieldAndMethodAccessor.DoubleArrSeq.UncheckedList.ListItr.parent(clone));
                        Assertions.assertEquals(expectedCursor,
    ...
    private static interface MonitoredFunctionTest<MONITOR extends MonitoredSequence<? extends AbstractSeq<?>>>{
        ...
        @SuppressWarnings("unchecked")
        private void runAllTests(String testName,long maxRand,boolean useAllStructs){
                                            ...
                                            if(size == 0 || objGen.expectedException == null){
                                                ...
                                                final var monitor=SequenceInitialization.Ascending
                                                        .initializeWithMonitoredObj(
                                                                getMonitoredSequence(initParams,size),size,0,objGen,
                                                                throwSwitch);
                                                monitor.illegalMod(illegalMod);
                                                final Class<? extends Throwable> expectedException=illegalMod.expectedException == null
