...
public class OpenAddressHashSetTest{
        ...
        for(final var collectionType:StructType.OpenAddressHashSet.validDataTypes){
            for(final var checkedType:CheckedType.values()){
                TestExecutorService.submitTest(
                        ()->new OpenAddressHashSetMonitor(collectionType,checkedType).verifyCollectionState());
            }
        }
    ...
    private static void testItrnext_voidHelper(float loadFactor,int initialCapacity,IllegalModification preMod,
        ...
        final var setMonitor=initSeq
                .initialize(new OpenAddressHashSetMonitor(collectionType,checkedType,initialCapacity,loadFactor));
        final var itrMonitor=setMonitor.getMonitoredIterator();
        itrMonitor.illegalMod(preMod);
            ...
            for(final var functionGen:StructType.OpenAddressHashSet.validMonitoredFunctionGens){
                for(final var initSeq:VALID_INIT_SEQS){
                    final int size=initSeq
                            .initialize(new OpenAddressHashSetMonitor(collectionType,CheckedType.UNCHECKED)).size();
                    for(final var functionCallType:FunctionCallType.values()){
                        if(collectionType != DataType.REF || functionCallType != FunctionCallType.Boxed){
                            final long randSeedBound=functionGen.randomized && size > 1 && !functionCallType.boxed?100
                                    :0;
                            for(final var checkedType:CheckedType.values()){
                                if(checkedType.checked || functionGen.expectedException == null || size == 0){
                                    ...
                                    .forEach(randSeed->TestExecutorService.submitTest(
                                            ()->testforEach_ConsumerHelper(collectionType,functionGen,
                            ...
                            for(final var loadFactor:LOAD_FACTORS){
                                if(loadFactor > 0.f && loadFactor <= 1.0f && loadFactor == loadFactor){
                                    for(final var initialCapacity:GENERAL_PURPOSE_INITIAL_CAPACITIES){
                                        for(final var initSeq:VALID_INIT_SEQS){
                                            TestExecutorService
                                            .submitTest(()->testItrnext_voidHelper(loadFactor,initialCapacity,
                                                    preMod,outputType,collectionType,checkedType,initSeq));
                                        }
                                    }
                                }
                            }
                ...
                for(final var initSeq:VALID_INIT_SEQS){
                    for(final var itrRemoveScenario:IteratorType.AscendingItr.validItrRemoveScenarios){
                        if((!initSeq.isEmpty || itrRemoveScenario == IteratorRemoveScenario.PostInit)
                            ...
                            for(final var preMod:IteratorType.AscendingItr.validPreMods){
                                if(checkedType.checked || preMod.expectedException == null){
                                    final var monitor=initSeq
                                            .initialize(new OpenAddressHashSetMonitor(collectionType,checkedType));
                                                    ...
                                                    TestExecutorService.submitTest(()->testItrremove_voidHelper(
                                                            initSeq.initialize(
                                                                    new OpenAddressHashSetMonitor(collectionType,
                                                                            checkedType,initCapacity,loadFactor)),
            ...
            for(final var checkedType:CheckedType.values()){
                for(final var initSeq:VALID_INIT_SEQS){
                    for(final float loadFactor:LOAD_FACTORS){
                        if(checkedType.checked || loadFactor == loadFactor && loadFactor <= 1.0f && loadFactor > 0){
                            TestExecutorService.submitTest(
                                    ()->initSeq.initialize(new OpenAddressHashSetMonitor(collectionType,checkedType))
    ...
    public void testtoArray_ObjectArray(){
        for(final var collectionType:StructType.OpenAddressHashSet.validDataTypes){
            for(final var initSeq:VALID_INIT_SEQS){
                final int size=initSeq.initialize(new OpenAddressHashSetMonitor(collectionType,CheckedType.UNCHECKED))
                        .size();
                for(final var checkedType:CheckedType.values()){
    ...
    private interface QueryTest{
        ...
        boolean callMethod(OpenAddressHashSetMonitor monitor,QueryVal queryVal,DataType inputType,
        ...
        default void runTest(DataType collectionType,QueryVal queryVal,QueryVal.QueryValModification modification,
            ...
            if(position < 0){
                ...
                case DOUBLE:
                    ...
                    break;
                case REF:
                    queryVal.initContains((OmniSet.OfRef<Object>)monitor.set,setSize,0,position,modification);
                    break;
                default:
                    throw DataType.invalidDataType(collectionType);
                }
    ...
    private static class OpenAddressHashSetMonitor implements MonitoredSet<AbstractOpenAddressHashSet<?>>{
        ...
        @Override
        public AbstractOpenAddressHashSet<?> getCollection(){
            return set;
        }
        @Override
        ...
        @Override
        public MonitoredIterator<? extends OmniIterator<?>,AbstractOpenAddressHashSet<?>> getMonitoredIterator(){
            int expectedOffset;
            int numLeft;
            if((numLeft=expectedSize) != 0){
                outer:for(;;){
                    ...
                    case REF:{
                        Object[] table=(Object[])expectedTable;
                        for(expectedOffset=table.length;;){
                            Object tableVal;
        ...
        public void verifyRemoveIf(boolean result,MonitoredRemoveIfPredicate filter){
            ...
            if(result){
                ...
                case FLOAT:{
                    do{
                        float v;
                        Assertions.assertFalse(set.contains(v=(float)removedValsItr.next()));
                        removeVal(v);
                    }while(removedValsItr.hasNext());
                    while(retainedValsItr.hasNext()){
                        Assertions.assertTrue(set.contains((float)retainedValsItr.next()));
                    }
                    break;
                }
                case DOUBLE:{
                    do{
                        double v;
                        Assertions.assertFalse(set.contains(v=(double)removedValsItr.next()));
                        removeVal(v);
                    }while(removedValsItr.hasNext());
                    while(retainedValsItr.hasNext()){
                        Assertions.assertTrue(set.contains((double)retainedValsItr.next()));
                    }
                    break;
                }
                case REF:{
                    do{
                        Object v;
                        Assertions.assertFalse(set.contains(v=removedValsItr.next()));
                        removeVal(v);
                    }while(removedValsItr.hasNext());
                    while(retainedValsItr.hasNext()){
                        Assertions.assertTrue(set.contains(retainedValsItr.next()));
                    }
                    break;
                }
                default:
        ...
        private void insertInTable(Object v){
            ...
            if((t=(Object[])expectedTable) == null){
                ...
                int insertHere=-1;
                int expectedTableLength;
                insertInTable:for(int initialHash=hash&=expectedTableLength=this.expectedTableLength - 1;;){
                    Object existingVal;
        ...
        private class UncheckedItrMonitor extends AbstractItrMonitor{
            ...
            public void verifyIteratorState(Object itr){
                ...
                case FLOAT:
                    ...
                    Assertions.assertEquals(expectedOffset,
                            FieldAndMethodAccessor.FloatOpenAddressHashSet.Itr.offset(itr));
                    break;
                case DOUBLE:
                    Assertions.assertSame(set,FieldAndMethodAccessor.DoubleOpenAddressHashSet.Itr.root(itr));
                    Assertions.assertEquals(expectedOffset,
                            FieldAndMethodAccessor.DoubleOpenAddressHashSet.Itr.offset(itr));
                    break;
                case REF:
                    Assertions.assertSame(set,FieldAndMethodAccessor.RefOpenAddressHashSet.Itr.root(itr));
                    Assertions.assertEquals(expectedOffset,
                            FieldAndMethodAccessor.RefOpenAddressHashSet.Itr.offset(itr));
                    break;
                default:
                    throw DataType.invalidDataType(dataType);
                }
            ...
            public void updateItrRemoveState(){
                ...
                case FLOAT:{
                    int[] table=(int[])expectedTable;
                    for(int offset=this.expectedOffset;;){
                        switch(table[++offset]){
                        default:
        ...
        private class CheckedItrMonitor extends AbstractItrMonitor{
            ...
            public void verifyIteratorState(Object itr){
                ...
                case REF:
                    ...
                    Assertions.assertEquals(expectedOffset,
                            FieldAndMethodAccessor.RefOpenAddressHashSet.Checked.Itr.offset(itr));
                    Assertions.assertEquals(expectedItrModCount,
        ...
        implements
        MonitoredSet.MonitoredSetIterator<OmniIterator<?>,AbstractOpenAddressHashSet<?>>{
            ...
            @Override
            public MonitoredCollection<AbstractOpenAddressHashSet<?>> getMonitoredCollection(){
                    ...
                    for(final var initSeq:VALID_INIT_SEQS){
                        for(final var loadFactor:LOAD_FACTORS){
                            if(loadFactor > 0.f && loadFactor <= 1.0f && loadFactor == loadFactor){
                                for(final var initCapacity:GENERAL_PURPOSE_INITIAL_CAPACITIES){
                                    TestExecutorService.submitTest(
                                            ()->runTest(loadFactor,initCapacity,collectionType,checkedType,initSeq));
                                }
                            }
                        }
                    }
