...
public class ArrSeqTest{
  private static abstract class AbstractArrSeqMonitor<SEQ extends AbstractSeq<?>&Externalizable>
    ...
    @Override abstract void updateModCount();
      ...
      for(var initParams:ALL_STRUCT_INIT_PARAMS) {
        for(var illegalMod:initParams.structType.validPreMods) {
          if(initParams.checkedType.checked || illegalMod.expectedException==null) {
            for(int size:SIZES) {
              for(int initCap:INIT_CAPACITIES) {
                TestExecutorService.submitTest(()->{
                  var monitor=SequenceInitialization.Ascending.initialize(getMonitoredSequence(initParams,initCap),size,0);
  ...
  private static interface MonitoredFunctionTest<MONITOR extends MonitoredSequence<?>>{
    @SuppressWarnings("unchecked") private void runAllTests(String testName,long maxRand,
        EnumSet<FunctionCallType> functionCallTypes,boolean useAllStructs){
      for(var initParams:useAllStructs?ALL_STRUCT_INIT_PARAMS:LIST_STRUCT_INIT_PARAMS) {
        for(var illegalMod:initParams.structType.validPreMods) {
          if(initParams.checkedType.checked || illegalMod.expectedException==null) {
            for(var functionGen:initParams.structType.validMonitoredFunctionGens) {
              if(initParams.checkedType.checked || functionGen.expectedException==null) {
                for(var size:SIZES) {
                  ...
                  for(var functionCallType:functionCallTypes) {
                    if(!functionCallType.boxed || initParams.collectionType!=DataType.REF) {
                      long randSeedBound=size>1&&functionGen.randomized && !functionCallType.boxed?maxRand:0;
                      for(long tmpRandSeed=0;tmpRandSeed<=randSeedBound;++tmpRandSeed) {
                        ...
                        for(int tmpInitVal=0;tmpInitVal<=initValBound;++tmpInitVal) {
                          ...
                          for(var initCap:INIT_CAPACITIES) {
                            TestExecutorService.submitTest(()->runTest((MONITOR)SequenceInitialization.Ascending.initialize(getMonitoredSequence(initParams,initCap),size,initVal),functionGen,functionCallType,illegalMod,randSeed));
                          }
                        }
                      }
                    }
  ...
  private static interface QueryTest<MONITOR extends MonitoredSequence<?>>{
                  ...
                  castTypesToInputTypes.forEach((castType,inputTypes)->{
                    inputTypes.forEach(inputType->{
                      if(queryVal==QueryVal.NonNull) {
                        for(var monitoredObjectGen:initParams.structType.validMonitoredObjectGens) {
                          if(monitoredObjectGen.expectedException!=null && initParams.checkedType.checked) {
                            for(var size:SIZES) {
                              if(size>0) {
                                Class<? extends Throwable> expectedException=illegalMod.expectedException==null?monitoredObjectGen.expectedException:illegalMod.expectedException;
                                TestExecutorService.submitTest(()->Assertions.assertThrows(expectedException,()->runTest(initParams,illegalMod,queryVal,modification,castType,inputType,monitoredObjectGen,size,-1)));
                              }
                            }
                          }
                        }
                      }else {
                        final boolean queryCanReturnTrue=queryVal.queryCanReturnTrue(modification,castType,inputType,initParams.collectionType);
                        for(var size:SIZES) {
                          for(var position:POSITIONS) {
                            ...
                            TestExecutorService.submitTest(()->{
                              if(cmeFilter(illegalMod,inputType,initParams.collectionType,queryVal,modification,castType)){
                                runTest(initParams,illegalMod,queryVal,modification,castType,inputType,null,size,position);
                              }else{
                                Assertions.assertThrows(illegalMod.expectedException,()->runTest(initParams,illegalMod,queryVal,modification,castType,inputType,null,size,position));
                              }
                            });
                          }
                        }
    ...
    @SuppressWarnings("unchecked") private void runTest(SequenceInitParams initParams,IllegalModification illegalMod,QueryVal queryVal,QueryVal.QueryValModification modification,QueryCastType castType,DataType inputType,MonitoredObjectGen monitoredObjectGen,int seqSize,double position){
  ...
  private static interface ToStringAndHashCodeTest{
    private void runAllTests(String testName){
      for(var initParams:ALL_STRUCT_INIT_PARAMS) {
        int initValBound=initParams.collectionType==DataType.BOOLEAN?1:0;
        for(var illegalMod:initParams.structType.validPreMods) {
          if(illegalMod.expectedException==null||initParams.checkedType.checked) {
            for(int size:SIZES) {
                  ...
                  if(objGen.expectedException==null||initParams.checkedType.checked) {
                    TestExecutorService.submitTest(()->{
                      if(size==0||objGen.expectedException==null) {
                        var monitor=SequenceInitialization.Ascending.initialize(getMonitoredSequence(initParams,size),size,0);
                        if(illegalMod.expectedException==null) {
                          callVerify(monitor);
                        }else {
                          monitor.illegalMod(illegalMod);
                        ...
                        final var monitor=SequenceInitialization.Ascending.initializeWithMonitoredObj(
                            getMonitoredSequence(initParams,size),
                            size,0,objGen,throwSwitch);
                        monitor.illegalMod(illegalMod);
                        final Class<? extends Throwable> expectedException
                            =illegalMod.expectedException == null?objGen.expectedException
  ...
  private static final EnumSet<StructType> ALL_STRUCTS
      =EnumSet.of(StructType.ArrList,StructType.ArrStack,StructType.ArrSubList);
...
//  }
  ...
  private static MonitoredList<?> getMonitoredList(SequenceInitParams initParams,int initCapacity){
    var rootMonitor=new ArrListMonitor<>(initParams,getInitCapacity(initCapacity,initParams.preAllocs,initParams.postAllocs));
    if(initParams.structType!=StructType.ArrSubList) {
      return rootMonitor;
    }
    var preAllocs=initParams.preAllocs;
    var postAllocs=initParams.postAllocs;
    ...
    for(int i=1;i<preAllocs.length;++i) {
      ...
      SequenceInitialization.Ascending.initialize(subListMonitor,preAlloc=preAllocs[i],Integer.MIN_VALUE+totalPreAlloc);
  ...
  static {
    ...
    Stream.Builder<SequenceInitParams> listStructBuilder=Stream.builder();
    Stream.Builder<SequenceInitParams> stackStructBuilder=Stream.builder();
    Stream.Builder<SequenceInitParams> rootStructBuilder=Stream.builder();
    for(var checkedType:CheckedType.values()) {
      for(var collectionType:DataType.values()) {
        var arrListParams=new SequenceInitParams(StructType.ArrList,collectionType,checkedType,OmniArray.OfInt.DEFAULT_ARR,OmniArray.OfInt.DEFAULT_ARR);
        var arrStackParams=new SequenceInitParams(StructType.ArrStack,collectionType,checkedType,OmniArray.OfInt.DEFAULT_ARR,OmniArray.OfInt.DEFAULT_ARR);
    ...
    for(int pre0=0;pre0<=5;pre0+=5) {
      for(int pre1=0;pre1<=5;pre1+=5) {
        var preAllocs=new int[] {pre0,pre1};
        for(int post0=0;post0<=5;post0+=5) {
          for(int post1=0;post1<=5;post1+=5) {
            var postAllocs=new int[] {post0,post1};
            for(var checkedType:CheckedType.values()) {
              for(var collectionType:DataType.values()) {
                var subListParams=new SequenceInitParams(StructType.ArrSubList,collectionType,checkedType,preAllocs,postAllocs);
  ...
  @Test public void testadd_intval(){
    for(var initParams:LIST_STRUCT_INIT_PARAMS) {
      for(var illegalMod:initParams.structType.validPreMods) {
        if(illegalMod.expectedException==null||initParams.checkedType.checked) {
                ...
                for(var initCap:INIT_CAPACITIES) {
                  for(var position:POSITIONS) {
                    if(position>=0 || initParams.checkedType.checked) {
                      TestExecutorService.submitTest(()->{
                        if(illegalMod.expectedException==null) {
                          var monitor=getMonitoredList(initParams,initCap);
                          if(position<0) {
                            for(int i=0;i<100;++i) {
                              final Object inputVal=inputType.convertValUnchecked(i);
                              final int finalI=i;
                              Assertions.assertThrows(IndexOutOfBoundsException.class,()->monitor.verifyAdd(-1,inputVal,inputType,functionCallType));
                              Assertions.assertThrows(IndexOutOfBoundsException.class,()->monitor.verifyAdd(finalI + 1,inputVal,inputType,functionCallType));
                              monitor.add(i);
                            }
                          }else {
                            for(int i=0;i<100;++i) {
                              monitor.verifyAdd((int)(i * position),inputType.convertValUnchecked(i),inputType,functionCallType);
                            }
                          }
                        }else {
                          {
                            final var monitor=SequenceInitialization.Ascending.initialize(getMonitoredList(initParams,initCap),10,0);
                            monitor.illegalMod(illegalMod);
                            final Object inputVal=inputType.convertValUnchecked(0);
                            Assertions.assertThrows(illegalMod.expectedException,()->monitor.verifyAdd(-1,inputVal,inputType,functionCallType));
                            Assertions.assertThrows(illegalMod.expectedException,()->monitor.verifyAdd(1,inputVal,inputType,functionCallType));
                            Assertions.assertThrows(illegalMod.expectedException,()->monitor.verifyAdd(0,inputVal,inputType,functionCallType));
                          }
                          {
                  ...
                  TestExecutorService.submitTest(()->{
                    if(illegalMod.expectedException == null){
                    ...
                    }else{
                      {
                        ...
                        monitor.illegalMod(illegalMod);
                        Assertions.assertThrows(illegalMod.expectedException,()->monitor.verifyAdd(inputType.convertValUnchecked(0),inputType,functionCallType));
                      }
                      {
                        final var monitor=SequenceInitialization.Ascending.initialize(getMonitoredSequence(initParams,initCap),10,0);
                        monitor.illegalMod(illegalMod);
                        Assertions.assertThrows(illegalMod.expectedException,()->monitor.verifyAdd(inputType.convertValUnchecked(0),inputType,functionCallType));
                      }
                    }
  ...
  @Test public void testConstructor_int(){
    for(var initParams:ROOT_STRUCT_INIT_PARAMS) {
      for(var initCap:INIT_CAPACITIES) {
        TestExecutorService.submitTest(()->{
      ...
      for(var illegalMod:initParams.structType.validPreMods) {
        if(illegalMod.expectedException==null||initParams.checkedType.checked) {
          for(var size:SHORT_SIZES) {
            TestExecutorService.submitTest(()->{
              var monitor=SequenceInitialization.Ascending.initialize(getMonitoredList(initParams,size),size,0);
              monitor.illegalMod(illegalMod);
              if(illegalMod.expectedException==null) {
  ...
  @Test public void testindexOf_val(){
    final QueryTest<MonitoredList<?>> test
        =(monitor,queryVal,inputType,castType,modification,monitoredObjectGen,position,seqSize)->{
          if(monitoredObjectGen == null){
            int expectedIndex;
            if(position >= 0){
  ...
  @Test public void testItrclone_void(){
    for(var initParams:ALL_STRUCT_INIT_PARAMS) {
      for(var itrType:initParams.structType.validItrTypes) {
        for(var size:SIZES) {
          int prevIndex=-1;
    ...
    for(var initParams:ALL_STRUCT_INIT_PARAMS) {
      for(var itrType:initParams.structType.validItrTypes) {
        for(var illegalMod:itrType.validPreMods) {
          if(illegalMod.expectedException==null||initParams.checkedType.checked) {
            for(var outputType:initParams.collectionType.validOutputTypes()) {
              for(var size:SHORT_SIZES) {
  ...
  @Test public void testListItradd_val(){
    for(var initParams:LIST_STRUCT_INIT_PARAMS) {
      var itrType=initParams.structType==StructType.ArrList?IteratorType.BidirectionalItr:IteratorType.SubBidirectionalItr;
      for(var illegalMod:itrType.validPreMods) {
        if(illegalMod.expectedException==null||initParams.checkedType.checked) {
          for(var inputType:initParams.collectionType.mayBeAddedTo()) {
            for(var functionCallType:FunctionCallType.values()) {
                    ...
                    if(position>=0) {
                      TestExecutorService.submitTest(()->{
                        ...
                        if(illegalMod.expectedException != null){
                          itrMonitor.illegalMod(illegalMod);
                          Assertions.assertThrows(illegalMod.expectedException,()->itrMonitor
                              .verifyAdd(inputType.convertValUnchecked(100),inputType,functionCallType));
  ...
  @Test public void testListItrprevious_void(){
    for(var initParams:LIST_STRUCT_INIT_PARAMS) {
      final var itrType=initParams.structType == StructType.ArrList?IteratorType.BidirectionalItr:IteratorType.SubBidirectionalItr;
      for(var illegalMod:itrType.validPreMods) {
        if(illegalMod.expectedException==null||initParams.checkedType.checked) {
          for(var outputType:initParams.collectionType.validOutputTypes()) {
            for(var size:SHORT_SIZES) {
  ...
  @Test public void testpeek_void(){
    for(var initParams: STACK_STRUCT_INIT_PARAMS) {
      TestExecutorService.submitTest(()->{
        var monitor=new ArrStackMonitor<>(initParams,100);
        for(int i=0;;++i){
          for(final var outputType:initParams.collectionType.validOutputTypes()){
  ...
  @Test public void testpoll_void() {
    for(var initParams: STACK_STRUCT_INIT_PARAMS) {
      for(var outputType:initParams.collectionType.validOutputTypes()) {
        TestExecutorService.submitTest(()->{
          final var monitor=SequenceInitialization.Ascending
              .initialize(new ArrStackMonitor<>(initParams,100),100,0);
          for(int i=0;;++i){
          ...
          for(final var filterGen:initParams.structType.validMonitoredRemoveIfPredicateGens){
            ...
            for(int tmpSize=0;tmpSize <= sizeBound;tmpSize+=sizeInc){
              ...
              if((size=tmpSize) == 0 || initParams.checkedType.checked || filterGen.expectedException == null){
                ...
                for(final var functionCallType:FunctionCallType.values()){
                  if(initParams.collectionType != DataType.REF || !functionCallType.boxed){
                    long randSeedBound;
                    double[] thresholdArr;
                      ...
                      for(int tmpInitVal=0;tmpInitVal <= initValBound;++tmpInitVal){
                        ...
                        for(int tmpPeriod=0;tmpPeriod <= periodBound;++tmpPeriod){
                          ...
                          for(final var threshold:thresholdArr){
                            TestExecutorService.submitTest(()->{
                              final var monitor=SequenceInitialization.Ascending
                                  .initialize(getMonitoredSequence(initParams,size),size,
                                      initVal,period);
                              final var filter
  ...
  @Test public void testremoveVal_val(){
    final QueryTest<MonitoredSequence<?>> test=new QueryTest<>(){
      @Override public void callAndVerifyResult(MonitoredSequence<?> monitor,QueryVal queryVal,DataType inputType,
  ...
  @Tag("Search") @Test public void testsearch_val(){
