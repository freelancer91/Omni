...
@TestMethodOrder(OrderAnnotation.class) @Tag("NewTest") public class ArrDeqTest{
  ...
  private static class ArrDeqMonitor implements MonitoredDeque<OmniDeque<?>>{
    ...
    private class AscendingItrMonitor extends AbstractItrMonitor{
      ...
      @Override void fragmentedRemove(){
      ...
      @Override void nonfragmentedRemove(){
        int head=expectedHead;
        final int tail=expectedTail;
        final int lastRet=expectedLastRet;
    ...
    private void rotate(int numToRotate){
      expectedModCount+=2 * numToRotate;
      switch(dataType){
      case BOOLEAN:{
        final var castSeq=(BooleanArrDeq)seq;
        for(int i=0;i < numToRotate;++i){
          final var tmp=castSeq.booleanElement();
          castSeq.addLast(tmp);
          castSeq.popBoolean();
        }
        expectedHead=castSeq.head;
        expectedTail=castSeq.tail;
      ...
      case BYTE:{
        final var castSeq=(ByteArrDeq)seq;
        for(int i=0;i < numToRotate;++i){
          final var tmp=castSeq.byteElement();
          castSeq.addLast(tmp);
          castSeq.popByte();
        }
        expectedHead=castSeq.head;
      ...
      case CHAR:{
        final var castSeq=(CharArrDeq)seq;
        for(int i=0;i < numToRotate;++i){
          final var tmp=castSeq.charElement();
          castSeq.addLast(tmp);
          castSeq.popChar();
        }
        expectedHead=castSeq.head;
      ...
      case SHORT:{
        final var castSeq=(ShortArrDeq)seq;
        for(int i=0;i < numToRotate;++i){
          final var tmp=castSeq.shortElement();
          castSeq.addLast(tmp);
          castSeq.popShort();
        }
        expectedHead=castSeq.head;
      ...
      case INT:{
        final var castSeq=(IntArrDeq)seq;
        for(int i=0;i < numToRotate;++i){
          final var tmp=castSeq.intElement();
          castSeq.addLast(tmp);
          castSeq.popInt();
        }
        expectedHead=castSeq.head;
      ...
      case LONG:{
        final var castSeq=(LongArrDeq)seq;
        for(int i=0;i < numToRotate;++i){
          final var tmp=castSeq.longElement();
          castSeq.addLast(tmp);
          castSeq.popLong();
        }
        expectedHead=castSeq.head;
      ...
      case FLOAT:{
        final var castSeq=(FloatArrDeq)seq;
        for(int i=0;i < numToRotate;++i){
          final var tmp=castSeq.floatElement();
          castSeq.addLast(tmp);
          castSeq.popFloat();
        }
        expectedHead=castSeq.head;
      ...
      case DOUBLE:{
        final var castSeq=(DoubleArrDeq)seq;
        for(int i=0;i < numToRotate;++i){
          final var tmp=castSeq.doubleElement();
          castSeq.addLast(tmp);
          castSeq.popDouble();
        }
        expectedHead=castSeq.head;
      ...
      case REF:{
        @SuppressWarnings("unchecked") final var castSeq=(RefArrDeq<Object>)seq;
        for(int i=0;i < numToRotate;++i){
          final var tmp=castSeq.element();
          castSeq.addLast(tmp);
          castSeq.pop();
        }
        expectedHead=castSeq.head;
                ...
                for(final var inputType:inputTypes){
                  ...
                  for(final var checkedType:CheckedType.values()){
                    for(final var size:SIZES){
                      final int interval=Math.max(1,size / 10);
                      final int rotateBound=size / 2 + interval;
                      for(int tmpInitCap=0,initCapBound=size + interval;tmpInitCap <= initCapBound;
                          tmpInitCap+=interval){
                        final int initCap=tmpInitCap;
                        for(int tmpNumToRotate=0;tmpNumToRotate <= rotateBound;tmpNumToRotate+=interval){
                          final int numToRotate=tmpNumToRotate;
                          if(queryVal == QueryVal.NonNull){
                            for(final var monitoredObjectGen:StructType.ArrDeq.validMonitoredObjectGens){
                              if(size != 0 && monitoredObjectGen.expectedException == null && checkedType.checked){
                                TestExecutorService
                                    .submitTest(()->Assertions.assertThrows(monitoredObjectGen.expectedException,
                            ...
                            for(final var position:POSITIONS){
                              ...
                              TestExecutorService
                                  .submitTest(()->runTest(new ArrDeqMonitor(checkedType,collectionType,initCap),
                                      queryVal,modification,castType,inputType,size,null,position,numToRotate));
                            }
                          }
                        }
                      }
            ...
            for(var collectionType:DataType.values()) {
              ...
              for(var checkedType:CheckedType.values()) {
                if(collectionType==DataType.REF) {
                  for(var objGen:StructType.ArrDeq.validMonitoredObjectGens) {
                    TestExecutorService.submitTest(()->{
                      if(size==0 || objGen.expectedException==null) {
                        final var monitor=SequenceInitialization.Ascending
                            .initialize(new ArrDeqMonitor(checkedType,collectionType,initCap),size,
                                    0);
                        if(size>0 && numToRotate>0) {
                          monitor.rotate(numToRotate);
                        }
                        ...
                        final var monitor=SequenceInitialization.Ascending
                                .initializeWithMonitoredObj(
                                    new ArrDeqMonitor(checkedType,collectionType,initCap),size,0,
                                        objGen,throwSwitch);
                        if(size>0 && numToRotate>0) {
                          monitor.rotate(numToRotate);
                        }
