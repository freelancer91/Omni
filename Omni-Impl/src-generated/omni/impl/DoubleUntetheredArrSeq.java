...
abstract class DoubleUntetheredArrSeq implements OmniCollection.OfDouble,Externalizable
{
  double[] arr;
  int head;
  int tail;
  ...
  boolean uncheckedAdd(int tail,double key,DoubleComparator sorter)
  {
    ...
    {
      switch(sorter.compare(key,(double)arr[0]))
      {
        case 0:
          return false;
        case -1:
    ...
    {
      final int mid;
      switch(sorter.compare(key,(double)arr[mid=(head+tail)>>>1]))
      {
        case 0:
          return false;
        case -1:
  ...
  boolean fragmentedInsertHi(double[] arr,int head,int tail,double key,DoubleComparator sorter){
    int lo=1;
    int hi=tail;
    while(lo<=hi)
    {
      final int mid;
      switch(sorter.compare(key,(double)arr[ mid=(hi+lo)>>>1]))
      {
        case 0:
          return false;
        case -1:
    ...
    if(++tail==head)
    {
      ...
      (tmp=new double[hi=OmniArray.growBy50Pct(arrLength)])[lo]=key;
      if(lo==0)
      {
  ...
  boolean fragmentedInsertLo(double[] arr,int head,int tail,double key,DoubleComparator sorter){
    int arrBound;
    ...
    if(++tail==head)
    {
      ...
      ArrCopy.uncheckedCopy(arr,0,tmp=new double[hi=OmniArray.growBy50Pct(arrBound)],0,tail);
  ...
  boolean nonfragmentedInsertLo(double[] arr,int head,int hi,double key,DoubleComparator sorter){
    int lo=head;
    while(lo<=hi)
    {
      final int mid;
      switch(sorter.compare(key,(double)arr[mid=(lo+hi)>>>1]))
      {
        case 0:
          return false;
        case -1:
    ...
    if((headDist=lo-head)==0){
      ...
      if(--head==-1 && this.tail==(head=arr.length-1)){
        //must grow
        ArrCopy.uncheckedCopy(arr,0,arr=new double[headDist=OmniArray.growBy50Pct(++head)],lo=headDist-head,head);
      ...
      if(head==0){
        if(this.tail==(head=arr.length-1)){
          //must grow
          final var tmp=new double[headDist=OmniArray.growBy50Pct(++head)];
          this.tail=headDist-1;
  ...
  boolean nonfragmentedInsertHi(double[] arr,int lo,int tail,double key,DoubleComparator sorter){
    int hi=tail;
    while(lo<=hi)
    {
      final int mid;
    ...
    if((tailDist=(++tail-lo))==0){
      ...
      if(tail==arr.length && head==0){
        //must grow
        ArrCopy.uncheckedCopy(arr,0,arr=new double[head=OmniArray.growBy50Pct(tail)],0,tail);
        this.tail=tail;
        arr[lo]=key;
        this.arr=arr;
      }else{
      ...
      if(tail==arr.length){
        if(this.head==0){
          //must grow
          final double[] tmp;
          ArrCopy.uncheckedCopy(arr,0,tmp=new double[hi=OmniArray.growBy50Pct(tail)],0,lo);
          tmp[lo]=key;
  ...
  boolean uncheckedAdd(int tail,double key,DoubleToIntFunction sorter)
  {
    final var arr=this.arr;
    final int head;
    if(tail<(head=this.head))
    {
      switch(sorter.applyAsInt((double)arr[0]))
      {
        case 0:
          return false;
        case -1:
    ...
    {
      final int mid;
      switch(sorter.applyAsInt((double)arr[mid=(head+tail)>>>1]))
      {
        case 0:
          return false;
        case -1:
  ...
  boolean fragmentedInsertHi(double[] arr,int head,int tail,double key,DoubleToIntFunction sorter){
    int lo=1;
    int hi=tail;
    while(lo<=hi)
    {
      final int mid;
      switch(sorter.applyAsInt((double)arr[ mid=(hi+lo)>>>1]))
      {
        case 0:
          return false;
        case -1:
  ...
  boolean fragmentedInsertLo(double[] arr,int head,int tail,double key,DoubleToIntFunction sorter){
    int arrBound;
    int hi=(arrBound=arr.length)-1;
    int lo=head;
    do
  ...
  boolean nonfragmentedInsertLo(double[] arr,int head,int hi,double key,DoubleToIntFunction sorter){
    int lo=head;
    while(lo<=hi)
    {
      final int mid;
      switch(sorter.applyAsInt((double)arr[mid=(lo+hi)>>>1]))
      {
        case 0:
          return false;
        case -1:
  ...
  boolean nonfragmentedInsertHi(double[] arr,int lo,int tail,double key,DoubleToIntFunction sorter){
    int hi=tail;
    while(lo<=hi)
    {
      final int mid;
  ...
  boolean uncheckedRemoveMatch(int tail,final DoubleToIntFunction comparator)
  {
    final int head;
    if((head=this.head)<=tail){
  ...
  static int findIndex(final double[] arr,int head,int tail,final DoubleToIntFunction comparator)
  {
    do
    {
      final int mid;
    ...
    if(filter.test((double)(arr=this.arr)[head])){
      for(int bound=arr.length-1;;){
        ...
        if(!filter.test((double)arr[++head])){
          ...
          for(head=0;!filter.test((double)arr[head]);++head){
            if(head==tail){
              return true;
            }
          }
  ...
  static abstract class AbstractUntetheredArrSeqItr extends AbstractDoubleItr{
    transient final DoubleUntetheredArrSeq root;
    transient int index;
    transient int numLeft;
    AbstractUntetheredArrSeqItr(DoubleUntetheredArrSeq root,int index,int numLeft){
      this.root=root;
      this.index=index;
    ...
    abstract void fragmentedRemove(final DoubleUntetheredArrSeq root,int head,int tail);
    abstract void nonfragmentedRemove(final DoubleUntetheredArrSeq root,int head,int tail);
  ...
  static class AscendingUntetheredArrSeqItr extends AbstractUntetheredArrSeqItr{
    AscendingUntetheredArrSeqItr(DoubleUntetheredArrSeq root,int index,int numLeft){
    ...
    @Override void nonfragmentedRemove(final DoubleUntetheredArrSeq root,final int head,int tail){
    ...
    @Override void fragmentedRemove(final DoubleUntetheredArrSeq root,final int head,int tail){
      final var arr=root.arr;
  ...
  static class DescendingUntetheredArrSeqItr extends AbstractUntetheredArrSeqItr{
    DescendingUntetheredArrSeqItr(DoubleUntetheredArrSeq root,int index,int numLeft){
      super(root,index,numLeft);
    }
    @Override public Object clone(){
      return new AscendingUntetheredArrSeqItr(root,index,numLeft);
