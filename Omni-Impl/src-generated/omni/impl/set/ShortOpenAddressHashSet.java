...
implements OmniSet.OfShort{
  ...
  public ShortOpenAddressHashSet(ShortOpenAddressHashSet that){
    ...
    if((tableSize=that.tableSize)!=0){
      short[] thisTable;
      int thisTableLength;
      this.table=thisTable=new short[thisTableLength=tableSizeFor(tableSize)];
    ...
    if(val instanceof Set){
      ...
      if(size==(that=(Set<?>)val).size()){
        for(final var thatVal:that){
          if(!(thatVal instanceof Short) || !this.contains((short)thatVal)){
            return false;
          }
        }
        return true;
  ...
  /*
  private static boolean isEqualToHelper(short[] smallTable,short[] bigTable,int bigTableLength,int numInTable){
  ...
  private boolean isEqualTo(int size,ShortOpenAddressHashSet that){
    if(size==that.size){
  ...
  public void readExternal(ObjectInput in) throws IOException
  {
      ...
      if(size != 0){
          ...
          if(size != 0){
              ...
              do{
                  final short val;
                  SetCommonImpl.quickInsert(val=in.readShort(),table,tableSize,tableSize&val);
              }while(--size != 0);
          }else{
              maxTableSize=1;
          }
