...
implements OmniSet.OfInt{
  ...
  public IntOpenAddressHashSet(IntOpenAddressHashSet that){
    ...
    if((tableSize=that.tableSize)!=0){
      int[] thisTable;
      int thisTableLength;
      this.table=thisTable=new int[thisTableLength=tableSizeFor(tableSize)];
    ...
    if(val instanceof Set){
      ...
      if(size==(that=(Set<?>)val).size()){
        for(final var thatVal:that){
          if(!(thatVal instanceof Integer) || !this.contains((int)thatVal)){
            return false;
          }
        }
        return true;
  ...
  private boolean isEqualTo(int size,IntOpenAddressHashSet that){
    if(size==that.size){
  ...
  public void readExternal(ObjectInput in) throws IOException
  {
      ...
      if(size != 0){
          ...
          if(size != 0){
              ...
              do{
                  final int val;
                  SetCommonImpl.quickInsert(val=in.readInt(),table,tableSize,tableSize&val);
              }while(--size != 0);
          }else{
              maxTableSize=1;
          }
