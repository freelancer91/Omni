...
implements OmniSet.OfDouble{
  ...
  public DoubleOpenAddressHashSet(DoubleOpenAddressHashSet that){
    ...
    if((size=that.size)!=0){
      long[] thisTable;
      int thisTableLength;
      this.table=thisTable=new long[thisTableLength=tableSizeFor(size)];
      this.maxTableSize=(int)(thisTableLength*loadFactor);
      final long[] thatTable;
      --thisTableLength;
      for(int thatTableLength=(thatTable=that.table).length;;){
        long tableVal;
        if((tableVal=thatTable[--thatTableLength]) == 0 || tableVal == 0x7ffc000000000000L){
          continue;
        }
    ...
    if(val instanceof Set){
      ...
      if(size==(that=(Set<?>)val).size()){
        for(final var thatVal:that){
          if(!(thatVal instanceof Double) || !this.contains((double)thatVal)){
            return false;
          }
        }
        return true;
  ...
  private boolean isEqualTo(int size,DoubleOpenAddressHashSet that){
    if(size==that.size){
