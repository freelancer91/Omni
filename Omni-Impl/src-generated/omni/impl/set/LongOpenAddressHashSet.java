...
implements OmniSet.OfLong{
  ...
  public LongOpenAddressHashSet(LongOpenAddressHashSet that){
    ...
    if((tableSize=that.tableSize)!=0){
      long[] thisTable;
      int thisTableLength;
      this.table=thisTable=new long[thisTableLength=tableSizeFor(tableSize)];
    ...
    if(val instanceof Set){
      ...
      if(size==(that=(Set<?>)val).size()){
        for(final var thatVal:that){
          if(!(thatVal instanceof Long) || !this.contains((long)thatVal)){
            return false;
          }
        }
        return true;
  ...
  /*
  private static boolean isEqualToHelper(long[] smallTable,long[] bigTable,int bigTableLength,int numInTable){
  ...
  private boolean isEqualTo(int size,LongOpenAddressHashSet that){
    if(size==that.size){
  ...
  public void readExternal(ObjectInput in) throws IOException
  {
      ...
      if(size != 0){
          ...
          if(size != 0){
              ...
              do{
                  final long val;
                  SetCommonImpl.quickInsert(val=in.readLong(),table,tableSize,tableSize&Long.hashCode(val));
              }while(--size != 0);
          }else{
              maxTableSize=1;
          }
