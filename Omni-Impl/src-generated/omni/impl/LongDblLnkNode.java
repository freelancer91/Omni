...
public class LongDblLnkNode implements Comparable<LongDblLnkNode>
{
  ...
  public static  void uncheckedForEachAscending(LongDblLnkNode node,int length,LongConsumer action){
    for(;;node=node.next){
  ...
  public static  void uncheckedForEachDescending(LongDblLnkNode node,int length,LongConsumer action){
    for(;;node=node.prev){
      action.accept(node.val);
      if(--length==0){
        return;
  ...
  public static  void uncheckedReplaceAll(LongDblLnkNode node,int length,LongUnaryOperator operator){
    for(;;node=node.next){
  ...
  public static  int uncheckedsearch (LongDblLnkNode head,LongDblLnkNode tail
  ,long val
  ){
    int index=1;
