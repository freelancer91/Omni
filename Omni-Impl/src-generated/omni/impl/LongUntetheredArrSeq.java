...
abstract class LongUntetheredArrSeq implements OmniCollection.OfLong,Externalizable
{
  long[] arr;
  int head;
  int tail;
  LongUntetheredArrSeq(int head,long[] arr,int tail){
    super();
    this.arr=arr;
    this.head=head;
    this.tail=tail;
  }
  LongUntetheredArrSeq(){
    super();
    this.tail=-1;
  }
...
Long[] uncheckedAscendingToArray(int head,int tail){
  final Long[] dst;
  int size;
  if((size=(++tail)-head)>0){
...
Long[] uncheckedDescendingToArray(int head,int tail){
  final Long[] dst;
  int size;
  if((size=(++tail)-head)>0){
...
float[] uncheckedDescendingToFloatArray(int head,int tail){
  ...
  void insertAtTail(long[] arr,long key,int head,int tail){
    switch(Integer.signum((++tail)-head)){
      case 0:
        //fragmented must grow
        final long[] tmp;
        int arrLength;
        ArrCopy.uncheckedCopy(arr,0,tmp=new long[head=OmniArray.growBy50Pct(arrLength=arr.length)],0,tail);
  ...
  void insertAtHead(long[] arr,long key,int head,int tail){
    int newHead;
    switch(Integer.signum(tail-(newHead=head-1))){
      case 0:
        ...
        final long[] tmp;
        int arrLength;
        ArrCopy.uncheckedCopy(arr,0,tmp=new long[tail=OmniArray.growBy50Pct(arrLength=arr.length)],0,head);
      ...
      default:
        ...
        if(newHead==-1 && tail==(newHead=arr.length-1)){
          ...
          ArrCopy.uncheckedCopy(arr,0,arr=new long[newHead],newHead-=(tail),tail);
          --newHead;
          this.arr=arr;
        }
      case -1:
  ...
  void insertAtMiddle(long key){
    long[] arr;
    if((arr=this.arr)==null){
      this.arr=new long[]{key};
      this.head=0;
      this.tail=0;
    }else if(arr==OmniArray.OfLong.DEFAULT_ARR){
      this.arr=arr=new long[OmniArray.DEFAULT_ARR_SEQ_CAP];
  ...
  */
  boolean uncheckedAdd(int tail,long key,LongComparator sorter)
  {
    ...
    {
      switch(sorter.compare(key,(long)arr[0]))
      {
        case 0:
          return false;
        case -1:
    ...
    {
      final int mid;
      switch(sorter.compare(key,(long)arr[mid=(head+tail)>>>1]))
      {
        case 0:
          return false;
        case -1:
  ...
  boolean fragmentedInsertHi(long[] arr,int head,int tail,long key,LongComparator sorter){
    int lo=1;
    int hi=tail;
    while(lo<=hi)
    {
      final int mid;
      switch(sorter.compare(key,(long)arr[ mid=(hi+lo)>>>1]))
      {
        case 0:
          return false;
        case -1:
    ...
    if(++tail==head)
    {
      ...
      (tmp=new long[hi=OmniArray.growBy50Pct(arrLength)])[lo]=key;
      if(lo==0)
      {
  ...
  boolean fragmentedInsertLo(long[] arr,int head,int tail,long key,LongComparator sorter){
    int arrBound;
    ...
    if(++tail==head)
    {
      ...
      ArrCopy.uncheckedCopy(arr,0,tmp=new long[hi=OmniArray.growBy50Pct(arrBound)],0,tail);
  ...
  boolean nonfragmentedInsertLo(long[] arr,int head,int hi,long key,LongComparator sorter){
    int lo=head;
    while(lo<=hi)
    {
      final int mid;
      switch(sorter.compare(key,(long)arr[mid=(lo+hi)>>>1]))
      {
        case 0:
          return false;
        case -1:
    ...
    if((headDist=lo-head)==0){
      ...
      if(--head==-1 && this.tail==(head=arr.length-1)){
        //must grow
        ArrCopy.uncheckedCopy(arr,0,arr=new long[headDist=OmniArray.growBy50Pct(++head)],lo=headDist-head,head);
      ...
      if(head==0){
        if(this.tail==(head=arr.length-1)){
          //must grow
          final var tmp=new long[headDist=OmniArray.growBy50Pct(++head)];
          this.tail=headDist-1;
  ...
  boolean nonfragmentedInsertHi(long[] arr,int lo,int tail,long key,LongComparator sorter){
    int hi=tail;
    while(lo<=hi)
    {
      final int mid;
    ...
    if((tailDist=(++tail-lo))==0){
      ...
      if(tail==arr.length && head==0){
        //must grow
        ArrCopy.uncheckedCopy(arr,0,arr=new long[head=OmniArray.growBy50Pct(tail)],0,tail);
        this.tail=tail;
        arr[lo]=key;
        this.arr=arr;
      }else{
      ...
      if(tail==arr.length){
        if(this.head==0){
          //must grow
          final long[] tmp;
          ArrCopy.uncheckedCopy(arr,0,tmp=new long[hi=OmniArray.growBy50Pct(tail)],0,lo);
          tmp[lo]=key;
  ...
  boolean uncheckedAdd(int tail,long key,LongToIntFunction sorter)
  {
    final var arr=this.arr;
    final int head;
    if(tail<(head=this.head))
    {
      switch(sorter.applyAsInt((long)arr[0]))
      {
        case 0:
          return false;
        case -1:
    ...
    {
      final int mid;
      switch(sorter.applyAsInt((long)arr[mid=(head+tail)>>>1]))
      {
        case 0:
          return false;
        case -1:
  ...
  boolean fragmentedInsertHi(long[] arr,int head,int tail,long key,LongToIntFunction sorter){
    int lo=1;
    int hi=tail;
    while(lo<=hi)
    {
      final int mid;
      switch(sorter.applyAsInt((long)arr[ mid=(hi+lo)>>>1]))
      {
        case 0:
          return false;
        case -1:
  ...
  boolean fragmentedInsertLo(long[] arr,int head,int tail,long key,LongToIntFunction sorter){
    int arrBound;
    int hi=(arrBound=arr.length)-1;
    int lo=head;
    do
  ...
  boolean nonfragmentedInsertLo(long[] arr,int head,int hi,long key,LongToIntFunction sorter){
    int lo=head;
    while(lo<=hi)
    {
      final int mid;
      switch(sorter.applyAsInt((long)arr[mid=(lo+hi)>>>1]))
      {
        case 0:
          return false;
        case -1:
  ...
  boolean nonfragmentedInsertHi(long[] arr,int lo,int tail,long key,LongToIntFunction sorter){
    int hi=tail;
    while(lo<=hi)
    {
      final int mid;
  ...
  boolean uncheckedRemoveMatch(int tail,final LongToIntFunction comparator)
  {
    final int head;
    if((head=this.head)<=tail){
  ...
  static int findIndex(final long[] arr,int head,int tail,final LongToIntFunction comparator)
  {
    do
    {
      final int mid;
  ...
  static abstract class AbstractUntetheredArrSeqItr extends AbstractLongItr{
    transient final LongUntetheredArrSeq root;
    transient int index;
    transient int numLeft;
    AbstractUntetheredArrSeqItr(LongUntetheredArrSeq root,int index,int numLeft){
      this.root=root;
      this.index=index;
    ...
    abstract void fragmentedRemove(final LongUntetheredArrSeq root,int head,int tail);
    abstract void nonfragmentedRemove(final LongUntetheredArrSeq root,int head,int tail);
  ...
  static class AscendingUntetheredArrSeqItr extends AbstractUntetheredArrSeqItr{
    AscendingUntetheredArrSeqItr(LongUntetheredArrSeq root,int index,int numLeft){
    ...
    @Override void nonfragmentedRemove(final LongUntetheredArrSeq root,final int head,int tail){
    ...
    @Override void fragmentedRemove(final LongUntetheredArrSeq root,final int head,int tail){
      final var arr=root.arr;
  ...
  static class DescendingUntetheredArrSeqItr extends AbstractUntetheredArrSeqItr{
    DescendingUntetheredArrSeqItr(LongUntetheredArrSeq root,int index,int numLeft){
      super(root,index,numLeft);
    }
    @Override public Object clone(){
      return new AscendingUntetheredArrSeqItr(root,index,numLeft);
