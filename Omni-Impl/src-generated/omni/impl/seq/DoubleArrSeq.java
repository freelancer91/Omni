 ...
 implements OmniCollection.OfDouble,Externalizable,RandomAccess{
      ...
      implements DoubleListDefault,Cloneable,RandomAccess
  {
    ...
    boolean isEqualTo(ListIterator<?> itr,int thisOffset,int thisBound){
      Object val;
      if(itr.hasNext() && (val=itr.next()) instanceof Double){
        for(final var arr=this.arr;TypeUtil.doubleEquals((double)val,arr[thisOffset]);){
          if(!itr.hasNext()){
            return thisOffset+1==thisBound;
          }
          if(++thisOffset>=thisBound || !((val=itr.next()) instanceof Double)){
            break;
          }
        }
      }
    ...
    private boolean isEqualTo(int rootOffset,int size,OmniList.OfDouble list){
      if(list instanceof DoubleArrSeq.UncheckedList){
        final DoubleArrSeq.UncheckedList uncheckedList;
        return (uncheckedList=(DoubleArrSeq.UncheckedList)list).size==size && ((uncheckedList==this) || this.isEqualToHelper(rootOffset,rootOffset+size,uncheckedList.arr,0));
    ...
    private boolean isEqualTo(int size,OmniList.OfDouble list){
      if(list instanceof DoubleArrSeq.UncheckedList){
        ...
        return size==(that=(DoubleArrSeq.UncheckedList)list).size && this.isEqualToHelper(0,size,that.arr,0);
        ...
        final DoubleArrSeq.UncheckedSubList subList;
        final DoubleArrSeq.UncheckedList thatRoot;
        return (subList=(DoubleArrSeq.UncheckedSubList)list).size==size && ((thatRoot=subList.root)==this || this.isEqualToHelper(0,size,thatRoot.arr,subList.rootOffset));
      }else{
    ...
    @Override public OmniListIterator.OfDouble listIterator(int index){
      ...
      void uncheckedInsert(int index,int size,double val){
        final int tailDist;
      ...
      extends UncheckedList
  {
    ...
    private boolean isEqualTo(int rootOffset,int size,OmniList.OfDouble list){
      if(list instanceof DoubleArrSeq.UncheckedList){
        final DoubleArrSeq.UncheckedList that;
        return size==(that=(DoubleArrSeq.UncheckedList)list).size && ((that==this)||(super.isEqualToHelper(rootOffset,rootOffset+size,that.arr,0)));
      }else if(list instanceof DoubleArrSeq.CheckedSubList){
        final DoubleArrSeq.CheckedSubList subList;
        final DoubleArrSeq.CheckedList thatRoot;
        CheckedCollection.checkModCount((subList=(DoubleArrSeq.CheckedSubList)list).modCount,(thatRoot=subList.root).modCount);
        ...
        final DoubleArrSeq.UncheckedSubList subList;
        return size==(subList=(DoubleArrSeq.UncheckedSubList)list).size && super.isEqualToHelper(rootOffset,rootOffset+size,subList.root.arr,subList.rootOffset);
      }else{
        //must be DoubleDblLnkSeq
        final DoubleDblLnkSeq dls;
        if((dls=(DoubleDblLnkSeq)list) instanceof DoubleDblLnkSeq.CheckedSubList){
    ...
    private boolean isEqualTo(int size,OmniList.OfDouble list){
      if(list instanceof DoubleArrSeq.UncheckedList){
        final DoubleArrSeq.UncheckedList that;
        return size==(that=(DoubleArrSeq.UncheckedList)list).size && super.isEqualToHelper(0,size,that.arr,0);
      }else if(list instanceof DoubleDblLnkSeq){
        final DoubleDblLnkSeq dls;
        if((dls=(DoubleDblLnkSeq)list) instanceof DoubleDblLnkSeq.CheckedSubList){
          final DoubleDblLnkSeq.CheckedSubList subList;
        ...
        final DoubleArrSeq.UncheckedSubList subList;
        return size==(subList=(DoubleArrSeq.UncheckedSubList)list).size && super.isEqualToHelper(0,size,subList.root.arr,subList.rootOffset);
      }
    }
    @Override public boolean equals(Object val){
      if(val==this){
