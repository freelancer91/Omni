...
public abstract class ByteArrSeq implements OmniCollection.OfByte
{
  ...
  static void uncheckedForEachAscending(byte[] arr,int offset,int bound,ByteConsumer action)
  {
    do
    {
  ...
  static void uncheckedForEachDescending(byte[] arr,int offset,int bound,ByteConsumer action)
  {
    do
    {
  ...
  private static int doRemoveIf(byte[] arr,int srcOffset,int srcBound,BytePredicate filter)
  {
    do
    {
    ...
    private static  int pullSurvivorsDown(byte[] arr,int srcOffset,int srcBound,BytePredicate filter)
    {
    ...
    private static int doRemoveIf(byte[] arr,int srcOffset,int srcBound,BytePredicate filter,CheckedCollection.AbstractModCountChecker modCountChecker)
    {
      try
      {
        do
    ...
    private static int markAndPullDown(byte[] arr,int srcOffset,int srcBound,BytePredicate filter,int dstOffset,CheckedCollection.AbstractModCountChecker modCountChecker)
    {
      int nRemaining;
    ...
    implements OmniStack.OfByte
  {
      ...
      extends AbstractItr
    {
      ...
      {
            return (byte)parent.arr[this.cursor--];
      }
      @Override
    ...
    implements ListDefault
  {
    ...
    */
    private void uncheckedReplaceAll(int offset,int bound,ByteUnaryOperator operator)
    {
      final var arr=this.arr;
      for(;;)
      {
    ...
    void uncheckedToString(int size,ToStringUtil.OmniStringBuilder builder)
    {
     ...
     private static class LstItr
       extends Itr implements OmniListIterator.OfByte
     {
       transient int lastRet;
       ...
       {
         //TODO implement LstItr.add(byte)
       }
       @Override
       public void set(byte val)
       {
         //TODO implement LstItr.set(byte)
       }
     }
    @Override
    ...
    implements SubListDefault
  {
      ...
      extends AbstractItr
    {
      ...
      public byte nextByte()
      {
            return (byte)parent.root.arr[this.cursor++];
      }
      @Override
      public void forEachRemaining(ByteConsumer action)
      {
    ...
    contains(boolean val)
    {
      ...
      if((size=this.size)!=0)
      {
        return ((UncheckedList)root).sublistcontains(this.rootOffset,size,TypeUtil.castToByte(val));
      }
      return false;
    }
    @Override
    ...
    (Object val)
    {
      ...
      if((size=this.size)!=0)
      {
        if(val instanceof Byte)
        {
          return ((UncheckedList)root).sublistcontains(this.rootOffset,size,(byte)(val));
        }
      }
      return false;
    }
    ...
    public void clear()
    {
       ...
       extends Itr implements OmniListIterator.OfByte
     {
       ...
       public byte nextByte()
       {
             final int ret;
             this.lastRet=ret=this.cursor++;
             return (byte)parent.root.arr[ret];
       }
       @Override
       public void forEachRemaining(ByteConsumer action)
       {
    ...
    {
      final byte[] arr;
      final byte ret=(byte)(arr=root.arr)[index+=this.rootOffset];
      arr[index]=val;
      return ret;
    }
    @Override
    ...
    indexOf(boolean val)
    {
      ...
      if((size=this.size)!=0)
      {
        return ((UncheckedList)root).sublistindexOf(this.rootOffset,size,TypeUtil.castToByte(val));
      }
      return -1;
    }
    @Override
    ...
    (Object val)
    {
      ...
      if((size=this.size)!=0)
      {
        if(val instanceof Byte)
        {
          return ((UncheckedList)root).sublistindexOf(this.rootOffset,size,(byte)(val));
        }
      }
      return -1;
    }
    ...
    lastIndexOf(boolean val)
    {
      ...
      if((size=this.size)!=0)
      {
        return ((UncheckedList)root).sublistlastIndexOf(this.rootOffset,size,TypeUtil.castToByte(val));
      }
      return -1;
    }
    @Override
    ...
    (Object val)
    {
      ...
      if((size=this.size)!=0)
      {
        if(val instanceof Byte)
        {
          return ((UncheckedList)root).sublistlastIndexOf(this.rootOffset,size,(byte)(val));
        }
      }
      return -1;
    }
    ...
    implements SubListDefault
  {
    ...
    private ByteArrSeq checkModCountAndGetRoot()
    {
      final CheckedList root;
    ...
    public byte removeByteAt(int index)
    {
      ...
      for(var curr=parent;curr!=null;curr.modCount=modCount,--curr.size,curr=curr.parent){}
      this.size=size-1;
      return ((ByteArrSeq)root).uncheckedremoveAtIndex(index+this.rootOffset,root.size);
    }
    //TODO implement AND and OR in the template switches to avoid writing this twice
    @Override
    public
  ...
  private static abstract class AbstractItr implements OmniIterator.OfByte
  {
    @Override
    public Byte next()
    {
  ...
  private static abstract interface ListDefault extends OmniList.OfByte
  {
    @Override
  ...
  private static abstract interface SubListDefault extends ListDefault
  {
    ...
    {
      return this.add(TypeUtil.castToByte(val));
    }
  }
}
