...
AbstractOmniCollection<Byte>
 implements OmniCollection.OfByte,Externalizable,RandomAccess{
  ...
  private ByteArrSeq(Collection<? extends Byte> that){
    super();
    //TODO optimize
    this.addAll(that);
  }
  private ByteArrSeq(OmniCollection.OfRef<? extends Byte> that){
    super();
    //TODO optimize
    this.addAll(that);
  }
  private ByteArrSeq(OmniCollection.OfBoolean that){
    super();
    //TODO optimize
    this.addAll(that);
  }
  private ByteArrSeq(OmniCollection.ByteOutput<?> that){
    super();
    //TODO optimize;
    this.addAll(that);
  }
  private ByteArrSeq(OmniCollection.OfByte that){
    super();
    //TODO optimize
    this.addAll(that);
  }
  ...
  static void pullSurvivorsDown(byte[] arr,int srcOffset,int dstOffset,int dstBound,long[] survivorSet){
      ...
      implements ByteListDefault,Cloneable,RandomAccess
  {
    ...
    boolean isEqualTo(ListIterator<?> itr,int thisOffset,int thisBound){
      Object val;
      if(itr.hasNext() && (val=itr.next()) instanceof Byte){
        for(final var arr=this.arr;(byte)val==arr[thisOffset];){
          if(!itr.hasNext()){
            return thisOffset+1==thisBound;
          }
          if(++thisOffset>=thisBound || !((val=itr.next()) instanceof Byte)){
            break;
          }
        }
      }
    ...
    private boolean isEqualTo(int rootOffset,int size,OmniList.OfByte list){
      if(list instanceof ByteArrSeq.UncheckedList){
        final ByteArrSeq.UncheckedList uncheckedList;
        return (uncheckedList=(ByteArrSeq.UncheckedList)list).size==size && ((uncheckedList==this) || this.isEqualToHelper(rootOffset,rootOffset+size,uncheckedList.arr,0));
    ...
    private boolean isEqualTo(int size,OmniList.OfByte list){
      if(list instanceof ByteArrSeq.UncheckedList){
        final ByteArrSeq.UncheckedList that;
        return size==(that=(ByteArrSeq.UncheckedList)list).size && this.isEqualToHelper(0,size,that.arr,0);
        ...
        final ByteArrSeq.UncheckedSubList subList;
        final ByteArrSeq.UncheckedList thatRoot;
        return (subList=(ByteArrSeq.UncheckedSubList)list).size==size && ((thatRoot=subList.root)==this || this.isEqualToHelper(0,size,thatRoot.arr,subList.rootOffset));
      }else{
    ...
    @Override public OmniListIterator.OfByte listIterator(int index){
      ...
      void uncheckedInsert(int index,int size,byte val){
        final int tailDist;
  ...
  @Override public int hashCode(){
    ...
    private static class ListItr extends Itr implements OmniListIterator.OfByte{
      ...
      @Override public void forEachRemaining(Consumer<? super Byte> action){
        ...
        final UncheckedSubList parent;
        if((cursor=this.cursor)<(bound=(parent=this.parent).rootOffset+parent.size)){
          OmniArray.OfByte.ascendingForEach(parent.root.arr,cursor,cursor=bound-1,action::accept);
          this.lastRet=cursor;
          this.cursor=bound;
        }
      }
      ...
      extends UncheckedList
  {
    ...
    private boolean isEqualTo(int rootOffset,int size,OmniList.OfByte list){
      if(list instanceof ByteArrSeq.UncheckedList){
        final ByteArrSeq.UncheckedList that;
        return size==(that=(ByteArrSeq.UncheckedList)list).size && ((that==this)||(super.isEqualToHelper(rootOffset,rootOffset+size,that.arr,0)));
      }else if(list instanceof ByteArrSeq.CheckedSubList){
        final ByteArrSeq.CheckedSubList subList;
        final ByteArrSeq.CheckedList thatRoot;
        CheckedCollection.checkModCount((subList=(ByteArrSeq.CheckedSubList)list).modCount,(thatRoot=subList.root).modCount);
        ...
        final ByteArrSeq.UncheckedSubList subList;
        return size==(subList=(ByteArrSeq.UncheckedSubList)list).size && super.isEqualToHelper(rootOffset,rootOffset+size,subList.root.arr,subList.rootOffset);
      }else{
        //must be ByteDblLnkSeq
        final ByteDblLnkSeq dls;
        if((dls=(ByteDblLnkSeq)list) instanceof ByteDblLnkSeq.CheckedSubList){
    ...
    private boolean isEqualTo(int size,OmniList.OfByte list){
      if(list instanceof ByteArrSeq.UncheckedList){
        final ByteArrSeq.UncheckedList that;
        return size==(that=(ByteArrSeq.UncheckedList)list).size && super.isEqualToHelper(0,size,that.arr,0);
      }else if(list instanceof ByteDblLnkSeq){
        final ByteDblLnkSeq dls;
        if((dls=(ByteDblLnkSeq)list) instanceof ByteDblLnkSeq.CheckedSubList){
          final ByteDblLnkSeq.CheckedSubList subList;
        ...
        //must be ByteArrSeq.UncheckedSubList
        final ByteArrSeq.UncheckedSubList subList;
        return size==(subList=(ByteArrSeq.UncheckedSubList)list).size && super.isEqualToHelper(0,size,subList.root.arr,subList.rootOffset);
      }
    }
    @Override public boolean equals(Object val){
      if(val==this){
      ...
      extends AbstractByteItr
    {
      ...
      @Override public void forEachRemaining(ByteConsumer action){
        ...
        if((cursor=this.cursor)<(bound=(parent=this.parent).size)){
          ...
          final int lastRet;
          try{
            OmniArray.OfByte.ascendingForEach(parent.arr,cursor,lastRet=bound-1,action);
          }finally{
            CheckedCollection.checkModCount(modCount,parent.modCount,cursor,this.cursor);
          }
          this.cursor=bound;
      ...
      @Override public void forEachRemaining(Consumer<? super Byte> action){
        ...
        if((cursor=this.cursor)<(bound=(parent=this.parent).size)){
          ...
          final int lastRet;
          try{
            OmniArray.OfByte.ascendingForEach(parent.arr,cursor,lastRet=bound-1,action::accept);
          }finally{
            CheckedCollection.checkModCount(modCount,parent.modCount,cursor,this.cursor);
          }
          this.cursor=bound;
      ...
      extends AbstractByteItr
    {
      ...
      @Override public void forEachRemaining(ByteConsumer action){
        ...
        if((cursor=this.cursor)<(bound=(parent=this.parent).rootOffset+parent.size)){
          ...
          final int lastRet;
          try{
            OmniArray.OfByte.ascendingForEach(root.arr,cursor,lastRet=bound-1,action);
          }finally{
      ...
      @Override public void forEachRemaining(Consumer<? super Byte> action){
        ...
        if((cursor=this.cursor)<(bound=(parent=this.parent).rootOffset+parent.size)){
          ...
          final int lastRet;
          try{
            OmniArray.OfByte.ascendingForEach(root.arr,cursor,lastRet=bound-1,action::accept);
          }finally{
            CheckedCollection.checkModCount(modCount,root.modCount,cursor,this.cursor);
          }
          this.cursor=bound;
