    ...
    private void bubbleUpInitHelper(int index,int size,IntDblLnkNode newNode){
      ...
      if((size-=index)<=index){
        ...
        if(index==0){
          ...
          this.bubbleUpIncrementSize();
          after=(before=IntDblLnkNode.uncheckedIterateAscending(after,index)).next;
          before.prev=newNode;
        }
        after.prev=newNode;
      }
    ...
    private static class BidirectionalItr extends AscendingItr implements OmniListIterator.OfInt{
      ...
      @Override public void add(int val){
        ...
        if(++(root=(currList=this.parent).root).size!=1){
          if(++currList.size!=1){
            IntDblLnkNode after,before,newNode;
            if((after=this.curr)!=null){
              if((before=after.prev)!=null){
                currList.bubbleUpIncrementSize();
                before.next=newNode=new IntDblLnkNode(before,val,after);
              }else{
                currList.bubbleUpPrepend(newNode=new IntDblLnkNode(val,after));
                root.head=newNode;
              }
              after.prev=newNode;
            }else{
    ...
    @Override public OmniList.OfInt subList(int fromIndex,int toIndex){
      ...
      if((subListSize=toIndex-fromIndex)!=0){
        ...
        if((tailDist=this.size-toIndex)<=fromIndex){
          ...
          subListHead=subListSize<=fromIndex?IntDblLnkNode.uncheckedIterateDescending(subListTail,subListSize):IntDblLnkNode.iterateAscending(this.head,fromIndex);
        }else{
          subListHead=IntDblLnkNode.iterateAscending(this.head,fromIndex);
          subListTail=subListSize<=tailDist?IntDblLnkNode.uncheckedIterateAscending(subListHead,subListSize):IntDblLnkNode.iterateDescending(this.tail,tailDist);
        }
