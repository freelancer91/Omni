   ...
   ShortSubListDefault
{
    ...
    private void bubbleUpInitHelper(int index,int size,ShortDblLnkNode newNode){
      ...
      if((size-=index)<=index){
        ...
        if(index==0){
          ...
          this.bubbleUpIncrementSize();
          after=(before=ShortDblLnkNode.uncheckedIterateAscending(after,index)).next;
          before.prev=newNode;
        }
        after.prev=newNode;
      }
    ...
    @Override public OmniList.OfShort subList(int fromIndex,int toIndex){
      ...
      if((subListSize=toIndex-fromIndex)!=0){
        ...
        if((tailDist=this.size-toIndex)<=fromIndex){
          ...
          subListHead=subListSize<=fromIndex?ShortDblLnkNode.uncheckedIterateDescending(subListTail,subListSize):ShortDblLnkNode.iterateAscending(this.head,fromIndex);
        }else{
          subListHead=ShortDblLnkNode.iterateAscending(this.head,fromIndex);
          subListTail=subListSize<=tailDist?ShortDblLnkNode.uncheckedIterateAscending(subListHead,subListSize):ShortDblLnkNode.iterateDescending(this.tail,tailDist);
        }
  ...
  private static class CheckedSubList extends ShortDblLnkSeq{
    ...
    private static  void pullSurvivorsDown(ShortDblLnkNode prev,long[] survivorSet,int numSurvivors,int numRemoved){
      int wordOffset;
    ...
    private static  void pullSurvivorsDown(ShortDblLnkNode prev,long word,int numSurvivors,int numRemoved){
    ...
    private static  int collapseBodyHelper(ShortDblLnkNode newHead,int numLeft,ShortPredicate filter,CheckedList.ModCountChecker modCountChecker)
    {
      if(numLeft!=0){
        int numSurvivors;
        if(numLeft>64){
