 ...
 implements OmniCollection.OfLong,Externalizable,RandomAccess{
      ...
      implements LongListDefault,Cloneable,RandomAccess
  {
    ...
    boolean isEqualTo(ListIterator<?> itr,int thisOffset,int thisBound){
      Object val;
      if(itr.hasNext() && (val=itr.next()) instanceof Long){
        for(final var arr=this.arr;(long)val==arr[thisOffset];){
          if(!itr.hasNext()){
            return thisOffset+1==thisBound;
          }
          if(++thisOffset>=thisBound || !((val=itr.next()) instanceof Long)){
            break;
          }
        }
      }
    ...
    private boolean isEqualTo(int size,OmniList.OfLong list){
      if(list instanceof LongArrSeq.UncheckedList){
        ...
        return size==(that=(LongArrSeq.UncheckedList)list).size && this.isEqualToHelper(0,size,that.arr,0);
        ...
        final LongArrSeq.UncheckedSubList subList;
        final LongArrSeq.UncheckedList thatRoot;
        return (subList=(LongArrSeq.UncheckedSubList)list).size==size && ((thatRoot=subList.root)==this || this.isEqualToHelper(0,size,thatRoot.arr,subList.rootOffset));
      }else{
    ...
    @Override public OmniListIterator.OfLong listIterator(int index){
      ...
      void uncheckedInsert(int index,int size,long val){
        final int tailDist;
      ...
      extends UncheckedList
  {
    ...
    private boolean isEqualTo(int size,OmniList.OfLong list){
      if(list instanceof LongArrSeq.UncheckedList){
        final LongArrSeq.UncheckedList that;
        return size==(that=(LongArrSeq.UncheckedList)list).size && super.isEqualToHelper(0,size,that.arr,0);
      }else if(list instanceof LongDblLnkSeq){
        final LongDblLnkSeq dls;
        if((dls=(LongDblLnkSeq)list) instanceof LongDblLnkSeq.CheckedSubList){
          final LongDblLnkSeq.CheckedSubList subList;
        ...
        final LongArrSeq.UncheckedSubList subList;
        return size==(subList=(LongArrSeq.UncheckedSubList)list).size && super.isEqualToHelper(0,size,subList.root.arr,subList.rootOffset);
      }
    }
    @Override public boolean equals(Object val){
      if(val==this){
