...
public abstract class LongArrSeq extends 
AbstractSeq<Long>
 implements OmniCollection.OfLong,Externalizable,RandomAccess{
  ...
  private LongArrSeq(Collection<? extends Long> that){
    super();
    //TODO optimize
    this.addAll(that);
  }
  private LongArrSeq(OmniCollection.OfRef<? extends Long> that){
    super();
    //TODO optimize
    this.addAll(that);
  }
  private LongArrSeq(OmniCollection.OfBoolean that){
    super();
    //TODO optimize
    this.addAll(that);
  }
  private LongArrSeq(OmniCollection.LongOutput<?> that){
    super();
    //TODO optimize;
    this.addAll(that);
  }
  private LongArrSeq(OmniCollection.OfByte that){
    super();
    //TODO optimize
    this.addAll(that);
  }
  private LongArrSeq(OmniCollection.OfShort that){
    super();
    //TODO optimize
    this.addAll(that);
  }  
  private LongArrSeq(OmniCollection.OfInt that){
    super();
    //TODO optimize
    this.addAll(that);
  }
  private LongArrSeq(OmniCollection.OfLong that){
    super();
    //TODO optimize
    this.addAll(that);
  }
  private LongArrSeq(OmniCollection.OfChar that){
    super();
    //TODO optimize
    this.addAll(that);
  }
      ...
      implements LongListDefault,Cloneable,RandomAccess
  {
    ...
    private boolean isEqualTo(ListIterator<?> itr,int thisOffset){
      if(itr.hasNext()){
        Object val;
        if((val=itr.next()) instanceof Long){
          for(final var arr=this.arr;(long)val==arr[thisOffset];++thisOffset){
            if(!itr.hasNext()){
              return true;
            }
            if(!((val=itr.next()) instanceof Long)){
    ...
    private boolean isEqualToHelper(int thisOffset,int thisBound,long[] thatArr,int thatOffset){
    ...
    private boolean isEqualTo(int rootOffset,int bound,OmniList.OfLong list){
      if(list instanceof LongArrSeq.UncheckedList){
        return this.isEqualToHelper(rootOffset,bound,((LongArrSeq.UncheckedList)list).arr,0);
        ...
        final LongArrSeq.UncheckedList thatRoot;
    ...
    private boolean isEqualTo(int size,OmniList.OfLong list){
      if(list instanceof LongArrSeq.UncheckedList){
        return this.isEqualToHelper(0,size,((LongArrSeq.UncheckedList)list).arr,0);
        ...
        final LongArrSeq.UncheckedSubList subList;
        final LongArrSeq.UncheckedList thatRoot;
        return (thatRoot=(subList=(LongArrSeq.UncheckedSubList)list).root)==this || this.isEqualToHelper(0,size,thatRoot.arr,subList.rootOffset);
      }else{
      ...
      if(val instanceof List){
        ...
        if((list=(List<?>)val) instanceof AbstractSeq){
          ...
          if((abstractSeq=(AbstractSeq<?>)list).size==size){
            if(abstractSeq instanceof OmniList.OfLong){
              return this.isEqualTo(size,(OmniList.OfLong)abstractSeq);
      ...
      if(val instanceof List){
         ...
         if((list=(List<?>)val) instanceof AbstractSeq){
            ...
            if((abstractSeq=(AbstractSeq<?>)list).size==size){
              if(abstractSeq instanceof OmniList.OfLong){
                final int rootOffset;
                return root.isEqualTo(rootOffset=this.rootOffset,rootOffset+size,(OmniList.OfLong)abstractSeq);
              }else if(abstractSeq instanceof OmniList.OfRef){
                final int rootOffset;
  ...
  @Override public int hashCode(){
    ...
    private static class ListItr extends Itr implements OmniListIterator.OfLong{
      ...
      @Override public void forEachRemaining(Consumer<? super Long> action){
        ...
        final UncheckedSubList parent;
        if((cursor=this.cursor)<(bound=(parent=this.parent).rootOffset+parent.size)){
          OmniArray.OfLong.ascendingForEach(parent.root.arr,cursor,cursor=bound-1,action::accept);
          this.lastRet=cursor;
          this.cursor=bound;
        }
      }
      ...
      extends UncheckedList
  {
    ...
    private boolean isEqualTo(int rootOffset,int bound,OmniList.OfLong list){
      if(list instanceof LongArrSeq.UncheckedList){
        return super.isEqualToHelper(rootOffset,bound,((LongArrSeq.UncheckedList)list).arr,0);
      }else if(list instanceof LongArrSeq.CheckedSubList){
        final LongArrSeq.CheckedSubList subList;
        final LongArrSeq.CheckedList thatRoot;
        if((thatRoot=(subList=(LongArrSeq.CheckedSubList)list).root)==this){
          return true;
        }
        ...
        final LongArrSeq.UncheckedSubList subList;
        return super.isEqualToHelper(rootOffset,bound,(subList=(LongArrSeq.UncheckedSubList)list).root.arr,subList.rootOffset);
      }else{
        //must be LongDblLnkSEq
        final LongDblLnkNode head;
        if(list instanceof LongDblLnkSeq.CheckedSubList){
    ...
    private boolean isEqualTo(int size,OmniList.OfLong list){
      if(list instanceof LongArrSeq.UncheckedList){
        return super.isEqualToHelper(0,size,((LongArrSeq.UncheckedList)list).arr,0);
      }else if(list instanceof LongDblLnkSeq){
        final LongDblLnkNode head;
        if(list instanceof LongDblLnkSeq.CheckedSubList){
          final LongDblLnkSeq.CheckedSubList subList;
        ...
        final LongArrSeq.UncheckedSubList subList;
        return super.isEqualToHelper(0,size,(subList=(LongArrSeq.UncheckedSubList)list).root.arr,subList.rootOffset);
      }
    }
    @Override public boolean equals(Object val){
      if(val==this){
      ...
      extends AbstractLongItr
    {
      ...
      @Override public void forEachRemaining(LongConsumer action){
        ...
        if((cursor=this.cursor)<(bound=(parent=this.parent).size)){
          ...
          final int lastRet;
          try{
            OmniArray.OfLong.ascendingForEach(parent.arr,cursor,lastRet=bound-1,action);
          }finally{
            CheckedCollection.checkModCount(modCount,parent.modCount,cursor,this.cursor);
          }
          this.cursor=bound;
      ...
      @Override public void forEachRemaining(Consumer<? super Long> action){
        ...
        if((cursor=this.cursor)<(bound=(parent=this.parent).size)){
          ...
          final int lastRet;
          try{
            OmniArray.OfLong.ascendingForEach(parent.arr,cursor,lastRet=bound-1,action::accept);
          }finally{
            CheckedCollection.checkModCount(modCount,parent.modCount,cursor,this.cursor);
          }
          this.cursor=bound;
      ...
      extends AbstractLongItr
    {
      ...
      @Override public void forEachRemaining(LongConsumer action){
        ...
        if((cursor=this.cursor)<(bound=(parent=this.parent).rootOffset+parent.size)){
          ...
          final int lastRet;
          try{
            OmniArray.OfLong.ascendingForEach(root.arr,cursor,lastRet=bound-1,action);
          }finally{
      ...
      @Override public void forEachRemaining(Consumer<? super Long> action){
        ...
        if((cursor=this.cursor)<(bound=(parent=this.parent).rootOffset+parent.size)){
          ...
          final int lastRet;
          try{
            OmniArray.OfLong.ascendingForEach(root.arr,cursor,lastRet=bound-1,action::accept);
          }finally{
            CheckedCollection.checkModCount(modCount,root.modCount,cursor,this.cursor);
          }
          this.cursor=bound;
