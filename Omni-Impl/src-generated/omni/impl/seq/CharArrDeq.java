...
public class CharArrDeq implements OmniDeque.OfChar,Externalizable,Cloneable,RandomAccess{
  ...
  public static class Checked extends CharArrDeq{
    ...
    private void noElementsLeftToMarkFragmentedBefore(char[] arr,int head,int gapBegin){
    ...
    private void noElementsLeftToMarkFragmentedGap(char[] arr,int head,int gapBegin,int gapEnd){
    ...
    private void noElementsLeftToMarkFragmentedAfter(char[] arr,int head,int gapBegin,int gapEnd){
      assert gapBegin>=0;
      assert head>gapEnd;
    ...
    private void noElementsLeftToMark(char[] arr,int head,int gapBegin,int gapEnd){
    ...
    private static abstract class CollapseData{
      ...
      abstract void pullDownMarkRangeSpansSplit(char[] arr,int srcOffset,int dstOffset,int survivorIndex,int dstBound);
      abstract void pullDownMarkRangeSpansSplitDstBoundGoesBeyondSplit(char[] arr,int srcOffset,int dstOffset,int survivorIndex,int dstBound);
      private int collapseTailHelper(char[] arr,int gapEnd,int tail){
    ...
    private static class BigCollapseData extends CollapseData{
      ...
      @Override void arrSeqPullDown(char[] arr,int srcOffset,int dstOffset,int dstBound){
        assert dstOffset>=0;
        assert srcOffset>dstOffset;
        assert dstOffset<dstBound;
        assert dstBound<arr.length;
        CharArrSeq.pullSurvivorsDown(arr,srcOffset,dstOffset,dstBound,survivorSet);
      }
      @Override void pullSurvivorsDown(char[] arr,int dstOffset,int survivorIndex,int dstBound){
        assert dstOffset>=0;
        assert dstOffset<dstBound;
        assert dstBound<arr.length;
        int wordOffset;
      ...
      @Override void pullDownMarkRangeSpansSplit(char[] arr,int srcOffset,int dstOffset,int survivorIndex,int dstBound){
        //TODO
        throw new UnsupportedOperationException();
      }
      @Override void pullDownMarkRangeSpansSplitDstBoundGoesBeyondSplit(char[] arr,int srcOffset,int dstOffset,int survivorIndex,int dstBound){
        //TODO
        throw new UnsupportedOperationException();
      }
    }
    private static class SmallCollapseData extends CollapseData{
      ...
      private void finalizePullDown(char[] arr,int srcOffset,int dstOffset,long word){
      ...
      private void pullDownFromTailSpanToHeadSpan(char[] arr,int srcOffset,int dstOffset,long word){
      ...
      @Override void arrSeqPullDown(char[] arr,int srcOffset,int dstOffset,int dstBound){
        ...
        CharArrSeq.pullSurvivorsDown(arr,srcOffset,dstOffset,survivorWord);
      }
      @Override void pullSurvivorsDown(char[] arr,int dstOffset,int survivorIndex,int dstBound){
        assert dstOffset>=0;
        assert dstOffset<dstBound;
        assert dstBound<arr.length;
        CharArrSeq.pullSurvivorsDown(arr,dstOffset+1,dstOffset,this.survivorWord>>>survivorIndex);
      }
    ...
    private void collapseBodyHelper(char[] arr,int head,int tail,CharPredicate filter,int modCount){
      for(int gapBegin=head+1;gapBegin!=tail;++gapBegin){
        if(filter.test((char)arr[gapBegin])){
          for(int gapEnd=gapBegin+1;gapEnd!=tail;++gapEnd){
            if(!filter.test((char)arr[gapEnd])){
    ...
    private CollapseData getNonfragmentedCollapseData(int srcOffset,int numLeft,CharPredicate filter){
      return numLeft>64?new BigCollapseData(this,srcOffset,numLeft,filter):new SmallCollapseData(this,srcOffset,numLeft,filter);
    }
    private CollapseData getFragmentedCollapseData(int srcOffset,int numLeft,CharPredicate filter,int arrBound){
