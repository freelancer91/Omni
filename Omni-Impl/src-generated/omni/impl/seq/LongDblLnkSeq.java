   ...
   LongSubListDefault
{
  ...
  private static class UncheckedSubList extends LongDblLnkSeq{
    ...
    private void bubbleUpAppend(LongDblLnkNode newNode){
      for(var curr=this;;){
        curr.tail=newNode;
    ...
    private void bubbleUpAppend(LongDblLnkNode newNode,LongDblLnkNode oldTail){
      for(var curr=this;;){
        curr.tail=newNode;
        if((curr=curr.parent)==null){
          return;
    ...
    private void bubbleUpPrepend(LongDblLnkNode newNode){
      for(var curr=this;;){
        curr.head=newNode;
        if((curr=curr.parent)==null){
          break;
    ...
    private void bubbleUpPrepend(LongDblLnkNode newNode,LongDblLnkNode oldHead){
      for(var curr=this;;){
        curr.head=newNode;
        if((curr=curr.parent)==null){
          return;
    ...
    @Override public void add(int index,long val){
      ...
      if((size-=index)<index){
        ...
        if(size==1){
          ...
          before=(after=LongDblLnkNode.iterateDescending(curr.tail,size-1)).prev;
          after.prev=newNode;
          curr.bubbleUpIncrementSize();
        }
        before.next=newNode;
        ...
        if(index==0){
          ...
          after=(before=LongDblLnkNode.iterateAscending(curr.head,index-1)).next;
          before.next=newNode;
          curr.bubbleUpIncrementSize();
        }
        after.prev=newNode;
    ...
    @Override void addLast(long val){
      ...
      var newNode=new LongDblLnkNode(val);
    ...
    private void collapseHeadHelper(LongDblLnkNode oldHead,LongDblLnkNode tail,LongPredicate filter)
    {
      //TODO
    }
    private void collapseTailHelper(LongDblLnkNode head,LongDblLnkNode oldTail,LongPredicate filter)
    {
      //TODO
    }
    private void collapseHeadAndTailHelper(LongDblLnkNode oldHead,LongDblLnkNode oldTail,LongPredicate filter)
    {
      //TODO
    }
    private boolean collapseBodyHelper(LongDblLnkNode head,LongDblLnkNode tail,LongPredicate filter)
    {
      //TODO
      return false;
    }
    ...
    @Override public OmniList.OfLong subList(int fromIndex,int toIndex){
      ...
      if((tailDist=this.size-toIndex)<=fromIndex){
        ...
        subListHead=subListSize<=fromIndex?LongDblLnkNode.iterateDescending(subListTail,subListSize):LongDblLnkNode.iterateAscending(this.head,fromIndex);
      }else{
        subListHead=LongDblLnkNode.iterateAscending(this.head,fromIndex);
        subListTail=subListSize<=tailDist?LongDblLnkNode.iterateAscending(subListHead,subListSize):LongDblLnkNode.iterateDescending(this.tail,tailDist);
      }
    ...
    private boolean uncheckedremoveVal(LongDblLnkNode head
    ,long val
    ){
      if(val==(head.val)){
        --root.size;
  ...
  private static class CheckedSubList extends LongDblLnkSeq{
      ...
      implements OmniListIterator.OfLong
    {
      ...
      BidirectionalItr(CheckedSubList parent,LongDblLnkNode curr,int currIndex){
        this.parent=parent;
        this.modCount=parent.modCount;
        this.curr=curr;
        this.currIndex=currIndex;
    ...
    @Override public void add(int index,long val){
      ...
      if((size-=index)<index){
        ...
        if(size==1){
          ...
          //iterate from the tail and insert
          LongDblLnkNode before;
          (before=(tail=LongDblLnkNode.iterateDescending(tail,size-1)).prev).next=before=new LongDblLnkNode(before,val,tail);
          tail.prev=before;
        }
      }else{
        //the insertion point is closer to the head
