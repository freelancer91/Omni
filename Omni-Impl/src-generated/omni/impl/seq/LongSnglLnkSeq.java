...
public abstract class LongSnglLnkSeq 
extends AbstractSeq.OfSignedIntegralPrimitive
implements OmniCollection.OfLong
{
  transient Node head;
  ...
  private LongSnglLnkSeq(Node onlyNode)
  {
    super(1);
    this.head=onlyNode;
  }
  private LongSnglLnkSeq(Node head,int size)
  {
    super(size);
    this.head=head;
  }
  ...
  public boolean contains(Object val)
  {
    ...
    if((head=this.head)!=null)
    {
      ...
      {
        return head.uncheckedcontains((long)(val));
      }
    }
    return false;
  }
  public int search(Object val)
  {
    ...
    if((head=this.head)!=null)
    {
      if(val instanceof Long)
      {
        return head.uncheckedsearch((long)(val));
      }
    }
    return -1;
  }
  ...
  boolean uncheckedremoveVal(Node head,long val)
  {
    if(val!=(head.val))
    {
      Node prev;
  ...
  private static abstract class AbstractCheckedItr extends AbstractUncheckedItr
  {
    ...
    @Override
    void uncheckedForEachRemaining(Node next,LongConsumer action)
    {
      Node prev,pPrev;
  ...
  public static class Stack extends LongSnglLnkSeq implements OmniStack.OfLong
  {
    public Stack()
    {
      super();
    ...
    private void uncheckedRemoveIfHelper(Node prev,Node curr,LongPredicate filter)
    {
      int numRemoved=1;
    ...
    @Override
    boolean uncheckedRemoveIf(Node curr,LongPredicate filter)
    {
      if(filter.test(curr.val))
      {
        while((curr=curr.next)!=null)
    ...
    public static class Checked extends Stack
    {
      ...
      private void uncheckedRemoveIfHelper(int modCount,int numLeft,Node prev,Node curr,LongPredicate filter)
      {
        int numRemoved=1;
        for(;;)
        {
  ...
  public static class Queue extends LongSnglLnkSeq implements OmniQueue.OfLong
  {
    transient Node tail;
    public Queue()
    {
    ...
    private int uncheckedRemoveIfHelper(Node curr,LongPredicate filter)
    {
      int numSurvivors=1;
    ...
    public static class Checked extends Queue
    {
      ...
      private int uncheckedRemoveIfHelper(int modCount,int numLeft,Node curr,LongPredicate filter)
      {
        int numSurvivors=1;
  ...
  static class Node
  {
    private int countNumRemainingNodes(CheckedCollection.AbstractModCountChecker modCountChecker,int numLeft,LongPredicate filter)
    {
      int numSurvivors=1;
    ...
    private int countNumRemovedNodes(CheckedCollection.AbstractModCountChecker modCountChecker,int numLeft,LongPredicate filter)
    {
    ...
    private int countNumRemovedNodes(LongPredicate filter)
    {
      int numRemoved=0;
    ...
    private int countNumRemainingNodes(LongPredicate filter)
    {
      int numLeft=1;
    ...
    private void uncheckedForEachForward(LongConsumer action)
    {
      var curr=this;
      do
      {
    ...
    private int uncheckedForwardHashCode()
    {
      ...
      for(var curr=next;curr!=null;hash=hash*31+Long.hashCode(curr.val),curr=curr.next)
      {  
      }
      return hash;
    }
