...
public abstract class LongSnglLnkSeq implements OmniCollection.OfLong,Cloneable,Externalizable{
  ...
  public static class CheckedStack extends UncheckedStack{
    ...
    private int removeIfHelper(LongSnglLnkNode prev,LongPredicate filter,int numLeft,int modCount)
    {
      if(numLeft!=0)
      {
        ...
        {
          long[] survivorSet;
          numSurvivors=LongSnglLnkNode.markSurvivors(prev.next,filter,survivorSet=BitSetUtil.getBitSet(numLeft));
          CheckedCollection.checkModCount(modCount,this.modCount);
          if((numLeft-=numSurvivors)!=0)
          {
            LongSnglLnkNode.pullSurvivorsDown(prev,filter,survivorSet,numSurvivors,numLeft);
          }
        }
        else
        {
          long survivorWord=LongSnglLnkNode.markSurvivors(prev.next,filter);
          CheckedCollection.checkModCount(modCount,this.modCount);
          if((numLeft-=(numSurvivors=Long.bitCount(survivorWord)))!=0)
          {
            LongSnglLnkNode.pullSurvivorsDown(prev,survivorWord,numSurvivors,numLeft);
          }
        }
        return numSurvivors;
      }
