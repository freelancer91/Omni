  ...
  public static class CheckedList extends UncheckedList{
    ...
    private boolean isEqualTo(int size,OmniList.OfByte list){
      if(list instanceof ByteDblLnkSeq){
        ...
        return size==(that=(ByteArrSeq.UncheckedList)list).size && SequenceEqualityUtil.isEqualTo(that.arr,0,size,this.head);
    ...
    private boolean isEqualTo(Node thisHead,int size,OmniList.OfByte list){
      if(list instanceof ByteDblLnkSeq){
        ...
        final ByteArrSeq.UncheckedSubList subList;
        final int thatOffset;
        return size==(subList=(ByteArrSeq.UncheckedSubList)list).size && SequenceEqualityUtil.isEqualTo(subList.root.arr,thatOffset=subList.rootOffset,thatOffset+size,thisHead);
      }
    }
    @Override public boolean equals(Object val){
      if(val==this){
