   ...
   FloatSubListDefault
{
  ...
  private FloatDblLnkNode getNode(int index,int size){
    ...
    if((tailDist=size-index)<index){
      ...
      return FloatDblLnkNode.iterateDescending(tail,tailDist-1);
    }else{
  ...
  private static class UncheckedSubList extends FloatDblLnkSeq{
    ...
    private void bubbleUpAppend(FloatDblLnkNode newNode){
      for(var curr=this;;){
        curr.tail=newNode;
    ...
    private void bubbleUpAppend(FloatDblLnkNode newNode,FloatDblLnkNode oldTail){
      for(var curr=this;;){
        curr.tail=newNode;
        if((curr=curr.parent)==null){
          return;
    ...
    private void bubbleUpPrepend(FloatDblLnkNode newNode){
      for(var curr=this;;){
        curr.head=newNode;
        if((curr=curr.parent)==null){
          break;
    ...
    private void bubbleUpPrepend(FloatDblLnkNode newNode,FloatDblLnkNode oldHead){
      for(var curr=this;;){
        curr.head=newNode;
        if((curr=curr.parent)==null){
          return;
    ...
    @Override public void add(int index,float val){
      ...
      if((size-=index)<index){
        ...
        if(size==1){
          ...
          before=(after=FloatDblLnkNode.iterateDescending(curr.tail,size-1)).prev;
          after.prev=newNode;
          curr.bubbleUpIncrementSize();
        }
        before.next=newNode;
        ...
        if(index==0){
          ...
          after=(before=FloatDblLnkNode.iterateAscending(curr.head,index-1)).next;
          before.next=newNode;
          curr.bubbleUpIncrementSize();
        }
        after.prev=newNode;
    ...
    @Override void addLast(float val){
      ...
      var newNode=new FloatDblLnkNode(val);
    ...
    private void collapseHeadHelper(FloatDblLnkNode oldHead,FloatDblLnkNode tail,FloatPredicate filter)
    {
      //TODO
    }
    private void collapseTailHelper(FloatDblLnkNode head,FloatDblLnkNode oldTail,FloatPredicate filter)
    {
      //TODO
    }
    private void collapseHeadAndTailHelper(FloatDblLnkNode oldHead,FloatDblLnkNode oldTail,FloatPredicate filter)
    {
      //TODO
    }
    private boolean collapseBodyHelper(FloatDblLnkNode head,FloatDblLnkNode tail,FloatPredicate filter)
    {
      //TODO
      return false;
    }
    ...
    @Override public float removeFloatAt(int index){
      ...
      if((size=(--this.size)-index)<=index){
        ...
        if(index==0){
          ...
          ret=(after=( head=FloatDblLnkNode.iterateAscending(head,index)).next).val;
        ...
        if((size=++(currList=this.parent).size)==1){
          ...
          do{
            ...
            if((parent=currList.parent)==null){
              //all parents were empty, insert in the root
              ((FloatDblLnkSeq)currList.root).insertNode(currList.parentOffset,newNode);
    ...
    @Override public OmniList.OfFloat subList(int fromIndex,int toIndex){
      ...
      if((tailDist=this.size-toIndex)<=fromIndex){
        ...
        subListHead=subListSize<=fromIndex?FloatDblLnkNode.iterateDescending(subListTail,subListSize):FloatDblLnkNode.iterateAscending(this.head,fromIndex);
      }else{
        subListHead=FloatDblLnkNode.iterateAscending(this.head,fromIndex);
        subListTail=subListSize<=tailDist?FloatDblLnkNode.iterateAscending(subListHead,subListSize):FloatDblLnkNode.iterateDescending(this.tail,tailDist);
      }
    ...
    private boolean uncheckedremoveValBits(FloatDblLnkNode head
    ,int bits
    ){
    ...
    private boolean uncheckedremoveVal0(FloatDblLnkNode head
    ){
      if(0==(head.val)){
        --root.size;
        if(--this.size==0){
    ...
    private boolean uncheckedremoveValNaN(FloatDblLnkNode head
    ){
  ...
  private static class CheckedSubList extends FloatDblLnkSeq{
      ...
      implements OmniListIterator.OfFloat
    {
      ...
      BidirectionalItr(CheckedSubList parent,FloatDblLnkNode curr,int currIndex){
        this.parent=parent;
        this.modCount=parent.modCount;
        this.curr=curr;
        this.currIndex=currIndex;
    ...
    @Override public void add(int index,float val){
      ...
      if((size-=index)<index){
        ...
        if(size==1){
          ...
          //iterate from the tail and insert
          FloatDblLnkNode before;
          (before=(tail=FloatDblLnkNode.iterateDescending(tail,size-1)).prev).next=before=new FloatDblLnkNode(before,val,tail);
          tail.prev=before;
        }
      }else{
        //the insertion point is closer to the head
