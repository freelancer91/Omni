   ...
   DoubleSubListDefault
{
    ...
    private void bubbleUpInitHelper(int index,int size,DoubleDblLnkNode newNode){
      ...
      if((size-=index)<=index){
        ...
        if(index==0){
          ...
          this.bubbleUpIncrementSize();
          after=(before=DoubleDblLnkNode.uncheckedIterateAscending(after,index)).next;
          before.prev=newNode;
        }
        after.prev=newNode;
      }
    ...
    private static class BidirectionalItr extends AscendingItr implements OmniListIterator.OfDouble{
      ...
      @Override public void add(double val){
        ...
        if(++(root=(currList=this.parent).root).size!=1){
          if(++currList.size!=1){
            DoubleDblLnkNode after,before,newNode;
            if((after=this.curr)!=null){
              if((before=after.prev)!=null){
                currList.bubbleUpIncrementSize();
                before.next=newNode=new DoubleDblLnkNode(before,val,after);
              }else{
                currList.bubbleUpPrepend(newNode=new DoubleDblLnkNode(val,after));
                root.head=newNode;
              }
              after.prev=newNode;
            }else{
    ...
    @Override public OmniList.OfDouble subList(int fromIndex,int toIndex){
      ...
      if((subListSize=toIndex-fromIndex)!=0){
        ...
        if((tailDist=this.size-toIndex)<=fromIndex){
          ...
          subListHead=subListSize<=fromIndex?DoubleDblLnkNode.uncheckedIterateDescending(subListTail,subListSize):DoubleDblLnkNode.iterateAscending(this.head,fromIndex);
        }else{
          subListHead=DoubleDblLnkNode.iterateAscending(this.head,fromIndex);
          subListTail=subListSize<=tailDist?DoubleDblLnkNode.uncheckedIterateAscending(subListHead,subListSize):DoubleDblLnkNode.iterateDescending(this.tail,tailDist);
        }
  ...
  private static class CheckedSubList extends DoubleDblLnkSeq{
    ...
    private static  void pullSurvivorsDown(DoubleDblLnkNode prev,long[] survivorSet,int numSurvivors,int numRemoved){
      int wordOffset;
    ...
    private static  void pullSurvivorsDown(DoubleDblLnkNode prev,long word,int numSurvivors,int numRemoved){
