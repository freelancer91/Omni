...
public class ByteArrDeq implements OmniDeque.OfByte,Externalizable,Cloneable,RandomAccess{
  ...
  public static class Checked extends ByteArrDeq{
    ...
    private void pullUpAcrossSplit(byte[] arr,int head,int gapBegin,int gapEnd){
    ...
    private static abstract class CollapseData{
      ...
      abstract void pullDownAcrossSplit(byte[] arr,int srcOffset,int dstOffset,int dstBound);
    ...
    private static class BigCollapseData extends CollapseData{
          ...
          do{
            ...
            if(++srcOffset==arrBound){
              for(currentRunLength=0;;){
                while((marker<<=1)!=0L){
                  if(filter.test((byte)arr[srcOffset-arrBound])){
                    currentRunLength=0;
                  }else{
                    word|=marker;
                    if(currentRunLength==0){
      ...
      @Override void pullDownAcrossSplit(byte[] arr,int srcOffset,int dstOffset,int dstBound){
        //TODO
    ...
    private static class SmallCollapseData extends CollapseData{
      ...
      @Override void arrSeqPullDown(byte[] arr,int srcOffset,int dstOffset,int dstBound){
        ...
        ByteArrSeq.pullSurvivorsDown(arr,srcOffset,dstOffset,dstBound,survivorWord);
      }
      @Override void pullSurvivorsDown(byte[] arr,int dstOffset,int survivorIndex,int dstBound){
        assert dstOffset>=0;
        assert dstOffset<dstBound;
        assert dstBound<arr.length;
        ByteArrSeq.pullSurvivorsDown(arr,dstOffset+1,dstOffset,dstBound,this.survivorWord>>>survivorIndex);
      }
