...
public class ByteArrDeq implements OmniDeque.OfByte,Externalizable,Cloneable,RandomAccess{
  ...
  private static class AscendingItr extends AbstractDeqItr
  {
    ...
    private void nonfragmentedRemove(int head,int cursor,int tail,ByteArrDeq root){
      int headDist,tailDist;
    ...
    private void fragmentedRemove(int head,int cursor,int tail,ByteArrDeq root){
      byte[] arr;
  ...
  private static class DescendingItr extends AscendingItr
  {
    ...
    @Override void uncheckedForEachRemaining(int cursor,ByteConsumer action){
      ...
      if(--cursor<(head=root.head)){
         if(cursor>=0){
           OmniArray.OfByte.descendingForEach(arr,cursor,0,action);
         }
         cursor=arr.length-1;
      }
    ...
    private void fragmentedRemove(int head,int lastRet,int tail,ByteArrDeq root){
      byte[] arr;
      int headDist,tailDist,arrBound=(arr=root.arr).length;
    ...
    private void nonfragmentedRemove(int head,int lastRet,int tail,ByteArrDeq root){
      int headDist,tailDist;
