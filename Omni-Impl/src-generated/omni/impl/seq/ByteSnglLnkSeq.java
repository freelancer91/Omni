...
public abstract class ByteSnglLnkSeq implements OmniCollection.OfByte,Cloneable,Externalizable{
  ...
  public static class CheckedStack extends UncheckedStack{
    ...
    private int removeIfHelper(ByteSnglLnkNode prev,BytePredicate filter,int numLeft,int modCount)
    {
      if(numLeft!=0)
      {
        ...
        {
          long[] survivorSet;
          numSurvivors=ByteSnglLnkNode.markSurvivors(prev.next,filter,survivorSet=BitSetUtil.getBitSet(numLeft));
          CheckedCollection.checkModCount(modCount,this.modCount);
          if((numLeft-=numSurvivors)!=0)
          {
            ByteSnglLnkNode.pullSurvivorsDown(prev,filter,survivorSet,numSurvivors,numLeft);
          }
        }
        else
        {
          long survivorWord=ByteSnglLnkNode.markSurvivors(prev.next,filter);
          CheckedCollection.checkModCount(modCount,this.modCount);
          if((numLeft-=(numSurvivors=Long.bitCount(survivorWord)))!=0)
          {
            ByteSnglLnkNode.pullSurvivorsDown(prev,survivorWord,numSurvivors,numLeft);
          }
        }
        return numSurvivors;
      }
