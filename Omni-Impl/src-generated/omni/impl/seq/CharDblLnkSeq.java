    ...
    private void bubbleUpInitHelper(int index,int size,CharDblLnkNode newNode){
      ...
      if((size-=index)<=index){
        ...
        if(index==0){
          ...
          this.bubbleUpIncrementSize();
          after=(before=CharDblLnkNode.uncheckedIterateAscending(after,index)).next;
          before.prev=newNode;
        }
        after.prev=newNode;
      }
    ...
    private static class BidirectionalItr extends AscendingItr implements OmniListIterator.OfChar{
      ...
      @Override public void add(char val){
        ...
        if(++(root=(currList=this.parent).root).size!=1){
          if(++currList.size!=1){
            CharDblLnkNode after,before,newNode;
            if((after=this.curr)!=null){
              if((before=after.prev)!=null){
                currList.bubbleUpIncrementSize();
                before.next=newNode=new CharDblLnkNode(before,val,after);
              }else{
                currList.bubbleUpPrepend(newNode=new CharDblLnkNode(val,after));
                root.head=newNode;
              }
              after.prev=newNode;
            }else{
    ...
    @Override public OmniList.OfChar subList(int fromIndex,int toIndex){
      ...
      if((subListSize=toIndex-fromIndex)!=0){
        ...
        if((tailDist=this.size-toIndex)<=fromIndex){
          ...
          subListHead=subListSize<=fromIndex?CharDblLnkNode.uncheckedIterateDescending(subListTail,subListSize):CharDblLnkNode.iterateAscending(this.head,fromIndex);
        }else{
          subListHead=CharDblLnkNode.iterateAscending(this.head,fromIndex);
          subListTail=subListSize<=tailDist?CharDblLnkNode.uncheckedIterateAscending(subListHead,subListSize):CharDblLnkNode.iterateDescending(this.tail,tailDist);
        }
