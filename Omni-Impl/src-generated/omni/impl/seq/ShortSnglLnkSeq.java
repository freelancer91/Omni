  ...
  public static class CheckedStack extends UncheckedStack{
    ...
    private int removeIfHelper(ShortSnglLnkNode prev,ShortPredicate filter,int numLeft,int modCount)
    {
      if(numLeft!=0)
      {
        ...
        {
          long[] survivorSet;
          numSurvivors=ShortSnglLnkNode.markSurvivors(prev.next,filter,survivorSet=BitSetUtil.getBitSet(numLeft));
          CheckedCollection.checkModCount(modCount,this.modCount);
          if((numLeft-=numSurvivors)!=0)
          {
            ShortSnglLnkNode.pullSurvivorsDown(prev,filter,survivorSet,numSurvivors,numLeft);
          }
        }
        else
        {
          long survivorWord=ShortSnglLnkNode.markSurvivors(prev.next,filter);
          CheckedCollection.checkModCount(modCount,this.modCount);
          if((numLeft-=(numSurvivors=Long.bitCount(survivorWord)))!=0)
          {
            ShortSnglLnkNode.pullSurvivorsDown(prev,survivorWord,numSurvivors,numLeft);
          }
        }
        return numSurvivors;
      }
