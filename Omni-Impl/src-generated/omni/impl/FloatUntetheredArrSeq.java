...
abstract class FloatUntetheredArrSeq implements OmniCollection.OfFloat,Externalizable
{
  ...
  static abstract class AbstractFullView implements OmniCollection.OfFloat
  {
    transient final FloatUntetheredArrSeq root;
    AbstractFullView(FloatUntetheredArrSeq root){
      this.root=root;
    }
    ...
    @Override public boolean removeIf(Predicate<? super Float> filter){
      return root.removeIf((FloatPredicate)filter::test);
    }
  }
  @Override public int size(){
    int tail;
...
Float[] uncheckedAscendingToArray(int head,int tail){
  final Float[] dst;
  int size;
  if((size=(++tail)-head)>0){
...
Float[] uncheckedDescendingToArray(int head,int tail){
  final Float[] dst;
  int size;
  if((size=(++tail)-head)>0){
...
double[] uncheckedDescendingToDoubleArray(int head,int tail){
  ...
  /*
  private static float ceilingImplHelper(float[] tmp,int head,int tail,FloatToIntFunction searchFunc){
    while(head<=tail){
      final int mid;
      final float tmp;
  ...
  private static float nonfragmentedCeilingImpl(float[] arr,int head,int tail,FloatToIntFunction searchFunc){
    float tmp;
    int mid;
  ...
  private static float fragmentedCeilingImpl(float[] arr,int head,int tail,FloatToIntFunction searchFunc){
    //TODO
    throw new omni.util.NotYetImplementedException();
  }
  float ceilingImpl(int tail,FloatToIntFunction searchFunc){
    int head;
