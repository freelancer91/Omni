...
public abstract class AbstractIntList extends AbstractSeq.OfSignedIntegralPrimitive{
  ...
  public int lastIndexOf(double val){
    final int size,v;
    if((size=this.size)!=0&&val==(v=(int)val)){ return uncheckedLastIndexOf(size,v); }
    return -1;
  }
  public int lastIndexOf(float val){
    final int size,v;
    if((size=this.size)!=0&&(double)val==(double)(v=(int)val)){ return uncheckedLastIndexOf(size,v); }
    return -1;
  }
  ...
  public int lastIndexOf(Object val){
    final int size;
    if((size=this.size)!=0&&val instanceof Integer){ return uncheckedLastIndexOf(size,(int)val); }
    return -1;
  }
