...
abstract class AbstractCharArrSeq extends AbstractCharList{
    ...
    static abstract class Checked extends AbstractCharArrSeq{
        ...
        private static int pullSurvivorsDown(char[] arr,long[] survivorSet,int dstOffset,int srcOffset,
                int numSurvivors){
        ...
        @Override
        void uncheckedReplaceAll(int size,CharUnaryOperator operator){
            final int expectedModCount=modCount;
            try{
        ...
        @Override
        void uncheckedSort(int size,CharComparator sorter){
            final int expectedModCount=modCount;
            try{
                        ...
                        if(filter.test(arr[srcOffset])){
                            ...
                            while(++srcOffset!=srcBound){
                                ...
                                if(!filter.test(v=arr[srcOffset])){
                                    ...
                                    this.size=size-(size=((AbstractCharArrSeq)root).finalizeSubListBatchRemove(arr,
                                            dstOffset,srcBound));
                                    bubbleUpDecrementSize(parent,size);
                                    return true;
                                }
                            ...
                            root.modCount=++expectedModCount;
                            modCount=expectedModCount;
                            this.size=size-(size=((AbstractCharArrSeq)root).finalizeSubListBatchRemove(arr,dstOffset,
                                    srcBound));
                            bubbleUpDecrementSize(parent,size);
                            return true;
                        }
