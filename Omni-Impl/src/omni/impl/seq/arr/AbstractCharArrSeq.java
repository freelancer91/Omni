...
abstract class AbstractCharArrSeq extends AbstractCharList{
  transient char[] arr;
  ...
  static int uncheckedIndexOf(char[] arr,int offset,int bound,int val){
    int index=offset;
    do{
  ...
  static int uncheckedLastIndexOf(char[] arr,int offset,int bound,int val){
    do{
  ...
  private AbstractCharArrSeq(){
    super();
    arr=OmniArray.OfChar.DEFAULT_ARR;
  }
  private AbstractCharArrSeq(int capacity){
    super();
    switch(capacity){
    default:
  ...
  private AbstractCharArrSeq(int size,char[] arr){
    super(size);
    this.arr=arr;
  }
  public boolean contains(boolean val){
    final int size;
    return (size=this.size)!=0&&uncheckedContains(arr,0,size,TypeUtil.castToChar(val));
  }
  ...
  public boolean contains(double val){
    final int size,v;
    return (size=this.size)!=0&&val==(v=(char)val)&&uncheckedContains(arr,0,size,v);
  }
  ...
  public boolean contains(int val){
    final int size;
    return (size=this.size)!=0&&val==(char)val&&uncheckedContains(arr,0,size,val);
  }
  ...
  public boolean contains(Object val){
    final int size;
    return (size=this.size)!=0&&val instanceof Character&&uncheckedContains(arr,0,size,(char)val);
  }
  @Override public boolean contains(short val){
    final int size;
    return val>=0&&(size=this.size)!=0&&uncheckedContains(arr,0,size,val);
  }
  ...
  public int indexOf(boolean val){
    final int size;
    if((size=this.size)!=0){ return uncheckedIndexOf(size,TypeUtil.castToChar(val)); }
    return -1;
  }
  ...
  public int indexOf(double val){
    final int size,v;
    if((size=this.size)!=0&&val==(v=(char)val)){ return uncheckedIndexOf(size,v); }
    return -1;
  }
  ...
  public int indexOf(int val){
    final int size;
    if((size=this.size)!=0&&val==(char)val){ return uncheckedIndexOf(size,val); }
    return -1;
  }
  ...
  public int indexOf(Object val){
    final int size;
    if((size=this.size)!=0&&val instanceof Character){ return uncheckedIndexOf(size,(char)val); }
    return -1;
  }
  public int indexOf(short val){
    final int size;
    if(val>=0&&(size=this.size)!=0){ return uncheckedIndexOf(size,val); }
    return -1;
  }
  ...
  static abstract class Checked extends AbstractCharArrSeq{
    ...
    static abstract class AbstractBidirectionalItr{
      ...
      public void add(char val){
        ...
        if((rootSize=root.size)!=0){
          ((AbstractCharArrSeq)root).uncheckedInsert(cursor,val,rootSize);
        }else{
          ((AbstractCharArrSeq)root).uncheckedInit(val);
        }
    ...
    static abstract class AbstractSubList extends AbstractSubArrSeq{
      ...
      public boolean add(char val){
        ...
        if((rootSize=root.size)!=0){
          ((AbstractCharArrSeq)root).uncheckedInsert(rootOffset+size,val,rootSize);
        }else{
          ((AbstractCharArrSeq)root).uncheckedInit(val);
        }
        root.modCount=++modCount;
      ...
      public void add(int index,char val){
        ...
        final int rootSize;
        if((rootSize=root.size)!=0){
          ((AbstractCharArrSeq)root).uncheckedInsert(rootOffset+index,val,rootSize);
        }else{
          ((AbstractCharArrSeq)root).uncheckedInit(val);
        }
        root.modCount=++modCount;
            ...
            if(filter.test(arr[srcOffset])){
              ...
              while(++srcOffset!=srcBound){
                ...
                if(!filter.test(v=arr[srcOffset])){
                  ...
                  this.size=size-(size=((AbstractCharArrSeq)root).finalizeSubListBatchRemove(arr,dstOffset,srcBound));
                  bubbleUpDecrementSize(parent,size);
                  return true;
                }
              }
  ...
  static abstract class Unchecked extends AbstractCharArrSeq{
