...
class UncheckedSubList extends AbstractSeq.Unchecked.AbstractSubList implements OmniList.OfFloat{
  ...
  @Override public boolean contains(boolean val){
    int size;
    if((size=this.size)!=0){
      if(val){ return uncheckedContainsFltBits(size,TypeUtil.FLT_TRUE_BITS); }
  ...
  @Override public boolean contains(double val){
    ...
    if((size=this.size)!=0){
      float v;
      if((v=(float)val)==val){ return uncheckedContainsFltBits(size,Float.floatToRawIntBits(v)); }
  ...
  @Override public boolean contains(int val){
    int size;
    if((size=this.size)!=0){
      if(val!=0){ return TypeUtil.checkCastToFloat(val)&&uncheckedContainsFltBits(size,Float.floatToRawIntBits(val)); }
      return uncheckedContainsFlt0(size);
    }
    return false;
  }
  ...
  @Override public int indexOf(double val){
    ...
    if((size=this.size)!=0){
      ...
      }else if(v!=v){ return uncheckedIndexOfFltNaN(size); }
    }
    return -1;
  }
  ...
  @Override public int indexOf(Object val){
    ...
    if((size=this.size)!=0&&val instanceof Float){
      final int rootOffset;
      return AbstractSeq.uncheckedIndexOfMatch(root.arr,rootOffset=this.rootOffset,rootOffset+size,(float)val);
    }
    return -1;
  }
  ...
  @Override public int lastIndexOf(double val){
    ...
    if((size=this.size)!=0){
      ...
      }else if(v!=v){ return uncheckedLastIndexOfFltNaN(size); }
    }
    return -1;
  }
  ...
  @Override public int lastIndexOf(Object val){
    ...
    if((size=this.size)!=0&&val instanceof Float){
      final int rootOffset;
      return AbstractSeq.uncheckedLastIndexOfMatch(root.arr,rootOffset=this.rootOffset,rootOffset+size,(float)val);
    }
    return -1;
  }
  ...
  @Override public boolean removeVal(double val){
    ...
    if((size=this.size)!=0){
      final float v;
      if((v=(float)val)==val){ return uncheckedRemoveFltBits(size,Float.floatToRawIntBits(v)); }
  ...
  @Override public boolean removeVal(int val){
    final int size;
    if((size=this.size)!=0){
      if(val!=0){ return TypeUtil.checkCastToFloat(val)&&uncheckedRemoveFltBits(size,Float.floatToRawIntBits(val)); }
