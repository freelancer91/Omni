...
class CheckedSubList extends AbstractSeq.Checked.AbstractSubList implements OmniList.OfInt{
    ...
    public boolean contains(Object val){
        ...
        return (size=this.size)!=0&&val instanceof Integer
                &&AbstractSeq.uncheckedContains(root.arr,rootOffset=this.rootOffset,rootOffset+size,(int)val);
    }
    @Override
    ...
    public int indexOf(Object val){
        ...
        if((size=this.size)!=0&&val instanceof Integer){
            final int rootOffset;
            return AbstractSeq.uncheckedIndexOf(root.arr,rootOffset=this.rootOffset,rootOffset+size,(int)val);
        }
        return -1;
    }
    @Override
    ...
    public int lastIndexOf(Object val){
        ...
        if((size=this.size)!=0&&val instanceof Integer){
            final int rootOffset;
            return AbstractSeq.uncheckedLastIndexOf(root.arr,rootOffset=this.rootOffset,rootOffset+size,(int)val);
        }
        return -1;
    }
    @Override
    ...
    public boolean remove(Object val){
        final int size;
        if((size=this.size)!=0&&val instanceof Integer){
            return uncheckedRemoveFirstMatch(size,(int)val);
        }
        CheckedCollection.checkModCount(modCount,root.modCount);
        return false;
    }
