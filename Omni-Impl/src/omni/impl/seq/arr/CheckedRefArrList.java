...
public class CheckedRefArrList<E>extends AbstractRefArrSeq.Checked<E> implements OmniList.OfRef<E>{
  ...
  private static class SubList<E>extends AbstractSubList<E> implements OmniList.OfRef<E>{
    ...
    @Override public E remove(int index){
      ...
      CheckedCollection.checkReadHi(index,size=this.size);
      final Object[] arr;
      @SuppressWarnings("unchecked") final var removed=(E)(arr=root.arr)[index+=rootOffset];
    ...
    @Override public boolean remove(Object val){
      final int size;
      if((size=this.size)!=0){
        if(val!=null){ return uncheckedRemoveNonNull(size,val); }
    ...
    @Override public boolean removeVal(boolean val){
      final int size;
      if((size=this.size)!=0){ return uncheckedRemoveFirstMatch(size,OmniPred.OfRef.getEqualsPred(val)); }
      CheckedCollection.checkModCount(modCount,root.modCount);
      return false;
    }
    @Override public boolean removeVal(Boolean val){
