...
abstract class AbstractIntArrSeq extends AbstractIntList{
  transient int[] arr;
  ...
  static int uncheckedIndexOf(int[] arr,int offset,int bound,int val){
    int index=offset;
    do{
  ...
  static int uncheckedLastIndexOf(int[] arr,int offset,int bound,int val){
    do{
  ...
  private AbstractIntArrSeq(){
    super();
    arr=OmniArray.OfInt.DEFAULT_ARR;
  }
  private AbstractIntArrSeq(int capacity){
    super();
    switch(capacity){
    default:
  ...
  private AbstractIntArrSeq(int size,int[] arr){
    super(size);
    this.arr=arr;
  }
  ...
  public boolean contains(double val){
    final int size,v;
    return (size=this.size)!=0&&val==(v=(int)val)&&uncheckedContains(arr,0,size,v);
  }
  public boolean contains(float val){
    final int size,v;
    return (size=this.size)!=0&&(double)val==(double)(v=(int)val)&&uncheckedContains(arr,0,size,v);
  }
  ...
  public boolean contains(Object val){
    final int size;
    return (size=this.size)!=0&&val instanceof Integer&&uncheckedContains(arr,0,size,(int)val);
  }
  ...
  public int indexOf(double val){
    final int size,v;
    if((size=this.size)!=0&&val==(v=(int)val)){ return uncheckedIndexOf(size,v); }
    return -1;
  }
  public int indexOf(float val){
    final int size,v;
    if((size=this.size)!=0&&(double)val==(double)(v=(int)val)){ return uncheckedIndexOf(size,v); }
    return -1;
  }
  ...
  public int indexOf(Object val){
    final int size;
    if((size=this.size)!=0&&val instanceof Integer){ return uncheckedIndexOf(size,(int)val); }
    return -1;
  }
  public Integer peek(){
    final int size;
  ...
  static abstract class Checked extends AbstractIntArrSeq{
    ...
    static abstract class AbstractBidirectionalItr{
      ...
      public void add(int val){
        ...
        if((rootSize=root.size)!=0){
          ((AbstractIntArrSeq)root).uncheckedInsert(cursor,val,rootSize);
        }else{
          ((AbstractIntArrSeq)root).uncheckedInit(val);
        }
    ...
    static abstract class AbstractSubList extends AbstractSubArrSeq{
      ...
      public boolean add(int val){
        ...
        if((rootSize=root.size)!=0){
          ((AbstractIntArrSeq)root).uncheckedInsert(rootOffset+size,val,rootSize);
        }else{
          ((AbstractIntArrSeq)root).uncheckedInit(val);
        }
        root.modCount=++modCount;
      ...
      public void add(int index,int val){
        ...
        final int rootSize;
        if((rootSize=root.size)!=0){
          ((AbstractIntArrSeq)root).uncheckedInsert(rootOffset+index,val,rootSize);
        }else{
          ((AbstractIntArrSeq)root).uncheckedInit(val);
        }
        root.modCount=++modCount;
            ...
            if(filter.test(arr[srcOffset])){
              ...
              while(++srcOffset!=srcBound){
                ...
                if(!filter.test(v=arr[srcOffset])){
                  ...
                  this.size=size-(size=((AbstractIntArrSeq)root).finalizeSubListBatchRemove(arr,dstOffset,srcBound));
                  bubbleUpDecrementSize(parent,size);
                  return true;
                }
              }
  ...
  static abstract class Unchecked extends AbstractIntArrSeq{
