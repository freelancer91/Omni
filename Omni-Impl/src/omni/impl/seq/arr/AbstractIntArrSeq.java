...
abstract class AbstractIntArrSeq extends AbstractIntList{
    ...
    static abstract class Checked extends AbstractIntArrSeq{
        ...
        @Override
        void uncheckedReplaceAll(int size,IntUnaryOperator operator){
            final int expectedModCount=modCount;
            try{
        ...
        @Override
        void uncheckedSort(int size,IntBinaryOperator sorter){
            final int expectedModCount=modCount;
            try{
                        ...
                        if(filter.test(arr[srcOffset])){
                            ...
                            while(++srcOffset!=srcBound){
                                ...
                                if(!filter.test(v=arr[srcOffset])){
                                    ...
                                    this.size=size-(size=((AbstractIntArrSeq)root).finalizeSubListBatchRemove(arr,
                                            dstOffset,srcBound));
                                    bubbleUpDecrementSize(parent,size);
                                    return true;
                                }
                            ...
                            root.modCount=++expectedModCount;
                            modCount=expectedModCount;
                            this.size=size-(size=((AbstractIntArrSeq)root).finalizeSubListBatchRemove(arr,dstOffset,
                                    srcBound));
                            bubbleUpDecrementSize(parent,size);
                            return true;
                        }
