...
abstract class AbstractBooleanArrSeq extends AbstractBooleanList{
  ...
  static ToStringUtil.OmniStringBuilder forwardToString(boolean[] arr,int begin,int end,
  ...
  static int uncheckedIndexOf(boolean[] arr,int offset,int bound,boolean val){
    int index=offset;
    do{
  ...
  static int uncheckedLastIndexOf(boolean[] arr,int offset,int bound,boolean val){
    do{
  ...
  private AbstractBooleanArrSeq(){
    super();
    arr=OmniArray.OfBoolean.DEFAULT_ARR;
  }
  private AbstractBooleanArrSeq(int capacity){
    super();
    switch(capacity){
    default:
  ...
  private AbstractBooleanArrSeq(int size,boolean[] arr){
    super(size);
    this.arr=arr;
  }
  ...
  public boolean contains(double val){
    ...
    if((size=this.size)!=0){
      ...
      return uncheckedContains(arr,0,size,v);
    }
    return false;
  }
  ...
  public boolean contains(Object val){
    final int size;
    return (size=this.size)!=0&&val instanceof Boolean&&uncheckedContains(arr,0,size,(boolean)val);
  }
  ...
  public int indexOf(Object val){
    final int size;
    if((size=this.size)!=0&&val instanceof Boolean){ return uncheckedIndexOf(size,(boolean)val); }
    return -1;
  }
  public Boolean peek(){
    final int size;
  ...
  // abstract ToStringUtil.OmniStringBuilder uncheckedToString(int size,ToStringUtil.OmniStringBuilder builder);
  // abstract int uncheckedToString(int size,char[] buffer,int bufferOffset);
  @Override protected int uncheckedLastIndexOf(int size,boolean val){
  ...
  static abstract class Checked extends AbstractBooleanArrSeq{
    transient int modCount;
    private static int markSurvivors(boolean[] arr,long[] survivorSet,int offset,int bound,BooleanPredicate filter){
    ...
    private static int pullSurvivorsDown(boolean[] arr,long[] survivorSet,int dstOffset,int srcOffset,int numSurvivors){
    ...
    static abstract class AbstractBidirectionalItr{
      ...
      public void add(boolean val){
        ...
        if((rootSize=root.size)!=0){
          ((AbstractBooleanArrSeq)root).uncheckedInsert(cursor,val,rootSize);
        }else{
          ((AbstractBooleanArrSeq)root).uncheckedInit(val);
        }
    ...
    static abstract class AbstractSubList extends AbstractSubArrSeq{
      ...
      public boolean add(boolean val){
        ...
        if((rootSize=root.size)!=0){
          ((AbstractBooleanArrSeq)root).uncheckedInsert(rootOffset+size,val,rootSize);
        }else{
          ((AbstractBooleanArrSeq)root).uncheckedInit(val);
        }
        root.modCount=++modCount;
      ...
      public void add(int index,boolean val){
        ...
        final int rootSize;
        if((rootSize=root.size)!=0){
          ((AbstractBooleanArrSeq)root).uncheckedInsert(rootOffset+index,val,rootSize);
        }else{
          ((AbstractBooleanArrSeq)root).uncheckedInit(val);
        }
        root.modCount=++modCount;
            ...
            if(filter.test(arr[srcOffset])){
              ...
              while(++srcOffset!=srcBound){
                ...
                if(!filter.test(v=arr[srcOffset])){
                  ...
                  this.size
                      =size-(size=((AbstractBooleanArrSeq)root).finalizeSubListBatchRemove(arr,dstOffset,srcBound));
                  bubbleUpDecrementSize(parent,size);
                  return true;
                }
              }
              ...
              root.modCount=++expectedModCount;
              modCount=expectedModCount;
              this.size=size-(size=((AbstractBooleanArrSeq)root).finalizeSubListBatchRemove(arr,dstOffset,srcBound));
              bubbleUpDecrementSize(parent,size);
              return true;
            }
          }while(++srcOffset!=srcBound);
  ...
  static abstract class Unchecked extends AbstractBooleanArrSeq{
    ...
    static abstract class AbstractBidirectionalItr{
      ...
      public void add(boolean val){
        final AbstractBooleanArrSeq root;
        final int rootSize,cursor=this.cursor;
