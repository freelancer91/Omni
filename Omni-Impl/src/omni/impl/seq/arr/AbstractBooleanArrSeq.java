...
abstract class AbstractBooleanArrSeq extends AbstractBooleanList{
    ...
    static abstract class Checked extends AbstractBooleanArrSeq{
        ...
        private static int pullSurvivorsDown(boolean[] arr,long[] survivorSet,int dstOffset,int srcOffset,
                int numSurvivors){
        ...
        @Override
        void uncheckedReplaceAll(int size,BooleanPredicate operator){
            final int expectedModCount=modCount;
            try{
        ...
        @Override
        void uncheckedSort(int size,BooleanComparator sorter){
            final int expectedModCount=modCount;
            try{
            ...
            public String toString(){
                ...
                if((size=this.size)!=0){
                    ...
                    if(size>OmniArray.MAX_ARR_SIZE/7){
                        ...
                        return forwardToString(arr,0,size-1,new ToStringUtil.OmniStringBuilder(1,buffer))
                                .uncheckedAppend(']').toString();
                    }else{
                        ...
                        if(filter.test(arr[srcOffset])){
                            ...
                            while(++srcOffset!=srcBound){
                                ...
                                if(!filter.test(v=arr[srcOffset])){
                                    ...
                                    this.size=size-(size=((AbstractBooleanArrSeq)root).finalizeSubListBatchRemove(arr,
                                            dstOffset,srcBound));
                                    bubbleUpDecrementSize(parent,size);
                                    return true;
                                }
                            ...
                            root.modCount=++expectedModCount;
                            modCount=expectedModCount;
                            this.size=size-(size=((AbstractBooleanArrSeq)root).finalizeSubListBatchRemove(arr,dstOffset,
                                    srcBound));
                            bubbleUpDecrementSize(parent,size);
                            return true;
                        }
