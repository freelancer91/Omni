...
abstract class AbstractByteArrSeq extends AbstractByteList{
    ...
    static abstract class Checked extends AbstractByteArrSeq{
        ...
        private static int pullSurvivorsDown(byte[] arr,long[] survivorSet,int dstOffset,int srcOffset,
                int numSurvivors){
        ...
        @Override
        void uncheckedReplaceAll(int size,ByteUnaryOperator operator){
            final int expectedModCount=modCount;
            try{
        ...
        @Override
        void uncheckedSort(int size,ByteComparator sorter){
            final int expectedModCount=modCount;
            try{
                        ...
                        if(filter.test(arr[srcOffset])){
                            ...
                            while(++srcOffset!=srcBound){
                                ...
                                if(!filter.test(v=arr[srcOffset])){
                                    ...
                                    this.size=size-(size=((AbstractByteArrSeq)root).finalizeSubListBatchRemove(arr,
                                            dstOffset,srcBound));
                                    bubbleUpDecrementSize(parent,size);
                                    return true;
                                }
                            ...
                            root.modCount=++expectedModCount;
                            modCount=expectedModCount;
                            this.size=size-(size=((AbstractByteArrSeq)root).finalizeSubListBatchRemove(arr,dstOffset,
                                    srcBound));
                            bubbleUpDecrementSize(parent,size);
                            return true;
                        }
