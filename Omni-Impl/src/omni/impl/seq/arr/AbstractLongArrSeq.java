...
abstract class AbstractLongArrSeq extends AbstractLongList{
  ...
  public int indexOf(boolean val){
    final int size;
    if((size=this.size)!=0){ return uncheckedIndexOf(size,TypeUtil.castToLong(val)); }
    return -1;
  }
  public int indexOf(double val){
    final int size;
    final long v;
    if((size=this.size)!=0&&TypeUtil.doubleEquals(val,v=(long)val)){ return uncheckedIndexOf(size,v); }
    return -1;
  }
  public int indexOf(float val){
    final int size;
    final long v;
    if((size=this.size)!=0&&TypeUtil.floatEquals(val,v=(long)val)){ return uncheckedIndexOf(size,v); }
    return -1;
  }
  ...
  public int indexOf(Object val){
    final int size;
    if((size=this.size)!=0&&val instanceof Long){ return uncheckedIndexOf(size,(long)val); }
    return -1;
  }
  public Long peek(){
    final int size;
  ...
  @Override protected int uncheckedLastIndexOf(int size,long val){
            ...
            if(filter.test(arr[srcOffset])){
              ...
              while(++srcOffset!=srcBound){
                ...
                if(!filter.test(v=arr[srcOffset])){
                  ...
                  this.size=size-(size=((AbstractLongArrSeq)root).finalizeSubListBatchRemove(arr,dstOffset,srcBound));
                  bubbleUpDecrementSize(parent,size);
                  return true;
                }
              }
