...
abstract class AbstractSeq extends AbstractDoubleList{
  ...
  static boolean uncheckedAnyMatchesDblBits(double[] arr,int offset,int bound,long dblBits){
    while(Double.doubleToRawLongBits(arr[offset])!=dblBits){
      if(++offset==bound){ return false; }
    }
    return true;
  }
  static boolean uncheckedAnyMatchesDblNaN(double[] arr,int offset,int bound){
  ...
  static int uncheckedIndexOfMatch(double[] arr,int offset,int bound,double val){
  ...
  static int uncheckedLastIndexOfMatch(double[] arr,int offset,int bound,double val){
  ...
  public boolean contains(boolean val){
    ...
    if((size=this.size)!=0){
      ...
      if(val){ return uncheckedAnyMatchesDblBits(arr,0,size,TypeUtil.DBL_TRUE_BITS); }
  ...
  public boolean contains(float val){
    ...
    if((size=this.size)!=0){
      final var arr=this.arr;
      if(val==val){ return uncheckedAnyMatchesDblBits(arr,0,size,Double.doubleToRawLongBits(val)); }
      return uncheckedAnyMatchesDblNaN(arr,0,size);
    }
    return false;
  }
  ...
  public boolean contains(long val){
    ...
    if((size=this.size)!=0){
      if(val!=0){ return TypeUtil.checkCastToDouble(val)
          &&uncheckedAnyMatchesDblBits(arr,0,size,Double.doubleToRawLongBits(val)); }
      return uncheckedAnyMatchesDbl0(arr,0,size);
    }
    return false;
  }
  ...
  public int indexOf(Object val){
    final int size;
    if((size=this.size)!=0&&val instanceof Double){ return uncheckedIndexOfMatch(size,(double)val); }
    return -1;
  }
  public Double peek(){
    final int size;
  ...
  private int uncheckedIndexOfMatch(int size,double val){
    if(val==val){ return uncheckedIndexOfDblBits(size,Double.doubleToRawLongBits(val)); }
    return uncheckedIndexOfDblNaN(size);
  }
