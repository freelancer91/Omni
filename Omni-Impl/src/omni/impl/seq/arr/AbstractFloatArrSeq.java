...
abstract class AbstractFloatArrSeq extends AbstractFloatList{
  ...
  static boolean uncheckedContains(float[] arr,int offset,int bound,float val){
    if(val==val){ return uncheckedContainsFltBits(arr,offset,bound,Float.floatToRawIntBits(val)); }
  ...
  static boolean uncheckedContainsRawInt(float[] arr,int offset,int bound,int val){
    if(val!=0){ return uncheckedContainsFltBits(arr,offset,bound,Float.floatToRawIntBits(val)); }
  ...
  public boolean contains(boolean val){
    ...
    if((size=this.size)!=0){
      ...
      if(val){ return uncheckedContainsFltBits(arr,0,size,TypeUtil.FLT_TRUE_BITS); }
  ...
  public boolean contains(double val){
    ...
    if((size=this.size)!=0){
      final float v;
      if((v=(float)val)==val){ return uncheckedContainsFltBits(arr,0,size,Float.floatToRawIntBits(v)); }
  ...
  public int indexOf(Object val){
    final int size;
    if((size=this.size)!=0&&val instanceof Float){ return uncheckedIndexOf(size,(float)val); }
    return -1;
  }
            ...
            if(filter.test(arr[srcOffset])){
              ...
              while(++srcOffset!=srcBound){
                ...
                if(!filter.test(v=arr[srcOffset])){
                  ...
                  this.size=size-(size=((AbstractFloatArrSeq)root).finalizeSubListBatchRemove(arr,dstOffset,srcBound));
                  bubbleUpDecrementSize(parent,size);
                  return true;
                }
              }
