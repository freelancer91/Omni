...
public class CheckedCharArrList extends AbstractCharArrSeq.Checked implements OmniList.OfChar{
  CheckedCharArrList(){
    super();
  }
  CheckedCharArrList(int capacity){
    super(capacity);
  }
  private CheckedCharArrList(int size,char[] arr){
    super(size,arr);
  }
  ...
  @Override public Object clone(){
    ...
    return new CheckedCharArrList(size,arr);
  }
  ...
  private static class SubList extends AbstractSubList implements OmniList.OfChar{
    ...
    @Override public boolean contains(byte val){
      final var root=checkModCountAndGetRoot();
      final int size,rootOffset;
      return val>=0&&(size=this.size)!=0&&uncheckedContains(root.arr,rootOffset=this.rootOffset,rootOffset+size,val);
    }
    ...
    @Override public boolean remove(Object val){
      final int size;
      if((size=this.size)!=0&&val instanceof Character){ return uncheckedRemoveFirstMatch(size,(char)val); }
      CheckedCollection.checkModCount(modCount,root.modCount);
      return false;
    }
    ...
    @Override public boolean removeVal(byte val){
      final int size;
      if(val>=0&&(size=this.size)!=0){ return uncheckedRemoveFirstMatch(size,val); }
      CheckedCollection.checkModCount(modCount,root.modCount);
      return false;
    }
    ...
    @Override public boolean removeVal(double val){
      final int size,v;
      if((size=this.size)!=0&&val==(v=(char)val)){ return uncheckedRemoveFirstMatch(size,v); }
      CheckedCollection.checkModCount(modCount,root.modCount);
      return false;
    }
    ...
    @Override public boolean removeVal(int val){
      final int size;
      if((size=this.size)!=0&&val==(char)val){ return uncheckedRemoveFirstMatch(size,val); }
      CheckedCollection.checkModCount(modCount,root.modCount);
      return false;
    }
    ...
    @Override public void sort(){
      ...
      if((size=this.size)>1){
        final int rootOffset;
        AbstractCharArrSeq.uncheckedSort(root.arr,rootOffset=this.rootOffset,size+rootOffset-1);
        root.modCount=++modCount;
        this.modCount=modCount;
        bubbleUpIncrementModCount(parent);
      }
    ...
    private void uncheckedReplaceAll(int size,CharUnaryOperator operator){
      ...
      try{
        final int rootOffset;
        AbstractCharArrSeq.uncheckedReplaceAll(root.arr,rootOffset=this.rootOffset,rootOffset+size,operator);
    ...
    private void uncheckedSort(int size,CharComparator sorter){
      ...
      try{
        final int rootOffset;
        AbstractCharArrSeq.uncheckedSort(root.arr,rootOffset=this.rootOffset,rootOffset+size-1,sorter);
      }catch(final RuntimeException e){
        throw CheckedCollection.checkModCount(modCount,root.modCount,e);
      }
      CheckedCollection.checkModCount(modCount,root.modCount);
