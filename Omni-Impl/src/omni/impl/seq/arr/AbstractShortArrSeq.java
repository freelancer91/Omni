...
abstract class AbstractShortArrSeq extends AbstractShortList{
  transient short[] arr;
  ...
  static int uncheckedIndexOf(short[] arr,int offset,int bound,int val){
    int index=offset;
    do{
  ...
  static int uncheckedLastIndexOf(short[] arr,int offset,int bound,int val){
    do{
  ...
  private AbstractShortArrSeq(){
    super();
    arr=OmniArray.OfShort.DEFAULT_ARR;
  }
  private AbstractShortArrSeq(int capacity){
    super();
    switch(capacity){
    default:
  ...
  private AbstractShortArrSeq(int size,short[] arr){
    super(size);
    this.arr=arr;
  }
  ...
  public boolean contains(char val){
    final int size;
    return val<=Short.MAX_VALUE&&(size=this.size)!=0&&uncheckedContains(arr,0,size,val);
  }
  public boolean contains(double val){
    final int size,v;
    return (size=this.size)!=0&&val==(v=(short)val)&&uncheckedContains(arr,0,size,v);
  }
  ...
  public boolean contains(int val){
    final int size;
    return (size=this.size)!=0&&val==(short)val&&uncheckedContains(arr,0,size,val);
  }
  ...
  public boolean contains(Object val){
    final int size;
    return (size=this.size)!=0&&val instanceof Short&&uncheckedContains(arr,0,size,(short)val);
  }
  ...
  public int indexOf(char val){
    final int size;
    if(val<=Short.MAX_VALUE&&(size=this.size)!=0){ return uncheckedIndexOf(size,val); }
    return -1;
  }
  public int indexOf(double val){
    final int size,v;
    if((size=this.size)!=0&&val==(v=(short)val)){ return uncheckedIndexOf(size,v); }
    return -1;
  }
  ...
  public int indexOf(int val){
    final int size;
    if((size=this.size)!=0&&val==(short)val){ return uncheckedIndexOf(size,val); }
    return -1;
  }
  ...
  public int indexOf(Object val){
    final int size;
    if((size=this.size)!=0&&val instanceof Short){ return uncheckedIndexOf(size,(short)val); }
    return -1;
  }
  ...
  static abstract class Checked extends AbstractShortArrSeq{
    ...
    static abstract class AbstractBidirectionalItr{
      ...
      public void add(short val){
        ...
        if((rootSize=root.size)!=0){
          ((AbstractShortArrSeq)root).uncheckedInsert(cursor,val,rootSize);
        }else{
          ((AbstractShortArrSeq)root).uncheckedInit(val);
        }
    ...
    static abstract class AbstractSubList extends AbstractSubArrSeq{
      ...
      public void add(int index,short val){
        ...
        final int rootSize;
        if((rootSize=root.size)!=0){
          ((AbstractShortArrSeq)root).uncheckedInsert(rootOffset+index,val,rootSize);
        }else{
          ((AbstractShortArrSeq)root).uncheckedInit(val);
        }
        root.modCount=++modCount;
      ...
      public boolean add(short val){
        ...
        if((rootSize=root.size)!=0){
          ((AbstractShortArrSeq)root).uncheckedInsert(rootOffset+size,val,rootSize);
        }else{
          ((AbstractShortArrSeq)root).uncheckedInit(val);
        }
        root.modCount=++modCount;
            ...
            if(filter.test(arr[srcOffset])){
              ...
              while(++srcOffset!=srcBound){
                ...
                if(!filter.test(v=arr[srcOffset])){
                  ...
                  this.size=size-(size=((AbstractShortArrSeq)root).finalizeSubListBatchRemove(arr,dstOffset,srcBound));
                  bubbleUpDecrementSize(parent,size);
                  return true;
                }
              }
  ...
  static abstract class Unchecked extends AbstractShortArrSeq{
    ...
    static abstract class AbstractBidirectionalItr{
      ...
      public void add(short val){
        final AbstractShortArrSeq root;
        final int rootSize,cursor=this.cursor;
