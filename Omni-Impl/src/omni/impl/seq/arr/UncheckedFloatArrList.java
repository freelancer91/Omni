...
public class UncheckedFloatArrList extends AbstractFloatArrSeq.Unchecked implements OmniList.OfFloat{
  ...
  private static class SubList extends AbstractSubList implements OmniList.OfFloat{
    ...
    @Override public boolean contains(byte val){
      ...
      return (size=this.size)!=0&&uncheckedContainsRawInt(root.arr,rootOffset=this.rootOffset,rootOffset+size,val);
    }
    ...
    @Override public boolean contains(int val){
      ...
      if((size=this.size)!=0){
        if(val!=0){ return TypeUtil.checkCastToFloat(val)
            &&uncheckedContainsFltBits(size,Float.floatToRawIntBits(val)); }
        return uncheckedContainsFlt0(size);
      }
      return false;
    }
