...
public abstract class AbstractShortList extends AbstractSeq.Of16BitPrimitive{
  ...
  public int lastIndexOf(char val){
    final int size;
    if(val<=Short.MAX_VALUE&&(size=this.size)!=0){ return uncheckedLastIndexOfMatch(size,val); }
    return -1;
  }
  public int lastIndexOf(double val){
    final int size,v;
    if((size=this.size)!=0&&val==(v=(short)val)){ return uncheckedLastIndexOfMatch(size,v); }
    return -1;
  }
  ...
  public int lastIndexOf(int val){
    final int size;
    if((size=this.size)!=0&&val==(short)val){ return uncheckedLastIndexOfMatch(size,val); }
    return -1;
  }
  ...
  public int lastIndexOf(Object val){
    final int size;
    if((size=this.size)!=0&&val instanceof Short){ return uncheckedLastIndexOfMatch(size,(short)val); }
    return -1;
  }
