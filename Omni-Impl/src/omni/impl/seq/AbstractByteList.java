...
public abstract class AbstractByteList extends AbstractSeq{
  ...
  public int lastIndexOf(char val){
    final int size;
    if(val<=Byte.MAX_VALUE&&(size=this.size)!=0){ return uncheckedLastIndexOf(size,val); }
    return -1;
  }
  public int lastIndexOf(double val){
    final int size,v;
    if((size=this.size)!=0&&val==(v=(byte)val)){ return uncheckedLastIndexOf(size,v); }
    return -1;
  }
  ...
  public int lastIndexOf(int val){
    final int size;
    if((size=this.size)!=0&&val==(byte)val){ return uncheckedLastIndexOf(size,val); }
    return -1;
  }
  ...
  public int lastIndexOf(Object val){
    final int size;
    if((size=this.size)!=0&&val instanceof Byte){ return uncheckedLastIndexOf(size,(byte)val); }
    return -1;
  }
