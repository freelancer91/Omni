...
class UncheckedSubList extends AbstractSeq.Unchecked{
  ...
  @Override public boolean add(boolean val){
    ...
    final var v=TypeUtil.castToChar(val);
    if(++size!=1){
      appendHelper(v);
    }else{
      initHelper(root,v);
  ...
  @Override final boolean collapseBody(Node head,Node tail,CharPredicate filter){
    Node prev;
  ...
  private void rootCollapseHead(Node headCandidate,Node tail,CharPredicate filter){
    int numRemoved;
  ...
  private void suffixCollapseHead(Node oldHead,Node oldTail,CharPredicate filter){
  ...
  static class Body extends UncheckedSubList.Prefix{
    ...
    @Override void findNewHead(Node curr,CharPredicate filter){
      final Node tail;
    ...
    @Override void initHelper(Unchecked root,char val){
      final Node newNode;
    ...
    @Override void prependHelper(char val){
  ...
  static class Prefix extends UncheckedSubList{
    ...
    @Override void appendHelper(char val){
    ...
    @Override void collapseTail(Node oldHead,Node oldTail,CharPredicate filter){
      final int oldSize=size;
      int numConsumed=2;
      final var root=this.root;
    ...
    private void collapseHeadAndTail(Node headCandidate,Node tail,CharPredicate filter){
      final int oldSize=size;
      int numConsumed=2;
      final var root=this.root;
      for(;;){
  ...
  static class Suffix extends UncheckedSubList{
    ...
    private void collapseHeadAndTail(Node head,Node tailCandidate,CharPredicate filter){
      final int oldSize=size;
      int numConsumed=2;
      final var root=this.root;
      for(;;){
  ...
  private static class BidirectionalItr extends UncheckedAscendingSubItr implements OmniListIterator.OfChar{
