...
abstract class AbstractSeq extends AbstractBooleanList implements OmniList.OfBoolean{
    static int collapseBodyHelper(Node prev,Node next,boolean retainThis){
        int numRemoved=0;
    ...
    private void collapseHead(Node headCandidate,Node tail,boolean removeThis){
        int numRemoved;
        for(numRemoved=1;headCandidate!=tail;++numRemoved,headCandidate=headCandidate.next){
            if(headCandidate.val^removeThis){
                numRemoved+=collapseBodyHelper(headCandidate,tail,!removeThis);
                break;
            }
        }
    ...
    static abstract class Checked extends AbstractSeq implements OmniDeque.OfBoolean{
                    ...
                    if(!filter.test(!removeThis)){
                        ...
                        while((tail=tail.prev)!=head){
                            if(tail.val^removeThis){
                                numRemoved+=collapseBodyHelper(head,tail,!removeThis);
                                break;
                            }
                            ++numRemoved;
                        }
        ...
        private boolean collapseBody(int modCount,Node head,Node tail,boolean retainThis,BooleanPredicate filter){
            Node prev;
                ...
                if(filter.test(v=head.val)){
                    if(head!=tail){
                        if(v==tail.val){
                            ...
                            CheckedCollection.checkModCount(modCount,this.modCount);
                            this.modCount=modCount+1;
                            super.collapseHead(head.next,tail,v);
                        }
                    }else{
                        CheckedCollection.checkModCount(modCount,this.modCount);
                        this.modCount=modCount+1;
                    ...
                    if(head!=tail){
                        if(v==tail.val){
                            return collapseBody(modCount,head,tail,v,filter);
                        }else if(filter.test(!v)){
                            CheckedCollection.checkModCount(modCount,this.modCount);
                            this.modCount=modCount+1;
        ...
        void findNewHead(Node head,boolean removeThis,BooleanPredicate filter){
            ...
            if((tail=this.tail)!=head){
                if(removeThis==tail.val){
                    ...
                    super.collapseHead(head.next,tail,removeThis);
                }
            }else{
                super.clear();
            }
