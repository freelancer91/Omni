...
class UncheckedSubList extends AbstractSeq.Unchecked{
  ...
  @Override public boolean add(boolean val){
    ...
    final var v=TypeUtil.castToLong(val);
    if(++size!=1){
      appendHelper(v);
    }else{
      initHelper(root,v);
  ...
  @Override final boolean collapseBody(Node head,Node tail,LongPredicate filter){
    Node prev;
  ...
  boolean uncheckedRemoveFirstMatchHelper(Node curr,long val){
  ...
  private void rootCollapseHead(Node headCandidate,Node tail,LongPredicate filter){
    int numRemoved;
  ...
  private void suffixCollapseHead(Node oldHead,Node oldTail,LongPredicate filter){
  ...
  static class Body extends UncheckedSubList.Prefix{
    ...
    @Override void findNewHead(Node curr,LongPredicate filter){
      final Node tail;
    ...
    @Override void initHelper(Unchecked root,long val){
      final Node newNode;
    ...
    @Override void prependHelper(long val){
  ...
  static class Prefix extends UncheckedSubList{
    ...
    @Override void appendHelper(long val){
    ...
    @Override void collapseTail(Node oldHead,Node oldTail,LongPredicate filter){
      final int oldSize=size;
      int numConsumed=2;
      final var root=this.root;
    ...
    @Override boolean uncheckedRemoveFirstMatchHelper(Node curr,long val){
      final var tail=this.tail;
      Node prev;
      do{
        if(curr==tail){ return false; }
    ...
    private void collapseHeadAndTail(Node headCandidate,Node tail,LongPredicate filter){
      final int oldSize=size;
      int numConsumed=2;
      final var root=this.root;
      for(;;){
  ...
  static class Suffix extends UncheckedSubList{
    ...
    private void collapseHeadAndTail(Node head,Node tailCandidate,LongPredicate filter){
      final int oldSize=size;
      int numConsumed=2;
      final var root=this.root;
      for(;;){
  ...
  private static class BidirectionalItr extends UncheckedAscendingSubItr implements OmniListIterator.OfLong{
