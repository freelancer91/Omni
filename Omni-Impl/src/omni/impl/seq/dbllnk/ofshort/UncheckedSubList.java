...
class UncheckedSubList extends AbstractSeq.Unchecked{
  ...
  @Override public boolean add(boolean val){
    ...
    final var v=TypeUtil.castToByte(val);
    if(++size!=1){
      appendHelper(v);
    }else{
      initHelper(root,v);
  ...
  @Override final boolean collapseBody(Node head,Node tail,ShortPredicate filter){
    Node prev;
  ...
  boolean uncheckedRemoveFirstMatchHelper(Node curr,int val){
  ...
  private void rootCollapseHead(Node headCandidate,Node tail,ShortPredicate filter){
    int numRemoved;
  ...
  private void suffixCollapseHead(Node oldHead,Node oldTail,ShortPredicate filter){
  ...
  static class Body extends UncheckedSubList.Prefix{
    ...
    @Override void findNewHead(Node curr,ShortPredicate filter){
      final Node tail;
    ...
    @Override void initHelper(Unchecked root,short val){
      final Node newNode;
    ...
    @Override void prependHelper(short val){
  ...
  static class Prefix extends UncheckedSubList{
    ...
    @Override void appendHelper(short val){
    ...
    @Override void collapseTail(Node oldHead,Node oldTail,ShortPredicate filter){
      final int oldSize=size;
      int numConsumed=2;
      final var root=this.root;
    ...
    @Override boolean uncheckedRemoveFirstMatchHelper(Node curr,int val){
      final var tail=this.tail;
      Node prev;
      do{
        if(curr==tail){ return false; }
    ...
    private void collapseHeadAndTail(Node headCandidate,Node tail,ShortPredicate filter){
      final int oldSize=size;
      int numConsumed=2;
      final var root=this.root;
      for(;;){
  ...
  static class Suffix extends UncheckedSubList{
    ...
    private void collapseHeadAndTail(Node head,Node tailCandidate,ShortPredicate filter){
      final int oldSize=size;
      int numConsumed=2;
      final var root=this.root;
      for(;;){
  ...
  private static class BidirectionalItr extends UncheckedAscendingSubItr implements OmniListIterator.OfShort{
