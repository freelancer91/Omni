...
abstract class AbstractSeq<E>extends AbstractRefList<E> implements OmniList.OfRef<E>{
  ...
  @Override public Object clone(){
    ...
    if((size=this.size)!=0){
      ...
      for(newHead=new Node<>((oldHead=this.head).val),newTail=newHead,oldTail=this.tail;oldHead!=oldTail;
  ...
  @Override public int indexOf(boolean val){
    Node<E> head;
    if((head=this.head)!=null){ return uncheckedIndexOfMatch(head,OmniPred.OfRef.getEqualsPred(val)); }
    return -1;
  }
  @Override public int indexOf(Boolean val){
    Node<E> head;
  ...
  Node<E> getItrNode(int headDist,int tailDist){
