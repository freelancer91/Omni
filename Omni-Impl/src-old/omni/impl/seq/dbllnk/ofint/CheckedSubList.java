...
public class CheckedSubList extends AbstractSeq{
  ...
  @Override public boolean remove(Object val){
    Node head;
    if((head=this.head)!=null&&val instanceof Integer){ return uncheckedRemoveFirstMatch(head,(int)val); }
    CheckedCollection.checkModCount(modCount,root.modCount);
    return false;
  }
  ...
  @Override public boolean removeVal(double val){
    Node head;
    int v;
    if((head=this.head)!=null&&val==(v=(int)val)){ return uncheckedRemoveFirstMatch(head,v); }
    CheckedCollection.checkModCount(modCount,root.modCount);
    return false;
  }
  @Override public boolean removeVal(float val){
    Node head;
    int v;
    if((head=this.head)!=null&&(double)val==(double)(v=(int)val)){ return uncheckedRemoveFirstMatch(head,v); }
    CheckedCollection.checkModCount(modCount,root.modCount);
    return false;
  }
  ...
  void appendHelper(int val,int modCount){
    this.modCount=modCount;
    Node newNode;
  ...
  void initHelper(Checked root,int val,int modCount){
  ...
  void prependHelper(int val,int modCount){
    this.modCount=modCount;
    Node newNode;
  ...
  private void collapseHeadAndTail(int modCount,Node headCandidate,Node tailCandidate,final IntPredicate filter){
    int size=this.size;
    int numConsumed=2;
    final var root=this.root;
    for(;;){
  ...
  private void insertHelper(Node before,int val,Node after,int modCount){
  ...
  private void rootCollapseHead(int modCount,Node headCandidate,final Node oldTail,IntPredicate filter){
    final int oldSize=size;
    int numConsumed=2;
    final var root=this.root;
  ...
  private void rootCollapseTail(int modCount,final Node oldHead,Node tailCandidate,IntPredicate filter){
    final int oldSize=size;
    int numConsumed=2;
    final var root=this.root;
  ...
  private void suffixCollapseHead(int modCount,Node oldHead,Node oldTail,IntPredicate filter){
    final int oldSize=size;
    int numConsumed=2;
    final var root=this.root;
  ...
  private void uncheckedAddLast(int val){
    if(++size!=1){
      appendHelper(val,modCount);
    }else{
      initHelper(root,val,modCount);
  ...
  static class Body extends CheckedSubList.Prefix{
    ...
    @Override void findNewHead(int modCount,Node curr,IntPredicate filter){
      final Node tail;
      if(curr==(tail=this.tail)){
        Checked root;
        CheckedCollection.checkModCount(modCount,(root=this.root).modCount);
    ...
    @Override void initHelper(Checked root,int val,int modCount){
      final Node newNode;
    ...
    @Override void prependHelper(int val,int modCount){
      this.modCount=modCount;
    ...
    private OmniList.OfInt getEmptySubList(Checked root,int headDist){
      return new CheckedSubList.Body(root,this,headDist);
    }
    private OmniList.OfInt getSubList1(Checked root,int headDist,int tailDist){
      return new CheckedSubList.Body(root,this,staticGetNode(this,headDist,tailDist),headDist);
  ...
  static class Prefix extends CheckedSubList{
    ...
    @Override void appendHelper(int val,int modCount){
      this.modCount=modCount;
    ...
    @Override boolean removeIfHelper(int modCount,Node curr,IntPredicate filter){
      final Node tail;
      if(curr!=(tail=this.tail)){
        if(filter.test(tail.val)){
    ...
    private void collapseHeadAndTail(int modCount,Node headCandidate,Node tail,IntPredicate filter){
      final int oldSize=size;
      int numConsumed=2;
      final var root=this.root;
      for(;;){
    ...
    private void prefixCollapseTail(int modCount,Node oldHead,Node oldTail,IntPredicate filter){
      final int oldSize=size;
      int numConsumed=2;
      final var root=this.root;
  ...
  static class Suffix extends CheckedSubList{
    ...
    private void collapseHeadAndTail(int modCount,Node head,Node tailCandidate,IntPredicate filter){
      final int oldSize=size;
      int numConsumed=2;
      final var root=this.root;
      for(;;){
  ...
  private static class BidirectionalItr extends CheckedAscendingSubItr implements OmniListIterator.OfInt{
