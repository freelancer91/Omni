...
public class BooleanArrDeqTest{
  ...
  public void testpeek_void(){
    runOutputTests(false,true,BooleanArrDeqTest::testpeek_voidHelper);
  }
  private static void testpeek_voidHelper(CheckedType checkedType,int head,int initialSize,BooleanOutputTestArgType outputArgType)
  {
  ...
  public void testpeekFirst_void(){
    runOutputTests(false,true,BooleanArrDeqTest::testpeekFirst_voidHelper);
  }
  private static void testpeekFirst_voidHelper(CheckedType checkedType,int head,int initialSize,BooleanOutputTestArgType outputArgType)
  {
    SeqMonitor seqMonitor=new SeqMonitor(checkedType,initialSize,head,initialSize);
    initializeAscending(seqMonitor.seq.arr,head,initialSize);
    for(int i=0;i<initialSize;++i)
  ...
  public void testpeekLast_void(){
    runOutputTests(false,true,BooleanArrDeqTest::testpeekLast_voidHelper);
  }
  private static void testpeekLast_voidHelper(CheckedType checkedType,int head,int initialSize,BooleanOutputTestArgType outputArgType)
  {
    SeqMonitor seqMonitor=new SeqMonitor(checkedType,initialSize,head,initialSize);
    initializeAscending(seqMonitor.seq.arr,head,initialSize);
    for(int i=0;i<initialSize;++i)
    ...
    for(var checkedType:CheckedType.values()){
      for(var monitoredFunctionGen:MonitoredFunctionGen.values()){
        if(monitoredFunctionGen.expectedException==null || (checkedType.checked && monitoredFunctionGen.appliesToRoot)){
          testreadandwriteObjectHelper(checkedType,monitoredFunctionGen,0,0,0);
          for(int tmpSeqSize=1;tmpSeqSize<=10;++tmpSeqSize){
            int seqSize=tmpSeqSize;
            for(int tmpHead=0;tmpHead<seqSize;++tmpHead){
              int head=tmpHead;
              testreadandwriteObjectHelper(checkedType,monitoredFunctionGen,seqSize,head,0);
              testreadandwriteObjectHelper(checkedType,monitoredFunctionGen,seqSize,head,1);
            }
          }
        }
      }
