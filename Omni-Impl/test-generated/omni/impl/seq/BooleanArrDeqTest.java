...
public class BooleanArrDeqTest{
    ...
    for(int tail=seqLength-1;tail>=(seqLength>>2);tail-=(seqLength>>1)){
      for(var checkedType:CheckedType.values()){
        ...
        for(int batchIndex=0;batchIndex<batchBound;++batchIndex){
          ...
          TestExecutorService.submitTest(()->AbstractBooleanSeqMonitor.verifyLargeStr(string,finalWayPointIndex,finalWayPointBound,verifyItr.getOffset(0)));
        ...
        TestExecutorService.completeAllTests("BooleanArrDeq.testMASSIVEtoString_void checkedType="+checkedType+"; tail="+tail);
      }
    }
  }
  @org.junit.jupiter.api.Test
  ...
  public void testremoveIf_Predicate(){
      ...
      for(var monitoredRemoveIfPredicateGen:MonitoredRemoveIfPredicateGen.values()){
        if(monitoredRemoveIfPredicateGen.expectedException==null || (checkedType.checked && monitoredRemoveIfPredicateGen.appliesToRoot)){
          for(var functionCallType:FunctionCallType.values()){
            TestExecutorService.submitTest(()->testremoveIf_PredicateHelper(checkedType,monitoredRemoveIfPredicateGen,0.5,0,functionCallType,0,0,0,0));
                ...
                for(int tmpPeriod=1,inc=Math.max(1,seqSize/10);;tmpPeriod+=inc){
                  ...
                  for(int tmpInitVal=0;tmpInitVal<=1;++tmpInitVal){
                    ...
                    for(int tmpHead=0;tmpHead<seqSize;++tmpHead){
                      ...
                      TestExecutorService.submitTest(()->testremoveIf_PredicateHelper(checkedType,monitoredRemoveIfPredicateGen,0.5,randSeed,functionCallType,seqSize,head,initVal,period));
                    }
                  }
                  if(period>=seqSize){
                    break;
    ...
    TestExecutorService.completeAllTests("BooleanArrDeq.testremoveIf_Predicate");
  }
  ...
  ,int initVal,int period
    ...
    OmniCollection.OfBoolean clone=(OmniCollection.OfBoolean)seqMonitor.seq.clone();
    ...
    for(var checkedType:CheckedType.values()){
      for(var monitoredFunctionGen:MonitoredFunctionGen.values()){
        if(monitoredFunctionGen.expectedException==null || (checkedType.checked && monitoredFunctionGen.appliesToRoot)){
          TestExecutorService.submitTest(()->testreadandwriteObjectHelper(checkedType,monitoredFunctionGen,0,0,0));
          for(int tmpSeqSize=1;tmpSeqSize<=10;++tmpSeqSize){
            int seqSize=tmpSeqSize;
            for(int tmpHead=0;tmpHead<seqSize;++tmpHead){
              int head=tmpHead;
              TestExecutorService.submitTest(()->testreadandwriteObjectHelper(checkedType,monitoredFunctionGen,seqSize,head,0));
              TestExecutorService.submitTest(()->testreadandwriteObjectHelper(checkedType,monitoredFunctionGen,seqSize,head,1));
            }
          }
        }
      }
  ...
  ,int initVal
    ...
    if(seqSize!=0){
      ...
      int index=seqMonitor.seq.head;
      int bound=arr.length;
      for(int i=0,val=initVal;i<seqSize;++i){
        arr[index]=TypeConversionUtil.convertToboolean(val++);
        if(++index==bound){
          index=0;
        }
      }
    ...
    if(monitoredFunctionGen.expectedException==null){
      ...
      seqMonitor.verifyPreAlloc().verifyAscending(initVal,seqSize);
      BooleanArrDeq readCol=null;
      try(var ois=new ObjectInputStream(new FileInputStream(file));){
        readCol=(BooleanArrDeq)ois.readObject();
      }catch(Exception e){
        Assertions.fail(e);
        return;
      }
      if(checkedType.checked){
        Assertions.assertEquals(0,((BooleanArrDeq.Checked)readCol).modCount);
      }
      if(seqSize==0){
        Assertions.assertEquals(-1,readCol.tail);
        Assertions.assertEquals(0,readCol.head);
        Assertions.assertSame(OmniArray.OfBoolean.DEFAULT_ARR,readCol.arr);
      }else{
        Assertions.assertEquals(0,readCol.head);
        Assertions.assertEquals(seqSize-1,readCol.tail);
        var arr=readCol.arr;
        for(int i=0;i<seqSize;++i,++initVal){
          Assertions.assertEquals(TypeConversionUtil.convertToboolean(initVal),arr[i]);
        }
      }
    }else{
            ...
            for(int tmpSeqSize=0;tmpSeqSize<=10;++tmpSeqSize){
              ...
              for(int tmpHead=0;tmpHead<tmpSeqSize;++tmpHead){
                ...
                TestExecutorService.submitTest(()->{
                  ...
                  itrMonitor.verifyIteratorState();
                  seqMonitor.verifyStructuralIntegrity();
                  Assertions.assertEquals(FieldAndMethodAccessor.BooleanArrDeq.AbstractDeqItr.cursor(itr),FieldAndMethodAccessor.BooleanArrDeq.AbstractDeqItr.cursor(itrClone));
                  switch(itrType){
                    case Itr:
                      if(checkedType.checked){
                        Assertions.assertSame(FieldAndMethodAccessor.BooleanArrDeq.Checked.AscendingItr.root(itr),FieldAndMethodAccessor.BooleanArrDeq.Checked.AscendingItr.root(itrClone));
                        Assertions.assertEquals(FieldAndMethodAccessor.BooleanArrDeq.Checked.AscendingItr.lastRet(itr),FieldAndMethodAccessor.BooleanArrDeq.Checked.AscendingItr.lastRet(itrClone));                        
                        Assertions.assertEquals(FieldAndMethodAccessor.BooleanArrDeq.Checked.AscendingItr.modCount(itr),FieldAndMethodAccessor.BooleanArrDeq.Checked.AscendingItr.modCount(itrClone));                        
                      }else{
                        Assertions.assertSame(FieldAndMethodAccessor.BooleanArrDeq.AscendingItr.root(itr),FieldAndMethodAccessor.BooleanArrDeq.AscendingItr.root(itrClone));
                      }
                      break;
                    case DescendingItr:
                      if(checkedType.checked){
                        Assertions.assertSame(FieldAndMethodAccessor.BooleanArrDeq.Checked.DescendingItr.root(itr),FieldAndMethodAccessor.BooleanArrDeq.Checked.DescendingItr.root(itrClone));
                        Assertions.assertEquals(FieldAndMethodAccessor.BooleanArrDeq.Checked.DescendingItr.lastRet(itr),FieldAndMethodAccessor.BooleanArrDeq.Checked.DescendingItr.lastRet(itrClone));                        
                        Assertions.assertEquals(FieldAndMethodAccessor.BooleanArrDeq.Checked.DescendingItr.modCount(itr),FieldAndMethodAccessor.BooleanArrDeq.Checked.DescendingItr.modCount(itrClone));                        
                      }else{
                        Assertions.assertSame(FieldAndMethodAccessor.BooleanArrDeq.DescendingItr.root(itr),FieldAndMethodAccessor.BooleanArrDeq.DescendingItr.root(itrClone));
                      }
                      break;
                    default:
  ...
  public void testConstructor_void(){
    for(var checkedType:CheckedType.values()){
      TestExecutorService.submitTest(()->{
        BooleanArrDeq deq;
        if(checkedType.checked){
          Assertions.assertEquals(0,((BooleanArrDeq.Checked)(deq=new BooleanArrDeq.Checked())).modCount);
        }else{
          deq=new BooleanArrDeq();
        }
    ...
    for(var checkedType:CheckedType.values()){
      for(int tmpInitialCapacity=0;tmpInitialCapacity<=15;tmpInitialCapacity+=5){
        ...
        TestExecutorService.submitTest(()->{
          BooleanArrDeq deq;
          if(checkedType.checked){
            Assertions.assertEquals(0,((BooleanArrDeq.Checked)(deq=new BooleanArrDeq.Checked(initialCapacity))).modCount);
          }else{
            deq=new BooleanArrDeq(initialCapacity);
          }
          Assertions.assertEquals(-1,deq.tail);
          Assertions.assertEquals(0,deq.head);
  ...
  interface OutputTest{
    void runTest(CheckedType checkedType,int head,int initialSize,BooleanOutputTestArgType outputArgType);
