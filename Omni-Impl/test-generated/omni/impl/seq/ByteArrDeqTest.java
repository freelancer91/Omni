...
public class ByteArrDeqTest{
    ...
    for(int tail=seqLength-1;tail>=(seqLength>>2);tail-=(seqLength>>1)){
      for(var checkedType:CheckedType.values()){
        ...
        for(int batchIndex=0;batchIndex<batchBound;++batchIndex){
          ...
          TestExecutorService.submitTest(()->AbstractByteSeqMonitor.verifyLargeStr(string,finalWayPointIndex,finalWayPointBound,verifyItr.getOffset(0)));
        ...
        TestExecutorService.completeAllTests("ByteArrDeq.testMASSIVEtoString_void checkedType="+checkedType+"; tail="+tail);
      }
    }
  }
  ...
  public void testremoveIf_Predicate(){
    ...
    TestExecutorService.completeAllTests("ByteArrDeq.testremoveIf_Predicate");
  }
  private static void testremoveIf_PredicateHelper(CheckedType checkedType,MonitoredRemoveIfPredicateGen monitoredRemoveIfPredicateGen,double threshold,long randSeed,final FunctionCallType functionCallType,int seqSize,int head
    ...
    OmniCollection.OfByte clone=(OmniCollection.OfByte)seqMonitor.seq.clone();
            ...
            for(int tmpSeqSize=0;tmpSeqSize<=10;++tmpSeqSize){
              ...
              for(int tmpHead=0;tmpHead<tmpSeqSize;++tmpHead){
                ...
                TestExecutorService.submitTest(()->{
                  ...
                  itrMonitor.verifyIteratorState();
                  seqMonitor.verifyStructuralIntegrity();
                  Assertions.assertEquals(FieldAndMethodAccessor.ByteArrDeq.AbstractDeqItr.cursor(itr),FieldAndMethodAccessor.ByteArrDeq.AbstractDeqItr.cursor(itrClone));
                  switch(itrType){
                    case Itr:
                      if(checkedType.checked){
                        Assertions.assertSame(FieldAndMethodAccessor.ByteArrDeq.Checked.AscendingItr.root(itr),FieldAndMethodAccessor.ByteArrDeq.Checked.AscendingItr.root(itrClone));
                        Assertions.assertEquals(FieldAndMethodAccessor.ByteArrDeq.Checked.AscendingItr.lastRet(itr),FieldAndMethodAccessor.ByteArrDeq.Checked.AscendingItr.lastRet(itrClone));                        
                        Assertions.assertEquals(FieldAndMethodAccessor.ByteArrDeq.Checked.AscendingItr.modCount(itr),FieldAndMethodAccessor.ByteArrDeq.Checked.AscendingItr.modCount(itrClone));                        
                      }else{
                        Assertions.assertSame(FieldAndMethodAccessor.ByteArrDeq.AscendingItr.root(itr),FieldAndMethodAccessor.ByteArrDeq.AscendingItr.root(itrClone));
                      }
                      break;
                    case DescendingItr:
                      if(checkedType.checked){
                        Assertions.assertSame(FieldAndMethodAccessor.ByteArrDeq.Checked.DescendingItr.root(itr),FieldAndMethodAccessor.ByteArrDeq.Checked.DescendingItr.root(itrClone));
                        Assertions.assertEquals(FieldAndMethodAccessor.ByteArrDeq.Checked.DescendingItr.lastRet(itr),FieldAndMethodAccessor.ByteArrDeq.Checked.DescendingItr.lastRet(itrClone));                        
                        Assertions.assertEquals(FieldAndMethodAccessor.ByteArrDeq.Checked.DescendingItr.modCount(itr),FieldAndMethodAccessor.ByteArrDeq.Checked.DescendingItr.modCount(itrClone));                        
                      }else{
                        Assertions.assertSame(FieldAndMethodAccessor.ByteArrDeq.DescendingItr.root(itr),FieldAndMethodAccessor.ByteArrDeq.DescendingItr.root(itrClone));
                      }
                      break;
                    default:
  ...
  public void testConstructor_void(){
    for(var checkedType:CheckedType.values()){
      TestExecutorService.submitTest(()->{
        ByteArrDeq deq;
        if(checkedType.checked){
          Assertions.assertEquals(0,((ByteArrDeq.Checked)(deq=new ByteArrDeq.Checked())).modCount);
        }else{
          deq=new ByteArrDeq();
        }
    ...
    for(var checkedType:CheckedType.values()){
      for(int tmpInitialCapacity=0;tmpInitialCapacity<=15;tmpInitialCapacity+=5){
        ...
        TestExecutorService.submitTest(()->{
          ByteArrDeq deq;
          if(checkedType.checked){
            Assertions.assertEquals(0,((ByteArrDeq.Checked)(deq=new ByteArrDeq.Checked(initialCapacity))).modCount);
          }else{
            deq=new ByteArrDeq(initialCapacity);
          }
          Assertions.assertEquals(-1,deq.tail);
          Assertions.assertEquals(0,deq.head);
  ...
  private static void initializeAscendingRepeating(byte[] arr,int head,int size)
  {
    if(size!=0)
    {
      ...
      for(;;){
        ...
        arr[head]=TypeConversionUtil.convertTobyte(val);
        if(++head==arrLength){
          head=0;
        }
        if(++i==threshold){
      ...
      if(retVal){
        ...
        if(forward){
          ...
          for(int i=0;;){
            ...
            var currVal=TypeConversionUtil.convertTobyte(val);
