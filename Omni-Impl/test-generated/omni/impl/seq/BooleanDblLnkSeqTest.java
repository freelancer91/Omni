...
public class BooleanDblLnkSeqTest{
        ...
        for(var preModScenario:PreModScenario.values()){
          if(preModScenario.expectedException==null || (checkedType.checked && preModScenario!=PreModScenario.ModSeq && !nestedType.rootType)){
            for(int seqSize=0;seqSize<=10;++seqSize){
              ...
              Stream.of(BooleanInputTestArgType.values())
              .parallel()
                  ...
                  .forEach(period->{
                    for(int initVal=0;initVal<=1;++initVal){
                      ...
                      switch(nestedType){
                        ...
                        case LISTDEQUE:
                          testremoveIf_PredicateHelper(new SeqMonitor(nestedType,checkedType),preModScenario,monitoredRemoveIfPredicateGen,0.5,randSeed,functionCallType,finalSeqSize,finalInitVal,period);
                      }
                    }
                  });
                });
  ...
  public void testQueueremove_void(){
    getPeekPollAndPopArgs().parallel().map(Arguments::get).forEach(args->{
        testQueueremove_voidHelper((SeqMonitor)args[0],(BooleanOutputTestArgType)args[1]);
    });
  }
