...
import omni.impl.seq.CharSeqMonitor.FunctionExceptionScenario;
import omni.impl.seq.CharSeqMonitor.SequenceContentsScenario;
import omni.impl.seq.CharSeqMonitor.ListItrSetExceptionScenario;
...
public class CharArrSeqTest{
  ...
  */
  ...
  public void testListItradd_val(CharSeqMonitor seqMonitor,PreModScenario preModScenario,SequenceContentsScenario seqContentsScenario,SequenceLocation seqLocationScenario,CharInputTestArgType inputArgType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
    CharItrMonitor itrMonitor;
  ...
  static Stream<Arguments> getListItrSetArgs(){
    ...
    for(var checkedType:CheckedType.values()){
      for(var listItrSetExceptionScenario:ListItrSetExceptionScenario.values()){
        ...
        for(var inputArgType:CharInputTestArgType.values()){
          builder.add(Arguments.of(new CharSeqMonitor(StructType.ARRSEQ,NestedType.LIST,checkedType),listItrSetExceptionScenario,inputArgType));
          builder.add(Arguments.of(new CharSeqMonitor(StructType.ARRSEQ,NestedType.SUBLIST,checkedType),listItrSetExceptionScenario,inputArgType));
        }
      }
    }
    return builder.build().parallel();
  ...
  public void testListItrset_val(CharSeqMonitor seqMonitor,ListItrSetExceptionScenario listItrSetExceptionScenario,CharInputTestArgType inputArgType){
    int numToAdd=100;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
    CharItrMonitor itrMonitor=seqMonitor.getListItrMonitor();
    ...
    CharSeqMonitor.SeqMonitorItr i;
