...
public class CharArrDeqTest{
  ...
  public void testremoveIf_Predicate(){
    ...
    TestExecutorService.completeAllTests("CharArrDeq.testremoveIf_Predicate");
  }
  private static void testremoveIf_PredicateHelper(CheckedType checkedType,MonitoredRemoveIfPredicateGen monitoredRemoveIfPredicateGen,double threshold,long randSeed,final FunctionCallType functionCallType,int seqSize,int head
    ...
    OmniCollection.OfChar clone=(OmniCollection.OfChar)seqMonitor.seq.clone();
            ...
            for(int tmpSeqSize=0;tmpSeqSize<=10;++tmpSeqSize){
              ...
              for(int tmpHead=0;tmpHead<tmpSeqSize;++tmpHead){
                ...
                TestExecutorService.submitTest(()->{
                  ...
                  itrMonitor.verifyIteratorState();
                  seqMonitor.verifyStructuralIntegrity();
                  Assertions.assertEquals(FieldAndMethodAccessor.CharArrDeq.AbstractDeqItr.cursor(itr),FieldAndMethodAccessor.CharArrDeq.AbstractDeqItr.cursor(itrClone));
                  switch(itrType){
                    case Itr:
                      if(checkedType.checked){
                        Assertions.assertSame(FieldAndMethodAccessor.CharArrDeq.Checked.AscendingItr.root(itr),FieldAndMethodAccessor.CharArrDeq.Checked.AscendingItr.root(itrClone));
                        Assertions.assertEquals(FieldAndMethodAccessor.CharArrDeq.Checked.AscendingItr.lastRet(itr),FieldAndMethodAccessor.CharArrDeq.Checked.AscendingItr.lastRet(itrClone));                        
                        Assertions.assertEquals(FieldAndMethodAccessor.CharArrDeq.Checked.AscendingItr.modCount(itr),FieldAndMethodAccessor.CharArrDeq.Checked.AscendingItr.modCount(itrClone));                        
                      }else{
                        Assertions.assertSame(FieldAndMethodAccessor.CharArrDeq.AscendingItr.root(itr),FieldAndMethodAccessor.CharArrDeq.AscendingItr.root(itrClone));
                      }
                      break;
                    case DescendingItr:
                      if(checkedType.checked){
                        Assertions.assertSame(FieldAndMethodAccessor.CharArrDeq.Checked.DescendingItr.root(itr),FieldAndMethodAccessor.CharArrDeq.Checked.DescendingItr.root(itrClone));
                        Assertions.assertEquals(FieldAndMethodAccessor.CharArrDeq.Checked.DescendingItr.lastRet(itr),FieldAndMethodAccessor.CharArrDeq.Checked.DescendingItr.lastRet(itrClone));                        
                        Assertions.assertEquals(FieldAndMethodAccessor.CharArrDeq.Checked.DescendingItr.modCount(itr),FieldAndMethodAccessor.CharArrDeq.Checked.DescendingItr.modCount(itrClone));                        
                      }else{
                        Assertions.assertSame(FieldAndMethodAccessor.CharArrDeq.DescendingItr.root(itr),FieldAndMethodAccessor.CharArrDeq.DescendingItr.root(itrClone));
                      }
                      break;
                    default:
  ...
  public void testConstructor_void(){
    for(var checkedType:CheckedType.values()){
      TestExecutorService.submitTest(()->{
        CharArrDeq deq;
        if(checkedType.checked){
          Assertions.assertEquals(0,((CharArrDeq.Checked)(deq=new CharArrDeq.Checked())).modCount);
        }else{
          deq=new CharArrDeq();
        }
    ...
    for(var checkedType:CheckedType.values()){
      for(int tmpInitialCapacity=0;tmpInitialCapacity<=15;tmpInitialCapacity+=5){
        ...
        TestExecutorService.submitTest(()->{
          CharArrDeq deq;
          if(checkedType.checked){
            Assertions.assertEquals(0,((CharArrDeq.Checked)(deq=new CharArrDeq.Checked(initialCapacity))).modCount);
          }else{
            deq=new CharArrDeq(initialCapacity);
          }
          Assertions.assertEquals(-1,deq.tail);
          Assertions.assertEquals(0,deq.head);
  ...
  interface OutputTest{
    void runTest(CheckedType checkedType,int head,int initialSize,CharOutputTestArgType outputArgType);
  ...
  private static void initializeAscendingRepeating(char[] arr,int head,int size)
  {
    if(size!=0)
    {
      ...
      for(;;){
        ...
        arr[head]=TypeConversionUtil.convertTochar(val);
        if(++head==arrLength){
          head=0;
        }
        if(++i==threshold){
      ...
      if(retVal){
        ...
        if(forward){
          ...
          for(int i=0;;){
            ...
            var currVal=TypeConversionUtil.convertTochar(val);
