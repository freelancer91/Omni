...
public class LongArrSeqTest{
    ...
    ConstructionArguments(StructType structType){
      ...
      switch(structType){
        ...
        case UNCHECKEDSUBLIST:
          ...
          parentPostAlloc=5;
          rootPostAlloc=5;
          arr=new long[rootPreAlloc+rootPostAlloc+parentPreAlloc+parentPostAlloc];
      ...
      switch(structType){
        case UNCHECKEDLIST:
        case UNCHECKEDSUBLIST:
          this.root=new LongArrSeq.UncheckedList(rootPreAlloc+rootPostAlloc+parentPreAlloc+parentPostAlloc,arr);
          break;
        case CHECKEDSTACK:
        ...
        case UNCHECKEDSTACK:
          ...
          break;
        default:
          this.root=new LongArrSeq.CheckedList(rootPreAlloc+rootPostAlloc+parentPreAlloc+parentPostAlloc,arr);
      }
      switch(structType){
        case CHECKEDSUBLIST:
        case UNCHECKEDSUBLIST:
  ...
  static class InputTestMonitor{
    ...
    public void seqItrAdd(OmniListIterator.OfLong seqItr,LongInputTestArgType inputArgType,int valToAdd){
    ...
    public void seqItrPrevious(OmniListIterator.OfLong seqItr){
      seqItr.previousLong();
      expectedLastRet=--expectedCursor; 
    }
    public void seqItrNext(OmniListIterator.OfLong seqItr){
      seqItr.nextLong();
      expectedLastRet=expectedCursor++;
    }
    public void seqItrRemove(OmniListIterator.OfLong seqItr){
      seqItr.remove();
    ...
    public void rootMod(ConstructionArguments constructionArgs,LongInputTestArgType inputArgType){
      ++expectedRootSize;
      ++expectedRootModCount;
      inputArgType.callCollectionAdd(constructionArgs.root,0);
    }
    public void parentMod(ConstructionArguments constructionArgs,LongInputTestArgType inputArgType){
      ++expectedRootSize;
      ++expectedRootModCount;
      ++expectedParentSize;
      ++expectedParentModCount;
    ...
    public void seqMod(ConstructionArguments constructionArgs,LongInputTestArgType inputArgType){
      ++expectedRootSize;
      ++expectedRootModCount;
      ++expectedParentSize;
      ++expectedParentModCount;
    ...
    public void collectionAdd(ConstructionArguments constructionArgs,LongInputTestArgType inputArgType,int val){
      ++expectedRootSize;
      ++expectedRootModCount;
      ++expectedParentSize;
      ++expectedParentModCount;
    ...
    public void listAdd(ConstructionArguments constructionArgs,LongInputTestArgType inputArgType,int index,int val){
      ++expectedRootSize;
      ++expectedRootModCount;
      ++expectedParentSize;
      ++expectedParentModCount;
    ...
    public void illegalMod(ConstructionArguments constructionArgs,LongInputTestArgType inputArgType,CMEScenario modScenario){
      switch(modScenario){
        case ModRoot:
    ...
    public void verifyItrState(OmniListIterator.OfLong seqItr,ConstructionArguments constructionArgs){
  ...
  @ParameterizedTest
  @MethodSource("getArgsForNonSubListTypes")
  public void testConstructor_int_longarr_happyPath(StructType structType){
    int size=5;
  ...
  */
            ...
            for(var inputArgType:LongInputTestArgType.values()){
              ...
              for(int rootPreAlloc=0;rootPreAlloc<=5;rootPreAlloc+=5){
                for(int parentPreAlloc=0;parentPreAlloc<=5;parentPreAlloc+=5){
                  for(int parentPostAlloc=0;parentPostAlloc<=5;parentPostAlloc+=5){
                    for(int rootPostAlloc=0;rootPostAlloc<=5;rootPostAlloc+=5){
                      builder.add(Arguments.of(new LongSeqMonitor(StructType.ARRSEQ,NestedType.SUBLIST,checkedType,OmniArray.DEFAULT_ARR_SEQ_CAP,rootPreAlloc,parentPreAlloc,parentPostAlloc,rootPostAlloc),preModScenario,seqContentsScenario,seqLocation,inputArgType));
                    }
                  }
                }
              }
      ...
      for(var nestedType:NestedType.values()){
        for(var preModScenario:PreModScenario.values()){
          ...
          for(var monitoredConsumerGen:MonitoredConsumerGen.values()){
            ...
            for(var seqContentsScenario:SequenceContentsScenario.values()){
              builder.add(Arguments.of(new LongSeqMonitor(StructType.ARRSEQ,nestedType,checkedType),preModScenario,monitoredConsumerGen,seqContentsScenario,FunctionCallType.Unboxed));
              builder.add(Arguments.of(new LongSeqMonitor(StructType.ARRSEQ,nestedType,checkedType),preModScenario,monitoredConsumerGen,seqContentsScenario,FunctionCallType.Boxed));
            }
          }  
        }
      }
  ...
  public void testforEach_ConsumerArgs(LongSeqMonitor seqMonitor,PreModScenario preModScenario,MonitoredConsumerGen monitoredConsumerGen,SequenceContentsScenario seqContentsScenario,FunctionCallType functionCallType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
  ...
  public void testListadd_int_val(ListAddIntValTestScenario testScenario,LongInputTestArgType inputArgType,ConstructionArguments constructionArgs){
  ...
  public void testListput_int_val(ListPutIntValTestScenario testScenario,LongInputTestArgType inputArgType,ConstructionArguments constructionArgs){
    var testMonitor=new InputTestMonitor();
    if(testScenario.nonEmpty){for(int i=0;i<100;++i){testMonitor.collectionAdd(constructionArgs,inputArgType,i);}}
    var expectedException=testScenario.expectedException;
  ...
  public void testCollectionadd_val(CollectionAddValTestScenario testScenario,LongInputTestArgType inputArgType,ConstructionArguments constructionArgs){
    var testMonitor=new InputTestMonitor();
    if(testScenario.nonEmpty){for(int i=0;i<100;++i){testMonitor.collectionAdd(constructionArgs,inputArgType,i);}}
    var expectedException=testScenario.expectedException;
    if(expectedException==null){
  ...
  public void testclear_void(StructType structType,int numToAdd,CMEScenario modScenario){
    ...
    if(modScenario==CMEScenario.NoMod){
      ...
      offset=constructionArgs.verifyParentPostAlloc(offset);
      if(modScenario==CMEScenario.ModParent){offset=constructionArgs.verifyIndex(offset,LongInputTestArgType.ARRAY_TYPE,0);}
      offset=constructionArgs.verifyRootPostAlloc(offset);
      if(modScenario==CMEScenario.ModRoot){constructionArgs.verifyIndex(offset,LongInputTestArgType.ARRAY_TYPE,0);}
    }
  }
  @ParameterizedTest
  @MethodSource("getBasicCollectionTestArguments")
  public void testclone_void(StructType structType,int numToAdd,CMEScenario modScenario){
    ...
    if(modScenario==CMEScenario.NoMod){
      ...
      var cloneSize=((LongArrSeq)clone).size;
      Assertions.assertEquals(numToAdd,cloneSize);
      if(numToAdd==0)
      {
        Assertions.assertSame(OmniArray.OfLong.DEFAULT_ARR,cloneArr);
      }
      else
      {
  ...
  public void testsize_void(int numToAdd,StructType structType,CMEScenario preModScenario)
  {
      ...
      default:
      {
        for(int i=0;i<numToAdd;++i)
        {
          ...
          switch(structType)
          {
            case CHECKEDSTACK:
            case UNCHECKEDSTACK:
              ((OmniStack.OfLong)constructionArgs.seq).popLong();
              break;
            default:
              ((OmniList.OfLong)constructionArgs.seq).removeLongAt(numToAdd-i-1);
          }
        }
    ...
    int offset=constructionArgs.verifyPreAlloc();
    if(preModScenario!=CMEScenario.NoMod){offset=constructionArgs.verifyAscending(offset,LongInputTestArgType.ARRAY_TYPE,numToAdd);}
    offset=constructionArgs.verifyParentPostAlloc(offset);
    if(preModScenario==CMEScenario.ModParent){offset=constructionArgs.verifyIndex(offset,LongInputTestArgType.ARRAY_TYPE,0);}
    offset=constructionArgs.verifyRootPostAlloc(offset);
    if(preModScenario==CMEScenario.ModRoot){offset=constructionArgs.verifyIndex(offset,LongInputTestArgType.ARRAY_TYPE,0);}
  ...
  public void testtoArray_IntFunction(StructType structType,int seqSize,ObjModScenario arrConstructorModScenario,CMEScenario preModScenario){
    ...
    Class<Long> componentClass=Long.class;
  ...
  public void testItrforEachRemaining(StructType structType,int seqSize,CMEScenario preModScenario,ItrForEachModScenario itrForEachModScenario,ItrType itrType,FunctionCallType functionCallType)
  {
    ...
    if(itrType==ItrType.ListItr)
    {
      itr=((OmniList.OfLong)constructionArgs.seq).listIterator();
    }
    else
    {
    ...
    if(expectedException==null)
    {
      ...
      {
        itr.forEachRemaining((Consumer<? super Long>)consumer::accept);
      }
    }
    else
    {
      ...
      {
        Assertions.assertThrows(expectedException,()->itr.forEachRemaining((Consumer<? super Long>)consumer::accept));
      }
    }
