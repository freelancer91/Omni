...
public class LongArrDeqTest{
    ...
    for(int tail=seqLength-1;tail>=(seqLength>>2);tail-=(seqLength>>1)){
      for(var checkedType:CheckedType.values()){
        ...
        for(int batchIndex=0;batchIndex<batchBound;++batchIndex){
          ...
          TestExecutorService.submitTest(()->AbstractLongSeqMonitor.verifyLargeStr(string,finalWayPointIndex,finalWayPointBound,verifyItr.getOffset(0)));
        ...
        TestExecutorService.completeAllTests("LongArrDeq.testMASSIVEtoString_void checkedType="+checkedType+"; tail="+tail);
      }
    }
  }
  ...
  public void testremoveIf_Predicate(){
    ...
    TestExecutorService.completeAllTests("LongArrDeq.testremoveIf_Predicate");
  }
  private static void testremoveIf_PredicateHelper(CheckedType checkedType,MonitoredRemoveIfPredicateGen monitoredRemoveIfPredicateGen,double threshold,long randSeed,final FunctionCallType functionCallType,int seqSize,int head
    ...
    OmniCollection.OfLong clone=(OmniCollection.OfLong)seqMonitor.seq.clone();
  ...
  public void testConstructor_void(){
    for(var checkedType:CheckedType.values()){
      TestExecutorService.submitTest(()->{
        LongArrDeq deq;
        if(checkedType.checked){
          Assertions.assertEquals(0,((LongArrDeq.Checked)(deq=new LongArrDeq.Checked())).modCount);
        }else{
          deq=new LongArrDeq();
        }
    ...
    for(var checkedType:CheckedType.values()){
      for(int tmpInitialCapacity=0;tmpInitialCapacity<=15;tmpInitialCapacity+=5){
        ...
        TestExecutorService.submitTest(()->{
          LongArrDeq deq;
          if(checkedType.checked){
            Assertions.assertEquals(0,((LongArrDeq.Checked)(deq=new LongArrDeq.Checked(initialCapacity))).modCount);
          }else{
            deq=new LongArrDeq(initialCapacity);
          }
          Assertions.assertEquals(-1,deq.tail);
          Assertions.assertEquals(0,deq.head);
  ...
  interface OutputTest{
    void runTest(CheckedType checkedType,int head,int initialSize,LongOutputTestArgType outputArgType);
  ...
  private static void initializeAscendingRepeating(long[] arr,int head,int size)
  {
    if(size!=0)
    {
      ...
      for(;;){
        ...
        arr[head]=TypeConversionUtil.convertTolong(val);
        if(++head==arrLength){
          head=0;
        }
        if(++i==threshold){
      ...
      if(retVal){
        ...
        if(forward){
          ...
          for(int i=0;;){
            ...
            var currVal=TypeConversionUtil.convertTolong(val);
