...
public class IntArrSeqStackTest
{
  ...
  public void testUncheckedStackpop_void_seqIsNotEmpty(){
    ...
    for(int i=0;i<100;++i)
    {
      Assertions.assertEquals(TypeConversionUtil.convertToInteger(100-i-1),seq.pop());
      Assertions.assertEquals(100-i-1,seq.size());
    }
  }
  @Test
  ...
  public void testUncheckedStackpoll_void_seqIsNotEmpty(){
    ...
    for(int i=0;i<100;++i)
    {
      Assertions.assertEquals(TypeConversionUtil.convertToInteger(100-i-1),seq.poll());
      Assertions.assertEquals(100-i-1,seq.size());
    }
    Assertions.assertEquals(null,seq.poll());
    Assertions.assertEquals(0,seq.size());
  ...
  public void testUncheckedStackpeek_void_seqIsNotEmpty(){
    ...
    {
      seq.push(TypeConversionUtil.convertToint(i));
      Assertions.assertEquals(TypeConversionUtil.convertToInteger(i),seq.peek());
      Assertions.assertEquals(i+1,seq.size());
    }
    for(int i=0;i<100;++i)
    {
      var peeked=seq.peek();
      var polled=seq.poll();
      Assertions.assertEquals(TypeConversionUtil.convertToInteger(100-i-1),peeked);
      Assertions.assertEquals(polled,peeked);
      Assertions.assertEquals(100-i-1,seq.size());
    }
    Assertions.assertEquals(null,seq.peek());
