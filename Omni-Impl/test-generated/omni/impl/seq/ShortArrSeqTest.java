...
import omni.impl.seq.ShortSeqMonitor.ItrRemoveScenario;
import omni.impl.seq.ShortSeqMonitor.MonitoredConsumerGen;
...
public class ShortArrSeqTest{
    ...
    ConstructionArguments(StructType structType){
      ...
      switch(structType){
        ...
        case UNCHECKEDSUBLIST:
          ...
          parentPostAlloc=5;
          rootPostAlloc=5;
          arr=new short[rootPreAlloc+rootPostAlloc+parentPreAlloc+parentPostAlloc];
      ...
      switch(structType){
        case UNCHECKEDLIST:
        case UNCHECKEDSUBLIST:
          this.root=new ShortArrSeq.UncheckedList(rootPreAlloc+rootPostAlloc+parentPreAlloc+parentPostAlloc,arr);
          break;
        case CHECKEDSTACK:
        ...
        case UNCHECKEDSTACK:
          ...
          break;
        default:
          this.root=new ShortArrSeq.CheckedList(rootPreAlloc+rootPostAlloc+parentPreAlloc+parentPostAlloc,arr);
      }
      switch(structType){
        case CHECKEDSUBLIST:
        case UNCHECKEDSUBLIST:
  ...
  static class InputTestMonitor{
    ...
    public void seqItrAdd(OmniListIterator.OfShort seqItr,ShortInputTestArgType inputArgType,int valToAdd){
    ...
    public void seqItrPrevious(OmniListIterator.OfShort seqItr){
      seqItr.previousShort();
      expectedLastRet=--expectedCursor; 
    }
    public void seqItrNext(OmniListIterator.OfShort seqItr){
      seqItr.nextShort();
      expectedLastRet=expectedCursor++;
    }
    public void seqItrRemove(OmniListIterator.OfShort seqItr){
      seqItr.remove();
    ...
    public void rootMod(ConstructionArguments constructionArgs,ShortInputTestArgType inputArgType){
      ++expectedRootSize;
      ++expectedRootModCount;
      inputArgType.callCollectionAdd(constructionArgs.root,0);
    }
    public void parentMod(ConstructionArguments constructionArgs,ShortInputTestArgType inputArgType){
      ++expectedRootSize;
      ++expectedRootModCount;
      ++expectedParentSize;
      ++expectedParentModCount;
    ...
    public void seqMod(ConstructionArguments constructionArgs,ShortInputTestArgType inputArgType){
      ++expectedRootSize;
      ++expectedRootModCount;
      ++expectedParentSize;
      ++expectedParentModCount;
    ...
    public void collectionAdd(ConstructionArguments constructionArgs,ShortInputTestArgType inputArgType,int val){
      ++expectedRootSize;
      ++expectedRootModCount;
      ++expectedParentSize;
      ++expectedParentModCount;
    ...
    public void listAdd(ConstructionArguments constructionArgs,ShortInputTestArgType inputArgType,int index,int val){
      ++expectedRootSize;
      ++expectedRootModCount;
      ++expectedParentSize;
      ++expectedParentModCount;
    ...
    public void illegalMod(ConstructionArguments constructionArgs,ShortInputTestArgType inputArgType,CMEScenario modScenario){
      switch(modScenario){
        case ModRoot:
    ...
    public void verifyItrState(OmniListIterator.OfShort seqItr,ConstructionArguments constructionArgs){
  ...
  @ParameterizedTest
  @MethodSource("getArgsForNonSubListTypes")
  public void testConstructor_int_shortarr_happyPath(StructType structType){
    int size=5;
  ...
  */
          ...
          for(var seqLocation:SequenceLocation.values()){
            for(var inputArgType:ShortInputTestArgType.values()){
              if(preModScenario.appliesToRootItr){
                for(int initialCapacity=0;initialCapacity<=15;initialCapacity+=5){
                    builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,NestedType.LIST,checkedType,initialCapacity),preModScenario,seqContentsScenario,seqLocation,inputArgType));
                }
              }
              for(int rootPreAlloc=0;rootPreAlloc<=5;rootPreAlloc+=5){
                for(int parentPreAlloc=0;parentPreAlloc<=5;parentPreAlloc+=5){
                  for(int parentPostAlloc=0;parentPostAlloc<=5;parentPostAlloc+=5){
                    for(int rootPostAlloc=0;rootPostAlloc<=5;rootPostAlloc+=5){
                      builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,NestedType.SUBLIST,checkedType,OmniArray.DEFAULT_ARR_SEQ_CAP,rootPreAlloc,parentPreAlloc,parentPostAlloc,rootPostAlloc),preModScenario,seqContentsScenario,seqLocation,inputArgType));
                    }
                  }
                }
              }
  ...
  public void testListItradd_val(ShortSeqMonitor seqMonitor,PreModScenario preModScenario,SequenceContentsScenario seqContentsScenario,SequenceLocation seqLocation,ShortInputTestArgType inputArgType){
      ...
      for(var listItrSetScenario:ListItrSetScenario.values()){
        ...
        for(var inputArgType:ShortInputTestArgType.values()){
          if(listItrSetScenario.preModScenario.appliesToRootItr){
            builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,NestedType.LIST,checkedType),listItrSetScenario,inputArgType));
          }
          builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,NestedType.SUBLIST,checkedType),listItrSetScenario,inputArgType));
        }
      }
    }
    return builder.build().parallel();
  ...
  public void testListItrset_val(ShortSeqMonitor seqMonitor,ListItrSetScenario listItrSetScenario,ShortInputTestArgType inputArgType){
    int numToAdd=100;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
          ...
          for(var itrType:ItrType.values()){
            for(var nestedType:NestedType.values()){
              ...
              for(var outputType:ShortOutputTestArgType.values()){
                builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,nestedType,checkedType),itrScenario,seqContentsScenario,itrType,outputType));
              }
            }
          }
        }
  ...
  public void testItrnext_void(ShortSeqMonitor seqMonitor,IterationScenario itrScenario,SequenceContentsScenario seqContentsScenario,ItrType itrType,ShortOutputTestArgType outputType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
        ...
        for(var seqContentsScenario:SequenceContentsScenario.values()){
          ...
          for(var outputType:ShortOutputTestArgType.values()){
            if(itrScenario.preModScenario.appliesToRootItr){
              builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,NestedType.LIST,checkedType),itrScenario,seqContentsScenario,outputType));
            }
            builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,NestedType.SUBLIST,checkedType),itrScenario,seqContentsScenario,outputType));
          }
        }
      }
    }
  ...
  public void testListItrprevious_void(ShortSeqMonitor seqMonitor,IterationScenario itrScenario,SequenceContentsScenario seqContentsScenario,ShortOutputTestArgType outputType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
            ...
            for(var itrType:ItrType.values()){
              ...
              for(var nestedType:NestedType.values()){
                ...
                for(var sequenceLocation:SequenceLocation.values()){
                  ...
                  builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,nestedType,checkedType),removeScenario,preModScenario,sequenceContentsScenario,itrType,sequenceLocation));
                }
              }
            }
          }
  ...
  public void testItrremove_void(ShortSeqMonitor seqMonitor,ItrRemoveScenario removeScenario,PreModScenario preModScenario,SequenceContentsScenario seqContentsScenario,ItrType itrType,SequenceLocation seqLocation){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
  ...
  static Stream<Arguments> getListItrpreviousIndex_void_And_ListItrnextIndex_voidArgs()
  {
    ...
    for(var checkedType:CheckedType.values()){
      for(var nestedType:NestedType.values()){
        ...
        builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,nestedType,checkedType)));
      }  
    }
    return builder.build().parallel();
  }
  ...
  public void testListItrpreviousIndex_void_And_ListItrnextIndex_void(ShortSeqMonitor seqMonitor){
    int numToAdd=100;
    for(int i=0;i<numToAdd;++i)
    {
      seqMonitor.add(i);
        ...
        for(var preModScenario:PreModScenario.values()){
          ...
          for(var monitoredConsumerGen:MonitoredConsumerGen.values()){
            ...
            for(var itrType:ItrType.values()){
              ...
              for(var seqContentsScenario:SequenceContentsScenario.values()){
                builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,nestedType,checkedType),preModScenario,monitoredConsumerGen,itrType,seqContentsScenario,FunctionCallType.Unboxed));
                builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,nestedType,checkedType),preModScenario,monitoredConsumerGen,itrType,seqContentsScenario,FunctionCallType.Boxed));
              }
            }
          }  
        }
  ...
  public void testItrforEachRemaining_Consumer(ShortSeqMonitor seqMonitor,PreModScenario preModScenario,MonitoredConsumerGen monitoredConsumerGen,ItrType itrType,SequenceContentsScenario seqContentsScenario,FunctionCallType functionCallType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
      ...
      for(var nestedType:NestedType.values()){
        for(var preModScenario:PreModScenario.values()){
          ...
          for(var monitoredConsumerGen:MonitoredConsumerGen.values()){
            ...
            for(var seqContentsScenario:SequenceContentsScenario.values()){
              builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,nestedType,checkedType),preModScenario,monitoredConsumerGen,seqContentsScenario,FunctionCallType.Unboxed));
              builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,nestedType,checkedType),preModScenario,monitoredConsumerGen,seqContentsScenario,FunctionCallType.Boxed));
            }
          }  
        }
      }
  ...
  public void testforEach_ConsumerArgs(ShortSeqMonitor seqMonitor,PreModScenario preModScenario,MonitoredConsumerGen monitoredConsumerGen,SequenceContentsScenario seqContentsScenario,FunctionCallType functionCallType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
  ...
  public void testListadd_int_val(ListAddIntValTestScenario testScenario,ShortInputTestArgType inputArgType,ConstructionArguments constructionArgs){
  ...
  public void testListput_int_val(ListPutIntValTestScenario testScenario,ShortInputTestArgType inputArgType,ConstructionArguments constructionArgs){
    var testMonitor=new InputTestMonitor();
    if(testScenario.nonEmpty){for(int i=0;i<100;++i){testMonitor.collectionAdd(constructionArgs,inputArgType,i);}}
    var expectedException=testScenario.expectedException;
  ...
  public void testCollectionadd_val(CollectionAddValTestScenario testScenario,ShortInputTestArgType inputArgType,ConstructionArguments constructionArgs){
    var testMonitor=new InputTestMonitor();
    if(testScenario.nonEmpty){for(int i=0;i<100;++i){testMonitor.collectionAdd(constructionArgs,inputArgType,i);}}
    var expectedException=testScenario.expectedException;
    if(expectedException==null){
  ...
  public void testclear_void(StructType structType,int numToAdd,CMEScenario modScenario){
    ...
    if(modScenario==CMEScenario.NoMod){
      ...
      offset=constructionArgs.verifyParentPostAlloc(offset);
      if(modScenario==CMEScenario.ModParent){offset=constructionArgs.verifyIndex(offset,ShortInputTestArgType.ARRAY_TYPE,0);}
      offset=constructionArgs.verifyRootPostAlloc(offset);
      if(modScenario==CMEScenario.ModRoot){constructionArgs.verifyIndex(offset,ShortInputTestArgType.ARRAY_TYPE,0);}
    }
  }
  @ParameterizedTest
  @MethodSource("getBasicCollectionTestArguments")
  public void testclone_void(StructType structType,int numToAdd,CMEScenario modScenario){
    ...
    if(modScenario==CMEScenario.NoMod){
      ...
      var cloneSize=((ShortArrSeq)clone).size;
      Assertions.assertEquals(numToAdd,cloneSize);
      if(numToAdd==0)
      {
        Assertions.assertSame(OmniArray.OfShort.DEFAULT_ARR,cloneArr);
      }
      else
      {
  ...
  public void testsize_void(int numToAdd,StructType structType,CMEScenario preModScenario)
  {
      ...
      default:
      {
        for(int i=0;i<numToAdd;++i)
        {
          ...
          switch(structType)
          {
            case CHECKEDSTACK:
            case UNCHECKEDSTACK:
              ((OmniStack.OfShort)constructionArgs.seq).popShort();
              break;
            default:
              ((OmniList.OfShort)constructionArgs.seq).removeShortAt(numToAdd-i-1);
          }
        }
    ...
    int offset=constructionArgs.verifyPreAlloc();
    if(preModScenario!=CMEScenario.NoMod){offset=constructionArgs.verifyAscending(offset,ShortInputTestArgType.ARRAY_TYPE,numToAdd);}
    offset=constructionArgs.verifyParentPostAlloc(offset);
    if(preModScenario==CMEScenario.ModParent){offset=constructionArgs.verifyIndex(offset,ShortInputTestArgType.ARRAY_TYPE,0);}
    offset=constructionArgs.verifyRootPostAlloc(offset);
    if(preModScenario==CMEScenario.ModRoot){offset=constructionArgs.verifyIndex(offset,ShortInputTestArgType.ARRAY_TYPE,0);}
  ...
  public void testtoArray_IntFunction(StructType structType,int seqSize,ObjModScenario arrConstructorModScenario,CMEScenario preModScenario){
    ...
    Class<Short> componentClass=Short.class;
  ...
  public void testItrforEachRemaining(StructType structType,int seqSize,CMEScenario preModScenario,ItrForEachModScenario itrForEachModScenario,ItrType itrType,FunctionCallType functionCallType)
  {
    ...
    if(itrType==ItrType.ListItr)
    {
      itr=((OmniList.OfShort)constructionArgs.seq).listIterator();
    }
    else
    {
    ...
    if(expectedException==null)
    {
      ...
      {
        itr.forEachRemaining((Consumer<? super Short>)consumer::accept);
      }
    }
    else
    {
      ...
      {
        Assertions.assertThrows(expectedException,()->itr.forEachRemaining((Consumer<? super Short>)consumer::accept));
      }
    }
