...
import omni.impl.seq.ShortSeqMonitor.FunctionExceptionScenario;
import omni.impl.seq.ShortSeqMonitor.SequenceContentsScenario;
import omni.impl.seq.ShortSeqMonitor.ListItrSetExceptionScenario;
...
public class ShortArrSeqTest{
  ...
  */
  ...
  public void testListItradd_val(ShortSeqMonitor seqMonitor,PreModScenario preModScenario,SequenceContentsScenario seqContentsScenario,SequenceLocation seqLocationScenario,ShortInputTestArgType inputArgType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
    ShortItrMonitor itrMonitor;
  ...
  static Stream<Arguments> getListItrSetArgs(){
    ...
    for(var checkedType:CheckedType.values()){
      for(var listItrSetExceptionScenario:ListItrSetExceptionScenario.values()){
        ...
        for(var inputArgType:ShortInputTestArgType.values()){
          builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,NestedType.LIST,checkedType),listItrSetExceptionScenario,inputArgType));
          builder.add(Arguments.of(new ShortSeqMonitor(StructType.ARRSEQ,NestedType.SUBLIST,checkedType),listItrSetExceptionScenario,inputArgType));
        }
      }
    }
    return builder.build().parallel();
  ...
  public void testListItrset_val(ShortSeqMonitor seqMonitor,ListItrSetExceptionScenario listItrSetExceptionScenario,ShortInputTestArgType inputArgType){
    int numToAdd=100;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
    ShortItrMonitor itrMonitor=seqMonitor.getListItrMonitor();
    ...
    ShortSeqMonitor.SeqMonitorItr i;
  ...
  @ParameterizedTest
  @MethodSource("getListItrSetArgs")
  public void testListItrSet(ListItrSetTestScenario testScenario,ShortInputTestArgType inputArgType,ConstructionArguments constructionArgs){
