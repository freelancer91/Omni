...
import omni.impl.seq.IntSeqMonitor.FunctionExceptionScenario;
import omni.impl.seq.IntSeqMonitor.SequenceContentsScenario;
import omni.impl.seq.IntSeqMonitor.ListItrSetExceptionScenario;
...
public class IntArrSeqTest{
  ...
  */
  ...
  public void testListItradd_val(IntSeqMonitor seqMonitor,PreModScenario preModScenario,SequenceContentsScenario seqContentsScenario,SequenceLocation seqLocationScenario,IntInputTestArgType inputArgType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
    IntItrMonitor itrMonitor;
  ...
  static Stream<Arguments> getListItrSetArgs(){
    ...
    for(var checkedType:CheckedType.values()){
      for(var listItrSetExceptionScenario:ListItrSetExceptionScenario.values()){
        ...
        for(var inputArgType:IntInputTestArgType.values()){
          builder.add(Arguments.of(new IntSeqMonitor(StructType.ARRSEQ,NestedType.LIST,checkedType),listItrSetExceptionScenario,inputArgType));
          builder.add(Arguments.of(new IntSeqMonitor(StructType.ARRSEQ,NestedType.SUBLIST,checkedType),listItrSetExceptionScenario,inputArgType));
        }
      }
    }
    return builder.build().parallel();
  ...
  public void testListItrset_val(IntSeqMonitor seqMonitor,ListItrSetExceptionScenario listItrSetExceptionScenario,IntInputTestArgType inputArgType){
    int numToAdd=100;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
    IntItrMonitor itrMonitor=seqMonitor.getListItrMonitor();
  ...
  @ParameterizedTest
  @MethodSource("getListItrSetArgs")
  public void testListItrSet(ListItrSetTestScenario testScenario,IntInputTestArgType inputArgType,ConstructionArguments constructionArgs){
