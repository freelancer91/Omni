...
import omni.impl.seq.BooleanSeqMonitor.FunctionExceptionScenario;
import omni.impl.seq.BooleanSeqMonitor.SequenceContentsScenario;
import omni.impl.seq.BooleanSeqMonitor.ListItrSetExceptionScenario;
...
public class BooleanArrSeqTest{
  ...
  */
  ...
  public void testListItradd_val(BooleanSeqMonitor seqMonitor,PreModScenario preModScenario,SequenceContentsScenario seqContentsScenario,SequenceLocation seqLocationScenario,BooleanInputTestArgType inputArgType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
    BooleanItrMonitor itrMonitor;
  ...
  static Stream<Arguments> getListItrSetArgs(){
    ...
    for(var checkedType:CheckedType.values()){
      for(var listItrSetExceptionScenario:ListItrSetExceptionScenario.values()){
        ...
        for(var inputArgType:BooleanInputTestArgType.values()){
          builder.add(Arguments.of(new BooleanSeqMonitor(StructType.ARRSEQ,NestedType.LIST,checkedType),listItrSetExceptionScenario,inputArgType));
          builder.add(Arguments.of(new BooleanSeqMonitor(StructType.ARRSEQ,NestedType.SUBLIST,checkedType),listItrSetExceptionScenario,inputArgType));
        }
      }
    }
    return builder.build().parallel();
  ...
  public void testListItrset_val(BooleanSeqMonitor seqMonitor,ListItrSetExceptionScenario listItrSetExceptionScenario,BooleanInputTestArgType inputArgType){
    int numToAdd=100;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
    BooleanItrMonitor itrMonitor=seqMonitor.getListItrMonitor();
    ...
    BooleanSeqMonitor.SeqMonitorItr i;
