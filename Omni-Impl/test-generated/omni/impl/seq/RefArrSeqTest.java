...
import omni.impl.seq.RefSeqMonitor.FunctionExceptionScenario;
import omni.impl.seq.RefSeqMonitor.SequenceContentsScenario;
import omni.impl.seq.RefSeqMonitor.ListItrSetExceptionScenario;
...
public class RefArrSeqTest{
  ...
  */
  ...
  public void testListItradd_val(RefSeqMonitor seqMonitor,PreModScenario preModScenario,SequenceContentsScenario seqContentsScenario,SequenceLocation seqLocationScenario,RefInputTestArgType inputArgType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
    RefItrMonitor itrMonitor;
  ...
  static Stream<Arguments> getListItrSetArgs(){
    ...
    for(var checkedType:CheckedType.values()){
      for(var listItrSetExceptionScenario:ListItrSetExceptionScenario.values()){
        ...
        for(var inputArgType:RefInputTestArgType.values()){
          builder.add(Arguments.of(new RefSeqMonitor(StructType.ARRSEQ,NestedType.LIST,checkedType),listItrSetExceptionScenario,inputArgType));
          builder.add(Arguments.of(new RefSeqMonitor(StructType.ARRSEQ,NestedType.SUBLIST,checkedType),listItrSetExceptionScenario,inputArgType));
        }
      }
    }
    return builder.build().parallel();
  ...
  public void testListItrset_val(RefSeqMonitor seqMonitor,ListItrSetExceptionScenario listItrSetExceptionScenario,RefInputTestArgType inputArgType){
    int numToAdd=100;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
    RefItrMonitor itrMonitor=seqMonitor.getListItrMonitor();
