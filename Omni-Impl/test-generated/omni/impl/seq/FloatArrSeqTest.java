...
import omni.impl.seq.FloatSeqMonitor.FunctionExceptionScenario;
import omni.impl.seq.FloatSeqMonitor.SequenceContentsScenario;
import omni.impl.seq.FloatSeqMonitor.ListItrSetExceptionScenario;
...
public class FloatArrSeqTest{
  ...
  */
  ...
  public void testListItradd_val(FloatSeqMonitor seqMonitor,PreModScenario preModScenario,SequenceContentsScenario seqContentsScenario,SequenceLocation seqLocationScenario,FloatInputTestArgType inputArgType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
    FloatItrMonitor itrMonitor;
  ...
  static Stream<Arguments> getListItrSetArgs(){
    ...
    for(var checkedType:CheckedType.values()){
      for(var listItrSetExceptionScenario:ListItrSetExceptionScenario.values()){
        ...
        for(var inputArgType:FloatInputTestArgType.values()){
          builder.add(Arguments.of(new FloatSeqMonitor(StructType.ARRSEQ,NestedType.LIST,checkedType),listItrSetExceptionScenario,inputArgType));
          builder.add(Arguments.of(new FloatSeqMonitor(StructType.ARRSEQ,NestedType.SUBLIST,checkedType),listItrSetExceptionScenario,inputArgType));
        }
      }
    }
    return builder.build().parallel();
  ...
  public void testListItrset_val(FloatSeqMonitor seqMonitor,ListItrSetExceptionScenario listItrSetExceptionScenario,FloatInputTestArgType inputArgType){
    int numToAdd=100;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
    FloatItrMonitor itrMonitor=seqMonitor.getListItrMonitor();
    ...
    FloatSeqMonitor.SeqMonitorItr i;
