...
import omni.impl.seq.ByteSeqMonitor.MonitoredConsumerGen;
...
public class ByteArrSeqTest{
  ...
  public void testListItradd_val(ByteSeqMonitor seqMonitor,PreModScenario preModScenario,SequenceContentsScenario seqContentsScenario,SequenceLocation seqLocation,ByteInputTestArgType inputArgType){
  ...
  public void testListItrset_val(ByteSeqMonitor seqMonitor,ListItrSetScenario listItrSetScenario,ByteInputTestArgType inputArgType){
    int numToAdd=100;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
  ...
  public void testItrnext_void(ByteSeqMonitor seqMonitor,IterationScenario itrScenario,SequenceContentsScenario seqContentsScenario,ItrType itrType,ByteOutputTestArgType outputType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
  ...
  public void testListItrprevious_void(ByteSeqMonitor seqMonitor,IterationScenario itrScenario,SequenceContentsScenario seqContentsScenario,ByteOutputTestArgType outputType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
  ...
  public void testItrremove_void(ByteSeqMonitor seqMonitor,ItrRemoveScenario removeScenario,PreModScenario preModScenario,SequenceContentsScenario seqContentsScenario,ItrType itrType,SequenceLocation seqLocation){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
  ...
  public void testListItrpreviousIndex_void_And_ListItrnextIndex_void(ByteSeqMonitor seqMonitor){
    int numToAdd=100;
    for(int i=0;i<numToAdd;++i)
    {
      seqMonitor.add(i);
        ...
        for(var preModScenario:PreModScenario.values()){
          ...
          for(var monitoredConsumerGen:MonitoredConsumerGen.values()){
            ...
            for(var itrType:ItrType.values()){
              ...
              for(var seqContentsScenario:SequenceContentsScenario.values()){
                builder.accept(Arguments.of(new ByteSeqMonitor(StructType.ARRSEQ,nestedType,checkedType),preModScenario,monitoredConsumerGen,itrType,seqContentsScenario,FunctionCallType.Unboxed));
                builder.accept(Arguments.of(new ByteSeqMonitor(StructType.ARRSEQ,nestedType,checkedType),preModScenario,monitoredConsumerGen,itrType,seqContentsScenario,FunctionCallType.Boxed));
              }
            }
          }  
        }
  ...
  public void testItrforEachRemaining_Consumer(ByteSeqMonitor seqMonitor,PreModScenario preModScenario,MonitoredConsumerGen monitoredConsumerGen,ItrType itrType,SequenceContentsScenario seqContentsScenario,FunctionCallType functionCallType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
      ...
      for(var nestedType:NestedType.values()){
        for(var preModScenario:PreModScenario.values()){
          ...
          for(var monitoredConsumerGen:MonitoredConsumerGen.values()){
            ...
            for(var seqContentsScenario:SequenceContentsScenario.values()){
              builder.accept(Arguments.of(new ByteSeqMonitor(StructType.ARRSEQ,nestedType,checkedType),preModScenario,monitoredConsumerGen,seqContentsScenario,FunctionCallType.Unboxed));
              builder.accept(Arguments.of(new ByteSeqMonitor(StructType.ARRSEQ,nestedType,checkedType),preModScenario,monitoredConsumerGen,seqContentsScenario,FunctionCallType.Boxed));
            }
          }  
        }
      }
  ...
  public void testforEach_ConsumerArgs(ByteSeqMonitor seqMonitor,PreModScenario preModScenario,MonitoredConsumerGen monitoredConsumerGen,SequenceContentsScenario seqContentsScenario,FunctionCallType functionCallType){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
  ...
  public void testListadd_int_val(ByteSeqMonitor seqMonitor,ByteInputTestArgType inputArgType,SequenceLocation seqLocation,PreModScenario preModScenario,SequenceContentsScenario seqContentsScenario){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
  ...
  public void testListput_int_val(ByteSeqMonitor seqMonitor,ByteInputTestArgType inputArgType,SequenceLocation seqLocation,PreModScenario preModScenario,SequenceContentsScenario seqContentsScenario){
    int numToAdd=seqContentsScenario.nonEmpty?100:0;
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
            ...
            for(var inputArgType:ByteInputTestArgType.values()){
              for(int initialCapacity=0;initialCapacity<=15;initialCapacity+=5){
                switch(nestedType){
                  case LIST:
                  case STACK:
                    builder.accept(Arguments.of(new ByteSeqMonitor(StructType.ARRSEQ,nestedType,checkedType,initialCapacity),inputArgType,preModScenario,seqContentsScenario));
                    break;
                  case SUBLIST:
                    for(int rootPreAlloc=0;rootPreAlloc<=5;rootPreAlloc+=5){
                      for(int parentPreAlloc=0;parentPreAlloc<=5;parentPreAlloc+=5){
                        for(int parentPostAlloc=0;parentPostAlloc<=5;parentPostAlloc+=5){
                          for(int rootPostAlloc=0;rootPostAlloc<=5;rootPostAlloc+=5){
                            builder.accept(Arguments.of(new ByteSeqMonitor(StructType.ARRSEQ,nestedType,checkedType,initialCapacity,rootPreAlloc,parentPreAlloc,parentPostAlloc,rootPostAlloc),inputArgType,preModScenario,seqContentsScenario));
                          }
                        }
                      }
                    }
  ...
  public void testadd_val(ByteSeqMonitor seqMonitor,ByteInputTestArgType inputArgType,PreModScenario preModScenario,SequenceContentsScenario sequenceContentsScenario){
  ...
  public void testStackpush_val(ByteSeqMonitor seqMonitor,ByteInputTestArgType inputArgType){
    for(int i=0;i<100;++i){
  ...
  public void testcontains_val(ByteSeqMonitor seqMonitor,QueryTester argType,QueryCastType queryCastType,SequenceLocation seqLocation,SequenceContentsScenario seqContentsScenario,PreModScenario preModScenario 
  ){
