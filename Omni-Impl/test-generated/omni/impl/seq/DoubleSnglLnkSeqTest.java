...
public class DoubleSnglLnkSeqTest{
  ...
  public void testItrnext_void(){
    getItrnext_voidArgs().parallel().map(Arguments::get).forEach(args->{
        testItrnext_voidHelper((SeqMonitor)args[0],(IterationScenario)args[1],(SequenceContentsScenario)args[2],(DoubleOutputTestArgType)args[3]);
    });
  }
  ...
  public void testadd_val(){
    getadd_valArgs().parallel().map(Arguments::get).forEach(args->{
        testadd_valHelper((SeqMonitor)args[0],(DoubleInputTestArgType)args[1]);
    });
  }
  ...
  public void testtoArray_void(){
    gettoArray_voidArgs().parallel().map(Arguments::get).forEach(args->{
        testtoArray_voidHelper((SeqMonitor)args[0],(int)args[1],(DoubleOutputTestArgType)args[2]);
    });
  }
  private static void testtoArray_voidHelper
  (SeqMonitor seqMonitor,int numToAdd,DoubleOutputTestArgType outputArgType){
    for(int i=0;i<numToAdd;++i){
      seqMonitor.add(i);
    }
