...
public class DoubleArrDeqTest{
  ...
  private static void testremoveIf_PredicateHelper(CheckedType checkedType,MonitoredRemoveIfPredicateGen monitoredRemoveIfPredicateGen,double threshold,long randSeed,final FunctionCallType functionCallType,int seqSize,int head
    ...
    OmniCollection.OfDouble clone=(OmniCollection.OfDouble)seqMonitor.seq.clone();
  ...
  private static void testreadandwriteObjectHelper(CheckedType checkedType,MonitoredFunctionGen monitoredFunctionGen,int seqSize,int head
    ...
    if(monitoredFunctionGen.expectedException==null){
      ...
      DoubleArrDeq readCol=null;
      try(var ois=new ObjectInputStream(new FileInputStream(file));){
        readCol=(DoubleArrDeq)ois.readObject();
      }catch(Exception e){
        Assertions.fail(e);
        return;
      }
      if(checkedType.checked){
        Assertions.assertEquals(0,((DoubleArrDeq.Checked)readCol).modCount);
      }
      if(seqSize==0){
        Assertions.assertEquals(-1,readCol.tail);
        Assertions.assertEquals(0,readCol.head);
        Assertions.assertSame(OmniArray.OfDouble.DEFAULT_ARR,readCol.arr);
      }else{
        Assertions.assertEquals(0,readCol.head);
        Assertions.assertEquals(seqSize-1,readCol.tail);
        var arr=readCol.arr;
        for(int i=0;i<seqSize;++i){
          Assertions.assertEquals(TypeConversionUtil.convertTodouble(i),arr[i]);
        }
      }
    }else{
            ...
            for(int tmpSeqSize=0;tmpSeqSize<=10;++tmpSeqSize){
              ...
              for(int tmpHead=0;tmpHead<tmpSeqSize;++tmpHead){
                ...
                TestExecutorService.submitTest(()->{
                  ...
                  itrMonitor.verifyIteratorState();
                  seqMonitor.verifyStructuralIntegrity();
                  Assertions.assertEquals(FieldAndMethodAccessor.DoubleArrDeq.AbstractDeqItr.cursor(itr),FieldAndMethodAccessor.DoubleArrDeq.AbstractDeqItr.cursor(itrClone));
                  switch(itrType){
                    case Itr:
                      if(checkedType.checked){
                        Assertions.assertSame(FieldAndMethodAccessor.DoubleArrDeq.Checked.AscendingItr.root(itr),FieldAndMethodAccessor.DoubleArrDeq.Checked.AscendingItr.root(itrClone));
                        Assertions.assertEquals(FieldAndMethodAccessor.DoubleArrDeq.Checked.AscendingItr.lastRet(itr),FieldAndMethodAccessor.DoubleArrDeq.Checked.AscendingItr.lastRet(itrClone));                        
                        Assertions.assertEquals(FieldAndMethodAccessor.DoubleArrDeq.Checked.AscendingItr.modCount(itr),FieldAndMethodAccessor.DoubleArrDeq.Checked.AscendingItr.modCount(itrClone));                        
                      }else{
                        Assertions.assertSame(FieldAndMethodAccessor.DoubleArrDeq.AscendingItr.root(itr),FieldAndMethodAccessor.DoubleArrDeq.AscendingItr.root(itrClone));
                      }
                      break;
                    case DescendingItr:
                      if(checkedType.checked){
                        Assertions.assertSame(FieldAndMethodAccessor.DoubleArrDeq.Checked.DescendingItr.root(itr),FieldAndMethodAccessor.DoubleArrDeq.Checked.DescendingItr.root(itrClone));
                        Assertions.assertEquals(FieldAndMethodAccessor.DoubleArrDeq.Checked.DescendingItr.lastRet(itr),FieldAndMethodAccessor.DoubleArrDeq.Checked.DescendingItr.lastRet(itrClone));                        
                        Assertions.assertEquals(FieldAndMethodAccessor.DoubleArrDeq.Checked.DescendingItr.modCount(itr),FieldAndMethodAccessor.DoubleArrDeq.Checked.DescendingItr.modCount(itrClone));                        
                      }else{
                        Assertions.assertSame(FieldAndMethodAccessor.DoubleArrDeq.DescendingItr.root(itr),FieldAndMethodAccessor.DoubleArrDeq.DescendingItr.root(itrClone));
                      }
                      break;
                    default:
  ...
  private static void testclone_voidHelper(CheckedType checkedType,int seqSize,int head){
    var seqMonitor=new SeqMonitor(checkedType,seqSize,head,seqSize);
    initializeAscending(seqMonitor.seq.arr,head,seqSize);
    var clone=(DoubleArrDeq)seqMonitor.seq.clone();
    if(seqMonitor.checkedType.checked){
      Assertions.assertEquals(0,((DoubleArrDeq.Checked)clone).modCount);
    }
    if((seqSize=seqMonitor.expectedSeqSize)==0)
    {
      Assertions.assertEquals(-1,clone.tail);
      Assertions.assertEquals(0,clone.head);
      Assertions.assertSame(OmniArray.OfDouble.DEFAULT_ARR,clone.arr);
    }
    else
    {
      Assertions.assertEquals(0,clone.head);
      Assertions.assertEquals(seqSize-1,clone.tail);
      double[] origArr,cloneArr=clone.arr;
      ...
      for(int tmpInitialCapacity=0;tmpInitialCapacity<=15;tmpInitialCapacity+=5){
        ...
        TestExecutorService.submitTest(()->{
          ...
          if(checkedType.checked){
            ...
            deq=new DoubleArrDeq(initialCapacity);
          }
          Assertions.assertEquals(-1,deq.tail);
          Assertions.assertEquals(0,deq.head);
  ...
  interface OutputTest{
    void runTest(CheckedType checkedType,int head,int initialSize,DoubleOutputTestArgType outputArgType);
  ...
  interface InputTest{
    void runTest(CheckedType checkedType,int initialCapacity,int head,int initialSize,DoubleInputTestArgType inputType);
    private void runTests(){
      for(var checkedType:CheckedType.values()){
  ...
  private static void initializeAscendingRepeating(double[] arr,int head,int size)
  {
    if(size!=0)
    {
      ...
      for(;;){
        ...
        arr[head]=TypeConversionUtil.convertTodouble(val);
        if(++head==arrLength){
          head=0;
        }
        if(++i==threshold){
  ...
  private static void initializeAscending(double[] arr,int head,int size)
  {
    if(size!=0)
    {
      for(int i=0,arrLength=arr.length;i<size;++i)
      {
        arr[head]=TypeConversionUtil.convertTodouble(i);
        if(++head==arrLength)
        {
          head=0;
        }
  ...
  private static int initializeArrayForQuery(boolean forwardIteration,double[] arr,int head,int size,QueryTester argType,SequenceLocation seqLocation
  ){
  ...
  private static class SeqMonitor extends AbstractDoubleSeqMonitor<DoubleArrDeq>{
    ...
    SeqMonitor(CheckedType checkedType,int capacity,int head,int size)
    {
      ...
      if(size==0)
      {
        this.seq=checkedType.checked?new DoubleArrDeq.Checked(capacity):new DoubleArrDeq(capacity);
      }
      else
      {
        ...
        this.seq=checkedType.checked?new DoubleArrDeq.Checked(head,arr,tail):new DoubleArrDeq(head,arr,tail);
        this.expectedSeqSize=size;
      }
    }
    SeqMonitor(CheckedType checkedType,int initialCapacity){
      super(checkedType);
      this.seq=checkedType.checked?new DoubleArrDeq.Checked(initialCapacity):new DoubleArrDeq(initialCapacity);
      //this.expectedTail=-1;
    }
    SeqMonitor(CheckedType checkedType){
      super(checkedType);
      this.seq=checkedType.checked?new DoubleArrDeq.Checked():new DoubleArrDeq();
      //this.expectedTail=-1;
    }
    ...
    class UncheckedAscendingItrMonitor extends AbstractDoubleSeqMonitor.AbstractItrMonitor{
      int expectedCursor;
      UncheckedAscendingItrMonitor(ItrType itrType,OmniIterator.OfDouble itr,int cursor){
      ...
      void verifyIteratorState(){
        Assertions.assertEquals(expectedCursor,FieldAndMethodAccessor.DoubleArrDeq.AscendingItr.cursor(itr));
    ...
    class CheckedAscendingItrMonitor extends UncheckedAscendingItrMonitor{
      int expectedLastRet;
      CheckedAscendingItrMonitor(ItrType itrType,OmniIterator.OfDouble itr,int cursor){
      ...
      void verifyIteratorState(){
        ...
        Assertions.assertEquals(expectedCursor,FieldAndMethodAccessor.DoubleArrDeq.Checked.AscendingItr.cursor(itr));
      ...
      if(retVal){
        ...
        if(forward){
          ...
          for(int i=0;;){
            ...
            var currVal=TypeConversionUtil.convertTodouble(val);
    ...
    public String toString(){
      return new StringBuilder("DoubleArrDeq").append(checkedType.checked?".Checked{":"{").append(seq.head).append(',').append(expectedSeqSize).append(',').append(seq.tail).append('}').toString();
    }
    void verifyStructuralIntegrity(){
      if(checkedType.checked){
        Assertions.assertEquals(expectedSeqModCount,((DoubleArrDeq.Checked)seq).modCount);
      }
    ...
    void writeObject(ObjectOutputStream oos) throws IOException{
      if(checkedType.checked){
        FieldAndMethodAccessor.DoubleArrDeq.Checked.writeObject(seq,oos);
      }else{
        FieldAndMethodAccessor.DoubleArrDeq.writeObject(seq,oos);
      }
    }
    void verifyRemoval(){
      --expectedSeqSize;
