...
public class FloatArrDeqTest{
    ...
    for(int tail=seqLength-1;tail>=(seqLength>>2);tail-=(seqLength>>1)){
      for(var checkedType:CheckedType.values()){
        ...
        for(int batchIndex=0;batchIndex<batchBound;++batchIndex){
          ...
          TestExecutorService.submitTest(()->AbstractFloatSeqMonitor.verifyLargeStr(string,finalWayPointIndex,finalWayPointBound,verifyItr.getOffset(0)));
        ...
        TestExecutorService.completeAllTests("FloatArrDeq.testMASSIVEtoString_void checkedType="+checkedType+"; tail="+tail);
      }
    }
  }
  ...
  public void testremoveIf_Predicate(){
    ...
    TestExecutorService.completeAllTests("FloatArrDeq.testremoveIf_Predicate");
  }
  private static void testremoveIf_PredicateHelper(CheckedType checkedType,MonitoredRemoveIfPredicateGen monitoredRemoveIfPredicateGen,double threshold,long randSeed,final FunctionCallType functionCallType,int seqSize,int head
    ...
    OmniCollection.OfFloat clone=(OmniCollection.OfFloat)seqMonitor.seq.clone();
            ...
            for(int tmpSeqSize=0;tmpSeqSize<=10;++tmpSeqSize){
              ...
              for(int tmpHead=0;tmpHead<tmpSeqSize;++tmpHead){
                ...
                TestExecutorService.submitTest(()->{
                  ...
                  itrMonitor.verifyIteratorState();
                  seqMonitor.verifyStructuralIntegrity();
                  Assertions.assertEquals(FieldAndMethodAccessor.FloatArrDeq.AbstractDeqItr.cursor(itr),FieldAndMethodAccessor.FloatArrDeq.AbstractDeqItr.cursor(itrClone));
                  switch(itrType){
                    case Itr:
                      if(checkedType.checked){
                        Assertions.assertSame(FieldAndMethodAccessor.FloatArrDeq.Checked.AscendingItr.root(itr),FieldAndMethodAccessor.FloatArrDeq.Checked.AscendingItr.root(itrClone));
                        Assertions.assertEquals(FieldAndMethodAccessor.FloatArrDeq.Checked.AscendingItr.lastRet(itr),FieldAndMethodAccessor.FloatArrDeq.Checked.AscendingItr.lastRet(itrClone));                        
                        Assertions.assertEquals(FieldAndMethodAccessor.FloatArrDeq.Checked.AscendingItr.modCount(itr),FieldAndMethodAccessor.FloatArrDeq.Checked.AscendingItr.modCount(itrClone));                        
                      }else{
                        Assertions.assertSame(FieldAndMethodAccessor.FloatArrDeq.AscendingItr.root(itr),FieldAndMethodAccessor.FloatArrDeq.AscendingItr.root(itrClone));
                      }
                      break;
                    case DescendingItr:
                      if(checkedType.checked){
                        Assertions.assertSame(FieldAndMethodAccessor.FloatArrDeq.Checked.DescendingItr.root(itr),FieldAndMethodAccessor.FloatArrDeq.Checked.DescendingItr.root(itrClone));
                        Assertions.assertEquals(FieldAndMethodAccessor.FloatArrDeq.Checked.DescendingItr.lastRet(itr),FieldAndMethodAccessor.FloatArrDeq.Checked.DescendingItr.lastRet(itrClone));                        
                        Assertions.assertEquals(FieldAndMethodAccessor.FloatArrDeq.Checked.DescendingItr.modCount(itr),FieldAndMethodAccessor.FloatArrDeq.Checked.DescendingItr.modCount(itrClone));                        
                      }else{
                        Assertions.assertSame(FieldAndMethodAccessor.FloatArrDeq.DescendingItr.root(itr),FieldAndMethodAccessor.FloatArrDeq.DescendingItr.root(itrClone));
                      }
                      break;
                    default:
  ...
  public void testConstructor_void(){
    for(var checkedType:CheckedType.values()){
      TestExecutorService.submitTest(()->{
        FloatArrDeq deq;
        if(checkedType.checked){
          Assertions.assertEquals(0,((FloatArrDeq.Checked)(deq=new FloatArrDeq.Checked())).modCount);
        }else{
          deq=new FloatArrDeq();
        }
    ...
    for(var checkedType:CheckedType.values()){
      for(int tmpInitialCapacity=0;tmpInitialCapacity<=15;tmpInitialCapacity+=5){
        ...
        TestExecutorService.submitTest(()->{
          FloatArrDeq deq;
          if(checkedType.checked){
            Assertions.assertEquals(0,((FloatArrDeq.Checked)(deq=new FloatArrDeq.Checked(initialCapacity))).modCount);
          }else{
            deq=new FloatArrDeq(initialCapacity);
          }
          Assertions.assertEquals(-1,deq.tail);
          Assertions.assertEquals(0,deq.head);
  ...
  interface OutputTest{
    void runTest(CheckedType checkedType,int head,int initialSize,FloatOutputTestArgType outputArgType);
  ...
  private static void initializeAscendingRepeating(float[] arr,int head,int size)
  {
    if(size!=0)
    {
      ...
      for(;;){
        ...
        arr[head]=TypeConversionUtil.convertTofloat(val);
        if(++head==arrLength){
          head=0;
        }
        if(++i==threshold){
      ...
      if(retVal){
        ...
        if(forward){
          ...
          for(int i=0;;){
            ...
            var currVal=TypeConversionUtil.convertTofloat(val);
