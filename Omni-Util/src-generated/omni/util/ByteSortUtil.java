...
public final class ByteSortUtil
{
  private ByteSortUtil()
  {
    ...
    public static  void uncheckedUnstableSort(byte[] arr,int offset,int bound,ByteComparator sorter)
    {
       if((--bound)-offset<286)
       {
       ...
       else
       {
         ...
         for(int k=offset;;)
         {
           ...
           switch(
           Integer.signum(sorter.compare((byte)(arr[k]),(byte)(arr[k+1])))
           )
           {
           case -1:
             for(;;)
             {
               if(++k==bound ||
               sorter.compare((byte)(arr[k]),(byte)(arr[k-1]))<0
               )
               {
                 break;
               }
           ...
           default:
             ...
             {
               if(++k==bound||
               sorter.compare((byte)(arr[k-1]),(byte)(arr[k]))<0
               )
               {
                 break;
               }
             }
             OmniArray.OfByte.reverseRange(arr,run[count],k-1);
             break;
           case 0:
             ++k;
             continue;
           ...
           sorter.compare((byte)(arr[r=run[count]]),(byte)(arr[r-1]))<0
           && ++count==67)
           {
             quickSortleftmost(arr,offset,bound,sorter);
             break;
  ...
  public static void uncheckedAscendingSort(byte[] arr,int offset,int bound)
  {
    if((--bound)-offset<30)
    {
      AscendinginsertSort(arr,offset,bound);
    }
    else
    {
  ...
  public static void uncheckedDescendingSort(byte[] arr,int offset,int bound)
  {
    if((--bound)-offset<30)
    {
      DescendinginsertSort(arr,offset,bound);
    }
    else
    {
  ...
  public static  void uncheckedStableSort(byte[] arr,int offset,int bound,ByteComparator sorter)
  {
    int nRemaining;
    if((nRemaining=bound-offset)<32)
    {
      ByteTimSort.binarySort(arr,offset,bound,offset+ByteTimSort.countRunAndMakeAscending(arr,offset,bound,sorter),sorter);
      return;
    }
    new ByteTimSort(arr,nRemaining,sorter,offset,bound);
  }
  private static class ByteTimSort
    extends AbstractTimSort
  {
    ...
    private ByteTimSort(byte[] arr,int nRemaining,ByteComparator sorter,int offset,int bound)
    {
      super(nRemaining);
      this.sorter=sorter;
      this.arr=arr;
      ...
      this.tmp=new byte[tmpLength];
      this.minGallop=7;
      int minRun=AbstractTimSort.minRunLength(nRemaining);
      int runLen;
      do
      {
        if((runLen=ByteTimSort.countRunAndMakeAscending(arr,offset,bound,sorter))<minRun)
        {
          int force;
          ByteTimSort.binarySort(arr,offset,offset+(force=nRemaining<=minRun?nRemaining:minRun),offset+runLen,sorter);
          runLen=force;
        }
    ...
    int mergeAt(int n,int stackSize,int[] runLenAndBase)
    {
      ...
      if((len1-=k)!=0)
      {
        ...
        //{
        //  if(len1<=len2)
        if(len1<=(len2=mergeAtGallopLeft((byte)arr[base1+len1-1],arr,base2,len2,sorter)))
        {
    ...
    private static  int mergeAtGallopLeft(byte key,byte[] arr,int base,int len,ByteComparator sorter)
    {
      ...
      int lastOfs;
      if(
      sorter.compare((byte)(key),(byte)(arr[base+len-1]))>0
      )
      {
        return len;
      }
      ...
      //)
      if(
      sorter.compare((byte)(key),(byte)(arr[base+len-2]))<=0
      )
      {
        lastOfs=-1;
      }
    ...
    private static  int mergeAtGallopRight(byte key,byte[] arr,int base,int len,ByteComparator sorter)
    {
      ...
      int lastOfs;
      if(
      sorter.compare((byte)(key),(byte)(arr[base]))<0
      )
      {
        return 0;
      }
      ...
      //)
      if(
      sorter.compare((byte)(key),(byte)(arr[base+1]))>=0
      )
      {
        ofs=len;
      }
    ...
    private void mergeHi(byte[] arr,int base1,int len1,int base2,int len2)
    {
      ...
      for(;;)
      {
        ...
        do
        {
          ...
          if((count2=len2-gallopLeft((byte)arr[cursor1],tmp,tmpOffset,len2,len2-1,sorter))!=0)
          {
    ...
    private static  int gallopLeft(byte key,byte[] arr,int base,int len,int hint,ByteComparator sorter)
    {
      ...
      )
      {
        ...
        sorter.compare((byte)(key),(byte)(arr[base+hint+1]))>0
        )
        {
          ofs=maxOfs+hint;
        }
  ...
  private static  void AscendinginsertSort(byte[] arr,int begin,int end)
  {
    for(int i=begin,j=i;i!=end;j=++i)
    {
      final byte ai=(byte)arr[i+1];
      byte aj;
      while(
      (ai)<(aj=arr[j])
      )
  ...
  private static  void DescendinginsertSort(byte[] arr,int begin,int end)
  {
    for(int i=begin,j=i;i!=end;j=++i)
    {
      final byte ai=(byte)arr[i+1];
  ...
  private static  void insertSort(byte[] arr,int begin,int end,ByteComparator sorter)
  {
    for(int i=begin,j=i;i!=end;j=++i)
    {
      final byte ai=(byte)arr[i+1];
      byte aj;
      while(
      sorter.compare((byte)(ai),(byte)(aj=arr[j]))<0
      )
      {
        arr[j+1]=aj;
        if(j--==begin)
  ...
  private static void AscendingcountingSort(byte[] arr,int begin,int end)
  {
  ...
  private static void DescendingcountingSort(byte[] arr,int begin,int end)
  {
    int[] count=new int[(Byte.MAX_VALUE+1)-Byte.MIN_VALUE];
    for(int i=begin;;++i)
    {
  ...
  private static  void sentinelInsertSort(byte[] arr,int begin,int end,ByteComparator sorter)
  {
    do
    {
      if(begin>=end)
    ...
    while(
    sorter.compare((byte)(arr[begin]),(byte)(arr[++begin]))<=0
    )
    ;
    for(int k=begin;++begin<=end;k=++begin)
    {
      byte a1,a2;
      if(
      sorter.compare((byte)(a1=(byte)arr[k]),(byte)(a2=(byte)arr[begin]))<0
      )
      {
        a2=a1;
        a1=(byte)arr[begin];
      ...
      byte ak;
      while(
      sorter.compare((byte)(a1),(byte)(ak=(byte)arr[--k]))<0
      )
      {
        arr[k+2]=ak;
      }
      arr[++k+1]=a1;
      while(
      sorter.compare((byte)(a2),(byte)(ak=(byte)arr[--k]))<0
      )
      {
        arr[k+1]=ak;
      }
    ...
    byte ae;
    byte last=(byte)arr[end];
    while(
    sorter.compare((byte)(last),(byte)(ae=(byte)arr[--end]))<0
    )
    {
      arr[end+1]=ae;
    }
  ...
  private static  void quickSortleftmost(byte[] arr,int begin,int end,ByteComparator sorter)
  {
    int length;
    ...
    if(
    sorter.compare((byte)(val2=(byte)arr[e2=(e3=(begin+end)>>>1)-(seventh=(length>>3)+(length>>6)+1)]),(byte)(val1=(byte)arr[e1=e2-seventh]))<0
    )
    {
      byte tmp=(byte)val2;
      val2=val1;
    ...
    if(
    sorter.compare((byte)(val3=(byte)arr[e3]),(byte)(val2))<0
    )
    {
      byte tmp=(byte)val3;
      val3=val2;
      if(
      sorter.compare((byte)(tmp),(byte)(val1))<0
      )
      {
        val2=val1;
        val1=tmp;
    ...
    if(
    sorter.compare((byte)(val4=(byte)arr[e4=e3+seventh]),(byte)(val3))<0
    )
    {
      byte tmp=(byte)val4;
      val4=val3;
      if(
      sorter.compare((byte)(tmp),(byte)(val2))<0
      )
      {
        val3=val2;
        if(
    ...
    if(
    sorter.compare((byte)(val5=(byte)arr[e5=e4+seventh]),(byte)(val4))<0
    )
    {
      byte tmp=(byte)val5;
      val5=val4;
      if(
      sorter.compare((byte)(tmp),(byte)(val3))<0
      )
      {
        val4=val3;
        if(
    ...
    arr[e5]=val5;
    if(
    sorter.compare((byte)(val1),(byte)(val2))==0
    ||
    sorter.compare((byte)(val2),(byte)(val3))==0
    ||
    sorter.compare((byte)(val3),(byte)(val4))==0
    ||
    sorter.compare((byte)(val4),(byte)(val5))==0
    )
    {
      arr[e2]=val2;
      arr[e4]=val4;
  ...
  private static  void quickSort(byte[] arr,int begin,int end,ByteComparator sorter)
  {
    int length;
    if((length=end-begin+1)<47)
    {
  ...
  private static  void quickSortleftmostDualPivot(byte[] arr,int begin,int end,byte pivot1, byte pivot2,int e1,int e5,ByteComparator sorter)
  {
    int less=begin;
    int great=end;
    while(
    sorter.compare((byte)(arr[++less]),(byte)(pivot1))<=0
    )
    {
    }
    while(
    sorter.compare((byte)(arr[--great]),(byte)(pivot2))>=0
    )
    {
    }
    outer: for(int k=less;k<=great;++k)
    {
      byte ak;
      if(
      sorter.compare((byte)(ak=(byte)arr[k]),(byte)(pivot1))<0
      )
      {
        arr[k]=arr[less];
        arr[less]=ak;
      ...
      if(
      sorter.compare((byte)(ak),(byte)(pivot2))>0
      )
      {
        byte ag;
        while(
        sorter.compare((byte)(ag=(byte)arr[great]),(byte)(pivot2))>0
        )
        {
          if(--great<k)
          {
        ...
        if(
        sorter.compare((byte)(ag),(byte)(pivot1))<0
        )
        {
          arr[k]=arr[less];
          arr[less]=ag;
  ...
  private static  void quickSortDualPivot(byte[] arr,int begin,int end,byte pivot1, byte pivot2,int e1,int e5,ByteComparator sorter)
  {
    int less=begin;
    int great=end;
    while(
  ...
  private static  void quickSortleftmostSinglePivot(byte[] arr,int begin,int end,byte pivot,ByteComparator sorter)
  {
    int less=begin;
    int great=end;
    for(int k=less;k<=great;++k)
    {
      byte ak;
      switch(
      Integer.signum(sorter.compare((byte)(ak=arr[k]),(byte)(pivot)))
      )
      {
      case -1:
        arr[k]=arr[less];
      ...
      default:
        forLoop: for(;;)
        {
          byte ag;
          switch(
          Integer.signum(sorter.compare((byte)(pivot),(byte)(ag=arr[great])))
          )
          {
            case 0:
              arr[k]=pivot;
  ...
  private static  void quickSortSinglePivot(byte[] arr,int begin,int end,byte pivot,ByteComparator sorter)
  {
    int less=begin;
    int great=end;
    for(int k=less;k<=great;++k)
  ...
  private static  void merge(byte[] arr,int begin,int end,int[] run,int count,ByteComparator sorter)
  {
    byte odd=0;
    for(int n=1;(n<<=1)<=count;odd^=1){}
    byte[] b;
    int ao,bo,blen;
    var work=new byte[blen=end-begin+1];
    if(odd==0)
    {
    ...
    do
    {
      for(last=0,k=2;k<=count;k+=2)
      {
        ...
        {
          if(q<hi && (p>=mi ||
          sorter.compare((byte)(arr[q+ao]),(byte)(arr[p+ao]))<0
          ))
          {
            b[i+bo]=arr[q++ +ao];
          }
