...
public final class LongSortUtil
{
  ...
  private static void sortmerge(long[] arr,int begin,int end,int[] run,int count)
  {
    ...
    long[] b;
    int ao,bo,blen;
    long[] work=new long[blen=end-begin];
    if(odd==0)
    {
  ...
  private static  int countRunAndMakeAscending(long[] arr,int begin,int end,LongComparator sorter)
  {
    //assert arr!=null;
    //assert begin<end;
    //assert begin>=0;
    ...
    )
    {
      for(;;)
      {
        ...
        if(sorter.compare((long)arr[runHi],(long)arr[++runHi])<=0)
        {
  ...
  private static  void binarySort(long[] arr,int lo,int hi,int begin,LongComparator sorter)
  {
    //assert lo < begin;
    for(;begin<=hi;++begin)
    {
      final long pivot=(long)arr[begin];
      int left=lo;
      for(int right=begin;left<right;)
      {
        final int mid;
        if(sorter.compare(pivot,(long)arr[mid=(left+right)>>>1])<0)
        {
          right=mid;
        }
        else
  ...
  private static class TimSort
  {
    ...
    int[] runBase;
    int[] runLen;
    private TimSort(long[] arr,LongComparator sorter,int nRemaining)
    {
      this.arr=arr;
      this.sorter=sorter;
      int tmpLen;
      this.tmp=new long[tmpLen=nRemaining<512?nRemaining>>>1:256];
      this.tmpLen=tmpLen;
      this.tmpBase=0;
      this.stackSize=0;
      this.minGallop=7;
    ...
    private void mergeLo(long[] arr,int base1,int len1,int base2,int len2,LongComparator sorter)
    {
      long[] tmp;
      int cursor1,dest;
      int cursor2=base2;
    ...
    private void mergeHi(long[] arr,int base1,int len1,int base2,int len2,LongComparator sorter)
    {
      long[] tmp;
      int cursor1,dest;
      int tmpBase;
      ...
      outer:for(;;)
      {
        ...
        do
        {
          ...
          if((count2=len2-gallopLeft((long)arr[cursor1],tmp,tmpBase,len2,len2-1,sorter))!=0)
          {
    ...
    private int mergeAt(int i,int stackSize,int[] runLen)
    {
      ...
      long[] arr;
      int k;
      base1=(base1=runBase[i])+(k=gallopRight((long)(arr=this.arr)[base2=runBase[i+1]],arr,base1,len1,0,sorter));
      if((len1-=k)!=0)
      {
         //assert k>=0;
         if((len2=gallopLeft((long)arr[base1+len1-1],arr,base2,len2,len2-1,sorter))!=0)
         {
           //assert len2>0;
           if(len1>len2)
           {
