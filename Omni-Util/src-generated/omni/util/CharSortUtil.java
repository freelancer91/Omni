...
public final class CharSortUtil
{
  ...
  private static void sortmerge(char[] arr,int begin,int end,int[] run,int count)
  {
    ...
    char[] b;
    int ao,bo,blen;
    char[] work=new char[blen=end-begin];
    if(odd==0)
    {
  ...
  private static  int countRunAndMakeAscending(char[] arr,int begin,int end,CharComparator sorter)
  {
    //assert arr!=null;
    //assert begin<end;
    //assert begin>=0;
    ...
    )
    {
      for(;;)
      {
        ...
        if(sorter.compare((char)arr[runHi],(char)arr[++runHi])<=0)
        {
  ...
  private static  void binarySort(char[] arr,int lo,int hi,int begin,CharComparator sorter)
  {
    //assert lo < begin;
    for(;begin<=hi;++begin)
    {
      final char pivot=(char)arr[begin];
      int left=lo;
      for(int right=begin;left<right;)
      {
        final int mid;
        if(sorter.compare(pivot,(char)arr[mid=(left+right)>>>1])<0)
        {
          right=mid;
        }
        else
  ...
  private static class TimSort
  {
    ...
    int[] runBase;
    int[] runLen;
    private TimSort(char[] arr,CharComparator sorter,int nRemaining)
    {
      this.arr=arr;
      this.sorter=sorter;
      int tmpLen;
      this.tmp=new char[tmpLen=nRemaining<512?nRemaining>>>1:256];
      this.tmpLen=tmpLen;
      this.tmpBase=0;
      this.stackSize=0;
      this.minGallop=7;
    ...
    private void mergeLo(char[] arr,int base1,int len1,int base2,int len2,CharComparator sorter)
    {
      char[] tmp;
      int cursor1,dest;
      int cursor2=base2;
    ...
    private void mergeHi(char[] arr,int base1,int len1,int base2,int len2,CharComparator sorter)
    {
      char[] tmp;
      int cursor1,dest;
      int tmpBase;
      ...
      outer:for(;;)
      {
        ...
        do
        {
          ...
          if((count2=len2-gallopLeft((char)arr[cursor1],tmp,tmpBase,len2,len2-1,sorter))!=0)
          {
    ...
    private int mergeAt(int i,int stackSize,int[] runLen)
    {
      ...
      char[] arr;
      int k;
      base1=(base1=runBase[i])+(k=gallopRight((char)(arr=this.arr)[base2=runBase[i+1]],arr,base1,len1,0,sorter));
      if((len1-=k)!=0)
      {
         //assert k>=0;
         if((len2=gallopLeft((char)arr[base1+len1-1],arr,base2,len2,len2-1,sorter))!=0)
         {
           //assert len2>0;
           if(len1>len2)
           {
